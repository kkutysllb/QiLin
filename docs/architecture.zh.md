...
运行中的 `qilin` 是一棵插件树，由启动时按序叠加的各层组合而成。
...
[`qilin-base`](../packages/bundle/base/README.zh.md) 是 `web`、`headless`、`sdk` 与 `acp` profile 的共享第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`qilin-web-app`](../packages/bundle/web-app/README.zh.md) 增加浏览器应用，[`qilin-headless`](../packages/bundle/headless/README.zh.md) 增加不带服务器的一次性运行器，[`qilin-sdk-app`](../packages/bundle/sdk-app/README.zh.md) 增加 SDK JSON-RPC 服务器，[`qilin-acp-app`](../packages/bundle/acp-app/README.zh.md) 增加仅用于自动化的 ACP 服务器。[`qilin-sdk-minimal`](../packages/bundle/sdk-minimal/README.zh.md) 是刻意保留的例外：一个组合包拥有完整的显式 SDK 配置树，不应用 `qilin-base`。
...
YAML 控制 HMR：base 启用仅监视配置的 `qilin-hmr`；Headless、SDK 和 ACP 禁用它；`sdk-minimal` 不包含它。Profile patch 覆盖这些默认值。HMR 协调监听和重载；启动器提供 profile 数据和就绪信号。
...
受支持的 Node 应用通过具名 `qilin` profile 启动。随附 profile 为 `web`、`headless`、`sdk`、`sdk-minimal` 和 `acp`，可通过 `qilin --profile <name>` 或 `qilin <name>` 选择。`plugin` 表示管理命令；同名 profile 必须用 `--profile plugin` 选择。TypeScript SDK 会解析其同版本 `qilin` 依赖并选择 `sdk`；自定义插件组合继续由 profile 与有序 patch 文件表达，而不是另一个可执行文件或内联应用树。`sdk-minimal` 是位于同一 launcher 后的仓库自有独立组合包，而不是由调用方提供的 Kylin 配置树。
...
[Electron 桌面应用](../apps/desktop/README.zh.md)在签名资源中携带精确匹配的 qilin 生产运行时，并拥有保留的 `$QILIN_HOME/profiles/desktop`。共享 profile helper 初始化其文件、协调已安装 bundle，并解析安装与 bundle 的依赖而不替换 pnpm 拥有的包。CLI 与 Desktop 共享产品数据，可执行包、启用选择与锁文件保持独立。公开 CLI 不能管理 Desktop profile。
...
| 添加持久化终端执行 | 注册 `ctx.terminals` 后端和 `qilin-tool-terminal` |

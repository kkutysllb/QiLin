...
Every capability built so far is a plugin, and `kylin.yml` selects the application's plugin tree. This chapter changes that composition, hot-reloads a plugin, and diagnoses a plugin that never loads.
...
Groups nest a sub-list of entries that load and unload as one unit, and `isolate` gives a group its own instance of a service name — two groups can each see a differently configured `shell` provider without affecting each other. The [Kylin primer](../kylin-primer.md) and the [service isolation example](../user/develop/framework/service.md#service-isolation) cover the details.
...
In `tmp/kylin-tutorial`, write `kylin.yml`:
...
Two support plugins joined the list: HMR logs through the Kylin logger service, so without a console exporter you would not see its messages, and it `inject`s the `timer` service for debouncing — without `@qilin/kylin-plugin-timer` it sits in PENDING forever, silently. That silence is the subject of the next section.

HMR reads Node's loader internals through the Loader's native helper. Run Kylin under tsx:
...
Run it (plain `node --import tsx ../../vendor/kylin/bin.js`; stop with Ctrl-C):

```
needs-timer is PENDING — a required service is missing
```

`inject: ['timer']` has no provider. Add `- name: '@qilin/kylin-plugin-timer'` to the list and the plugin loads. When a plugin does nothing and reports nothing, inspect its fiber state. Iterating without the PENDING filter also shows the loader's own plugins (Loader, Include) as ACTIVE fibers because plugins mount the config file itself.

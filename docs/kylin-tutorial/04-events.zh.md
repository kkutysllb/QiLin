...
创建 `stats.ts`，将它放在 `tmp/kylin-tutorial` 中。它是一项负责计数并在每次变化时发出通知的服务：
...
waterfall 是实现拦截的模式。每个监听器都会收到参数和一个 `next()` continuation；它可以转换 `next()` 的返回值，也可以不调用 `next()` 就直接返回，从而短路链条的其余部分。Kylin 文档把后一种行为称为否决。创建 `waterfall-demo.ts`：
...
由此得到一项纪律：**只负责观察或标注的 waterfall 监听器必须调用 `next()`**；不调用就直接返回代表有意短路。如果日志监听器忘记调用 `next()`，会悄无声息地吞掉所有下游的默认行为。这是本仓库的常设规则（[waterfall 语义](../kylin-primer.zh.md#kylin-waterfall-semantics)）。
...
下一章：[配置](05-config.zh.md)：来自 `kylin.yml` 的插件选项。

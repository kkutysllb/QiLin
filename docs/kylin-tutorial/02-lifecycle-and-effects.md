...
A Kylin plugin can be unloaded by a config edit, hot reload, explicit disposal, or loss of a required service. Registrations made through Kylin APIs are effects and are undone when their owning plugin unloads; resources managed outside those APIs must be wrapped in `ctx.effect()`.

## Effects

For a resource Kylin does not already manage — a timer, a connection, a watcher — wrap it in `ctx.effect()` and return a disposer:
...
Point `kylin.yml` at it:

```yaml
- name: './lifecycle.ts'
```

Run (`node --import tsx ../../vendor/kylin/bin.js`) and you get:
...
For a resource Kylin does not manage, acquire it inside `ctx.effect()` and return a disposer that releases it. Kylin then invokes that release during unloading, including hot reload.

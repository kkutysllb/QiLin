...
In the loader configuration used here, a Kylin plugin module named-exports an `apply` function. When Kylin loads it, it calls `apply` with a **context** — the `ctx` object through which the plugin registers everything it contributes.
...
This tutorial's launcher assembles the application from configuration. Create `kylin.yml`:
...
2. The Loader read `kylin.yml`, resolved `./hello.ts`, and mounted it as a child plugin.
3. Kylin called your `apply(ctx)`.

There is no framework bootstrap code in your file: a plugin describes what it contributes, and `cordis.yml` composes the application. The [`qilin` base](../../packages/bundle/base/cordis.patch.yml), for example, is a longer plugin composition that deployment overlays patch.
...
One caveat worth knowing early: a config entry whose module cannot be **resolved** — a typo'd path or package name — is reported through the Kylin logger service instead of crashing the process, and at boot that report can be lost before a console exporter is watching. If a freshly added entry seems to do nothing, check the spelling first.

...
Create `greet-tool.ts` in `tmp/kylin-tutorial`:
...
The `import type {} from '@qilin/tools'` line pulls in the package's declaration merges so `'tools/result'` and its payload are typed — the same move as chapter 4's `stats.ts` import, at package scale.
...
`@qilin/tools` injects the `systemPrompt` service because tools contribute schemas to the system prompt, so the composition lists its provider too. Without it, the tools plugin remains PENDING as described in [chapter 6](06-composition-and-hmr.md).
...
A real agent is this composition plus more plugins: an LLM adapter, the agent loop, persistence, and an application entry. Compare the [base profile layer](../../packages/bundle/base/kylin.patch.yml) and [headless layer](../../packages/bundle/headless/kylin.patch.yml) — you can read their entries now. Add your `greet-tool.ts` through a small `--patch` overlay.
...
- The generated `kylin-surface` regions on the [subsystem pages](../subsystems/core.md) — everything you can inject and listen to, each on its owning page.

...
真实 agent 就是这套组合再加上更多插件：LLM（大语言模型）适配器、agent loop（智能体循环）、持久化和应用入口。对照 [base profile 层](../../packages/bundle/base/kylin.patch.yml)与 [headless 层](../../packages/bundle/headless/kylin.patch.yml)，你现在已经可以读懂其中各项。通过一个小型 `--patch` overlay 加入 `greet-tool.ts` 即可。
...
- [子系统页面](../subsystems/core.zh.md)上生成的 `kylin-surface` 区块：可以注入和监听的所有内容，各在其所属页面上。

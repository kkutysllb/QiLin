...
English | [中文](kylin-primer.zh.md)

Kylin is the vendored plugin framework underneath DeepSeek Harness. This primer teaches the Kylin ideas a harness plugin author needs before reading the generated service/event reference on the [subsystem pages](subsystems/core.md); the [Kylin tutorial](kylin-tutorial/index.md) walks the same ideas hands-on. The vendored source and sync procedure live in [vendor/README.md](../vendor/README.md).

## Kylin In Five Ideas

- **A plugin is a object that implements Service.** It can be a function with optional `inject` and `apply(ctx)` fields, or a `Service` subclass whose lifecycle Kylin mounts into the current context.
...
Every registration should have a disposer, either by returning one from `ctx.effect()` or using a Kylin helper that does it for you. If teardown order matters, keep the related work in one effect so disposal unwinds in the intended sequence.

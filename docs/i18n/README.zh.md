...
共享生成区域（例如 Kylin 子系统区域）在两种语言间保持每个生成字节相同，仅配对文档路径按语言本地化。整页生成器则可以从相同的结构数据渲染源码维护的双语说明，例如 [persistence-catalog-text.ts](../../scripts/persistence-catalog-text.ts)，并保持 Markdown 结构对应、代码块完全相同。它同时重新生成两页及其一致性记录；评审仍须验证双语说明的含义与术语。

...
The previous tutorials loaded a local plugin through a `--patch` overlay. This tutorial packages it as an installable **bundle**, installs it into a **profile** with `qilin plugin add`, and explains the layer order that determines the composed configuration. It assumes the `qilin` CLI is installed. Complete [plugin configuration](./config.md) first.

To use a fresh source checkout instead, complete the [run-from-source section](../../../../README.md#run-from-source), keep this tutorial's `hello-plugin` directory at the repository root, and run the remaining `qilin ...` commands from there as `pnpm qilin ...`. See [source execution](../../../../apps/cli/reference/README.md#source-execution) for build and launcher behavior.
...
A bundle is what you author and distribute; a profile is what a user boots with `qilin --profile <name>`. Nothing is both.
...
```
hello-plugin/
├── package.json       # declares qilin.bundle
...
A package without the `qilin.bundle` declaration still installs, but only as a plain dependency: `qilin plugin` prints a warning and activates no layer. Use that package format for a library that plugin packages import rather than a plugin users enable.

### The profile manifest

A profile directory holds two files:

- `package.json` — the profile's out-of-tree plugin dependencies (managed by pnpm) plus the `qilin.profile` manifest with its ordered `bundles` list.
- `cordis.patch.yml` — the user's own patch layer, applied after every bundle layer.

You never write a profile manifest by hand: `qilin --profile <name> --from-default-profile <template>` can create one from a shipped application template, while `qilin plugin` creates a base-backed profile and maintains its installed bundle list. The [CLI behavior reference](../../../../apps/cli/reference/README.md#profile-boot) owns the creation rules; the next section shows the plugin path.

## Install into a profile

`qilin plugin --profile <name> <args...>` forwards to pnpm in the profile directory, so every pnpm verb works. From the directory that contains `hello-plugin`, install the package checkout:
...
```sh
qilin --profile demo --dump-config   # shows a "# == qilin-hello-plugin" layer
qilin --profile demo
```

`qilin plugin --profile demo remove qilin-hello-plugin` removes both the dependency and the layer.
...
```sh
qilin plugin --profile demo add github:you/hello-plugin
```
...
- **The user** allowlists the build. pnpm ≥10 refuses to run a git dependency's `prepare` script until it is explicitly allowed, so the first `add` fails; `qilin` points at the fix — copy the exact package key pnpm printed into the profile's `pnpm-workspace.yaml`:

  ```yaml
  allowBuilds:
    qilin-hello-plugin: true
  ```
...
- **Publish to npm** with `lib/` built at `pnpm publish` time; `qilin plugin add your-package` then installs prebuilt code.
- **Ship a tarball** from `pnpm pack`; users run `qilin plugin add ./hello-plugin-0.1.0.tgz`.

...
The first use initializes the profile (with `@qilin/base` as its first bundle), pnpm links the checkout, and `dsh` appends the bundle to `dsh.profile.bundles` because the package declares `dsh.bundle`:
...
1. Each bundle patch named in the profile's `dsh.profile.bundles` list, in list order — `@qilin/base` first, then each installed bundle in the order it was added.
...
In-box bundle names always resolve from the dsh installation itself; pnpm manages only out-of-tree packages, so your bundle can rely on `@qilin/base` being present and current.

...
Keys are write-only. The page receives a redacted descriptor after saving, never the literal secret. The key is stored in `$QILIN_HOME/.credentials.yaml`, while settings retain only its credential reference.

## Add a built-in provider

Choose **Add provider** and pick a provider qilin ships with; the list shows provider ids such as `anthropic`, `openai`, `moonshotai` for Kimi, or `zai` for GLM. Enter its API key and save. The installed catalog supplies the endpoint, protocol, and model list.
...
The generated [plugin configuration catalog](../../config-catalog.md) lists every supported field and default for every plugin; [`qilin-llm-pi-ai`](../../config-catalog.md#qilinllm-pi-ai) is the provider section this page configures. The [`qilin-llm-pi-ai`](../../../packages/llm/llm-pi-ai/README.md) and [`qilin-llm-deepseek`](../../../packages/llm/llm-deepseek/README.md) references own direct `settings.yaml` configuration, catalog resolution, reasoning controls, credentials, and adapter errors.

::: tip Additional settings
The Models page exposes the API key, display name, base URL, API protocol, and each model's id, display name, context window, max output tokens, and input types. Configure reasoning effort levels, request-compatibility switches, headers, timeouts, and retry policy in `$QILIN_HOME/settings.yaml`, the same document the page writes. Edit it directly, or, when the browser runs on the same machine as the server, open it with **Open configuration file** in the Settings header; the adapters re-read it on the next request, so nothing needs a restart. The subsections below cover the fields most gateways need.
:::
...
The checkboxes save `input` for pi-ai models and `inputModalities` for the direct DeepSeek adapter. You can also edit the model in `$QILIN_HOME/settings.yaml`; for example, this custom pi-ai provider declares one text-only model and one vision model:
...
Two account for most of it. A model that declares reasoning has its system prompt sent as `role: "developer"`, which many gateways reject outright, and the output cap is sent as `max_completion_tokens`, which a server that only knows `max_tokens` refuses. The form has no field for either; correct them on the route in `$QILIN_HOME/settings.yaml`:
...
Every switch, its accepted values, and the protocols that take it are listed under `PiAiCompatProfile` in the [generated `qilin-llm-pi-ai` configuration reference](../../config-catalog.md#qilinllm-pi-ai) — which is derived from the source, so it cannot fall behind what the adapter accepts.

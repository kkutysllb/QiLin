...
export function assertBuiltBenchmarkRuntime(
  ...
  if (!moduleUrl.endsWith('.js') || !moduleUrl.includes('/benchmarks/.qilin-build/')) {
    throw new Error(`benchmark worker is not running from benchmarks/.qilin-build: ${moduleUrl}`)
  }

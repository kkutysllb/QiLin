...
def text_to_speech(text: str, voice_type: str) -> bytes | None:
    ...
    cluster = os.getenv("TTS_CLUSTER", "default")
    tts_api_url = os.getenv("TTS_API_URL", "")

    if not app_id or not access_token or not tts_api_url:
        raise ValueError(
            "TTS_APPID, TTS_ACCESS_TOKEN, and TTS_API_URL environment variables must be set"
        )
...
def tts_node(script: Script, max_workers: int = 4) -> list[bytes]:
    ...
    if not os.getenv("TTS_APPID") or not os.getenv("TTS_ACCESS_TOKEN") or not os.getenv("TTS_API_URL"):
        raise ValueError(
            "Missing required environment variables: TTS_APPID, TTS_ACCESS_TOKEN, and TTS_API_URL must be set"
        )
    ...
    if not audio_chunks:
        raise ValueError(
            ...
            "Please check TTS_APPID, TTS_ACCESS_TOKEN, and TTS_API_URL environment variables."
        )

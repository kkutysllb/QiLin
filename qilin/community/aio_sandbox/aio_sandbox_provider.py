...
class SandboxIdentityCollisionError(RuntimeError):
    ...
    def __init__(
        ...
        super().__init__(
            f"sandbox ID collision for {sandbox_id}: tracked identity is {stored_key!r}, requested identity is {requested_key!r}"
        )
        self.sandbox_id = sandbox_id
...
async def _acquire_thread_lock_async(lock: threading.Lock) -> None:
    ...
    try:
        ...
        acquire_future.add_done_callback(
            lambda task: _release_cancelled_lock_acquire(lock, task)
        )
        raise
...
def _release_cancelled_lock_acquire(
    lock: threading.Lock, task: asyncio.Future[bool]
) -> None:
...
class AioSandboxProvider(WarmPoolLifecycleMixin[SandboxInfo], SandboxProvider):
    ...
    def __init__(self):
        ...
        self._sandbox_infos: dict[
            str, SandboxInfo
        ] = {}  # sandbox_id -> SandboxInfo (for destroy)
        self._thread_sandboxes: dict[
            tuple[str, str], str
        ] = {}  # (user_id, thread_id) -> sandbox_id
        self._thread_locks: dict[
            tuple[str, str], threading.Lock
        ] = {}  # (user_id, thread_id) -> in-process lock
        self._last_activity: dict[
            str, float
        ] = {}  # sandbox_id -> last activity timestamp
        ...
        self._ownership: SandboxOwnershipStore = make_sandbox_ownership_store(
            self._ownership_config, owner_id=self._owner_id
        )
    ...
    def _create_backend(self) -> SandboxBackend:
        ...
        if provisioner_url:
            logger.info(
                f"Using remote sandbox backend with provisioner at {provisioner_url}"
            )
            api_key = self._config.get("provisioner_api_key", "")
            return RemoteSandboxBackend(
                provisioner_url=provisioner_url, api_key=api_key
            )
    ...
    def _load_config(self) -> dict:
        ...
        return {
            ...
            "container_prefix": sandbox_config.container_prefix
            ...
            "provisioner_api_key": getattr(sandbox_config, "provisioner_api_key", None)
            or "",
        }
    ...
    def _publish_ownership(self, sandbox_id: str) -> None:
        ...
        """
        with self._lock:
            self._acquire_inflight[sandbox_id] = (
                self._acquire_inflight.get(sandbox_id, 0) + 1
            )
        try:
    ...
    def _reserve_local_teardown(
        self, sandbox_id: str, still_reapable: Callable[[], bool]
    ) -> bool:
    ...
    def _claim_ownership(self, sandbox_id: str, *, for_destroy: bool = False) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Sandbox ownership claim failed for %s (treating as not owned): %s",
                sandbox_id,
                e,
            )
            return False
    ...
    def _release_ownership(self, sandbox_id: str) -> None:
        try:
            ...
            logger.warning(
                "Failed to release sandbox ownership for %s: %s", sandbox_id, e
            )
    ...
    def _refresh_ownership(self, sandbox_id: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Could not renew sandbox ownership for %s, will retry: %s",
                sandbox_id,
                e,
            )
            return True
        ...
        if outcome is RenewOutcome.LAPSED:
            ...
            try:
                if self._ownership.claim(sandbox_id):
                    logger.info(
                        "Re-established a lapsed ownership lease for %s", sandbox_id
                    )
                    return True
            except OwnershipBackendError as e:
                logger.warning(
                    "Could not re-establish lapsed lease for %s, will retry: %s",
                    sandbox_id,
                    e,
                )
                return True
            logger.warning(
                "Lapsed ownership lease for %s was taken by a peer", sandbox_id
            )
            return False
        return False
    ...
    def _held_teardown_lease(self, sandbox_id: str):
            ...
            try:
                while not stop.wait(interval):
                    try:
                        ...
                        logger.warning(
                            "Could not refresh the teardown lease for %s, will retry: %s",
                            sandbox_id,
                            e,
                        )
            finally:
        ...
        beater = threading.Thread(
            target=beat, name="sandbox-teardown-lease", daemon=True
        )
        beater.start()
        try:
            yield
    ...
    def _adoptable_after_grace(self, sandbox_id: str, now: float) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Could not read sandbox ownership for %s during reconciliation (deferring adoption): %s",
                sandbox_id,
                e,
            )
            return False
    ...
    def _reconcile_orphans(self) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Failed to enumerate running containers during startup reconciliation: {e}"
            )
            return
        ...
        self._unowned_since = {
            sid: seen for sid, seen in self._unowned_since.items() if sid in running_ids
        }
        ...
        for info in running:
            age = (
                current_time - info.created_at if info.created_at > 0 else float("inf")
            )
            if not self._adoptable_after_grace(info.sandbox_id, current_time):
                deferred += 1
                logger.debug(
                    "Deferring container %s during reconciliation: owned, or not yet past the recovery grace",
                    info.sandbox_id,
                )
                continue
            ...
            if not self._claim_ownership(info.sandbox_id):
                skipped_live += 1
                logger.debug(
                    "Skipping container %s during reconciliation: owned by another instance",
                    info.sandbox_id,
                )
                continue
            ...
            with self._lock:
                if (
                    info.sandbox_id in self._sandboxes
                    or info.sandbox_id in self._warm_pool
                ):
                    continue
                if self._being_torn_down_locally(info.sandbox_id):
                    ...
                    deferred += 1
                    logger.debug(
                        "Deferring container %s during reconciliation: this instance is tearing it down",
                        info.sandbox_id,
                    )
                    continue
            ...
            adopted += 1
            logger.info(
                f"Adopted container {info.sandbox_id} into warm pool (age: {age:.0f}s)"
            )
    ...
    def _assert_active_identity_available_locked(
        ...
        if stored_key is None:
            matching_keys = [
                key
                for key, mapped_id in self._thread_sandboxes.items()
    ...
    def _get_extra_mounts(
        self, thread_id: str | None, *, user_id: str | None = None
    ...
    @staticmethod
    def _dedupe_mounts_by_container_path(
        mounts: list[tuple[str, str, bool]],
    ) -> list[tuple[str, str, bool]]:
    ...
    def _get_thread_mounts(
        ...
        return [
            (
                paths.host_sandbox_work_dir(thread_id, user_id=effective_user_id),
                f"{VIRTUAL_PATH_PREFIX}/workspace",
                False,
            ),
            (
                paths.host_sandbox_uploads_dir(thread_id, user_id=effective_user_id),
                f"{VIRTUAL_PATH_PREFIX}/uploads",
                False,
            ),
            (
                paths.host_sandbox_outputs_dir(thread_id, user_id=effective_user_id),
            ...
            (
                paths.host_acp_workspace_dir(thread_id, user_id=effective_user_id),
                "/mnt/acp-workspace",
                True,
            ),
        ]
    ...
    def _get_skills_mounts(
        *, user_id: str | None = None
        ...
        try:
            ...
            host_skills_root = os.environ.get("QILIN_HOST_SKILLS_PATH") or str(
                skills_path
            )
            ...
            if (
                user_should_see_legacy_skills(
                    effective_user_id, host_path=str(skills_path)
                )
                and legacy_skills_path.exists()
            ):
                mounts.append(
                    (
    ...
    def _get_user_skill_mounts(
        *, user_id: str | None = None
        ...
        try:
            ...
            return [
                (
                    paths.host_integration_skills_dir(),
    ...
    @staticmethod
    def _get_lark_cli_runtime_mounts(
        *, user_id: str | None = None
    ) -> list[tuple[str, str, bool]]:
        ...
        try:
            ...
            mounts = [
                (
                    paths.host_user_integration_config_dir(
                        effective_user_id, LARK_CLI_INTEGRATION_ID
                    ),
                    LARK_CLI_SANDBOX_CONFIG_DIR,
                    True,
                ),
                (
                    paths.host_user_integration_data_dir(
                        effective_user_id, LARK_CLI_INTEGRATION_ID
                    ),
                    LARK_CLI_SANDBOX_DATA_DIR,
                    False,
    ...
    def _start_lease_renewal(self) -> None:
        ...
        logger.info(
            ...
            self._ownership_config.renewal_interval_seconds
            * self._ownership_config.ttl_multiplier,
        )
    ...
    def _renew_owned_leases(self) -> None:
        ...
        for sandbox_id in owned_ids:
            ...
            if not self._refresh_ownership(sandbox_id):
                logger.warning(
                    "Lost sandbox ownership lease for %s; dropping it from this instance",
                    sandbox_id,
                )
                self._forget_lost_sandbox(sandbox_id, expected_epoch=epoch)

    def _forget_lost_sandbox(
        self, sandbox_id: str, *, expected_epoch: int | None = None
    ) -> None:
        ...
        with self._lock:
            ...
            if sandbox_id in self._local_teardown:
                logger.debug(
                    "Not dropping sandbox %s: this instance is tearing it down",
                    sandbox_id,
                )
                return
            ...
            if sandbox_id in self._acquire_inflight:
                logger.info(
                    "Not dropping sandbox %s: an acquire is publishing ownership for it",
                    sandbox_id,
                )
                return
            if (
                expected_epoch is not None
                and self._acquire_epoch.get(sandbox_id, 0) != expected_epoch
            ):
                logger.info(
                    "Not dropping sandbox %s: this instance re-acquired it after the lease check",
                    sandbox_id,
                )
                return
        ...
        if sandbox is not None:
            try:
                ...
                logger.warning(
                    f"Error closing sandbox {sandbox_id} after losing its lease: {e}"
                )
    ...
    def _cleanup_idle_sandboxes(self, idle_timeout: float) -> None:
        ...
        with self._lock:
            ...
            for sandbox_id, last_activity in self._last_activity.items():
                ...
                if idle_duration > idle_timeout:
                    ...
                    logger.info(
                        f"Sandbox {sandbox_id} idle for {idle_duration:.1f}s, marking for destroy"
                    )
        ...
        def still_idle(sandbox_id: str) -> bool:
            ...
            if last_activity is None:
                ...
                logger.info(
                    f"Sandbox {sandbox_id} already gone before idle destroy, skipping"
                )
                return False
            if (time.time() - last_activity) < idle_timeout:
                # Re-acquired (activity updated) since the snapshot — skip.
                logger.info(
                    f"Sandbox {sandbox_id} was re-acquired before idle destroy, skipping"
                )
                return False
            return True
        ...
        for sandbox_id in active_to_destroy:
            try:
                ...
                self._destroy_tracked(
                    sandbox_id, still_reapable=lambda sid=sandbox_id: still_idle(sid)
                )
            except Exception as e:
    ...
    def _reap_expired_warm(self, idle_timeout: float | None = None) -> None:
        ...
        timeout = float(
            self._config.get("idle_timeout", DEFAULT_IDLE_TIMEOUT)
    ...
    def _evict_oldest_warm(self) -> str | None:
        ...
        with self._lock:
            ...
            candidates = [
                (sandbox_id, entry)
                for sandbox_id, (entry, _) in sorted(
                    self._warm_pool.items(), key=lambda item: item[1][1]
                )
            ]
    ...
    def _sandbox_id_for_thread(self, thread_id: str | None, user_id: str | None) -> str:
        ...
        return (
            self._deterministic_sandbox_id(
                thread_id, self._effective_acquire_user_id(user_id)
            )
            if thread_id
        ...
        *,
        ...
        with self._lock:
            ...
            if self._being_torn_down_locally(existing_id):
                ...
                logger.info(
                    "Cached sandbox %s is being destroyed by this instance; not reusing it",
                    existing_id,
                )
                return None
        ...
        alive = (
            self._check_tracked_sandbox_alive(existing_id, info)
            if info is not None
            else True
        )
        if alive is False:
        ...
        with self._lock:
            ...
            logger.info(
                f"Reusing in-process sandbox {existing_id} for user/thread {effective_user_id}/{thread_id}{suffix}"
            )
        ...
        try:
            ...
            logger.info(
                "Cached sandbox %s is being destroyed by another instance; not reusing it",
                existing_id,
            )
        ...
        with self._lock:
            if self._being_torn_down_locally(existing_id):
                ...
                logger.info(
                    "Cached sandbox %s was reserved for teardown while publishing ownership; not reusing it",
                    existing_id,
                )
                return None
            if existing_id not in self._sandboxes:
                ...
                logger.info(
                    "Cached sandbox %s was dropped while publishing ownership; falling through to discovery",
                    existing_id,
                )
                return None
        return existing_id
        ...
        *,
        ...
        with self._lock:
            ...
            if self._being_torn_down_locally(sandbox_id):
                ...
                logger.info(
                    "Warm-pool sandbox %s is being destroyed by this instance; not reclaiming it",
                    sandbox_id,
                )
                return None
        ...
        try:
            ...
            logger.info(
                "Warm-pool sandbox %s is being destroyed by another instance; not reclaiming it",
                sandbox_id,
            )
        ...
        with self._lock:
            if self._being_torn_down_locally(sandbox_id):
                ...
                logger.info(
                    "Warm-pool sandbox %s was claimed for teardown while publishing ownership; not reclaiming it",
                    sandbox_id,
                )
                return None
            self._assert_warm_identity_available_locked(sandbox_id, key)
        ...
        logger.info(
            f"Reclaimed warm-pool sandbox {sandbox_id} for user/thread {effective_user_id}/{thread_id}{suffix}"
        )
        return sandbox_id
    ...
    def _recheck_cached_sandbox(
        ...
        return self._reuse_in_process_sandbox(
            thread_id, user_id=user_id, post_lock=True
    ...
    def _register_discovered_sandbox(
        self, thread_id: str, info: SandboxInfo, *, user_id: str
    ) -> str:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Error closing sandbox {info.sandbox_id} after failed ownership publish: {e}"
                )
            raise

        logger.info(
            f"Discovered existing sandbox {info.sandbox_id} for user/thread {user_id}/{thread_id} at {info.sandbox_url}"
        )
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Error closing sandbox {sandbox_id} during ownership rollback: {e}"
                )
            try:
        ...
        logger.info(
            f"Created sandbox {sandbox_id} for thread {thread_id} at {info.sandbox_url}"
        )
        return sandbox_id

    def _check_tracked_sandbox_alive(
        self, sandbox_id: str, info: SandboxInfo
    ) -> bool | None:
        ...
        *,
        ...
        with self._lock:
            ...
            if (
                ...
                and active_info is not expected_info
                and warm_info is not expected_info
            ):
    ...
    def _drop_unhealthy_sandbox(
        ...
        if not self._reserve_local_teardown(sandbox_id, lambda: True):
            logger.info(
                f"Skipped dropping sandbox {sandbox_id}: already being torn down by this instance"
            )
            return
        try:
            self._drop_unhealthy_reserved(
                sandbox_id, reason, expected_info=expected_info
            )
        finally:
            self._finish_local_teardown(sandbox_id)

    def _drop_unhealthy_reserved(
        self, sandbox_id: str, reason: str, *, expected_info: SandboxInfo | None = None
    ) -> None:
        sandbox, info, removed = self._remove_tracked_sandbox(
            sandbox_id, expected_info=expected_info
        )
        if not removed:
            logger.info(
                f"Skipped dropping sandbox {sandbox_id}: tracked info changed after health check"
            )
            return
        ...
        if info is not None:
            ...
            if self._claim_ownership(sandbox_id, for_destroy=True):
                try:
                    ...
                    logger.warning(
                        f"Error destroying unhealthy sandbox {sandbox_id}: {e}"
                    )
            else:
                logger.info(
                    "Not destroying unhealthy sandbox %s: owned by another instance",
                    sandbox_id,
                )
    ...
    def _destroy_warm_entry(
        self,
        sandbox_id: str,
        entry: SandboxInfo,
        *,
        reason: str,
        still_reapable: Callable[[], bool],
    ) -> bool:
        ...
        if not self._reserve_local_teardown(sandbox_id, still_reapable):
            logger.info(
                "Refusing to destroy warm-pool sandbox %s for %s: reclaimed by this instance",
                sandbox_id,
                reason,
            )
            return False
        ...
        try:
            if not self._claim_ownership(sandbox_id, for_destroy=True):
                logger.info(
                    "Refusing to destroy warm-pool sandbox %s for %s: owned by another instance",
                    sandbox_id,
                    reason,
                )
                return False
            ...
            try:
                ...
                if reason == "idle_timeout":
                    logger.error(
                        f"Failed to destroy idle warm-pool sandbox {sandbox_id}: {e}"
                    )
                elif reason == "replica_enforcement":
                    logger.error(
                        f"Failed to destroy warm-pool sandbox {sandbox_id}: {e}"
                    )
                else:
                    logger.error(
                        f"Failed to destroy warm-pool sandbox {sandbox_id} for {reason}: {e}"
                    )
                return False
    ...
    def _acquire_internal(self, thread_id: str | None, *, user_id: str) -> str:
        ...
        reclaimed_id = self._reclaim_warm_pool_sandbox(
            thread_id, sandbox_id, user_id=user_id
        )
    ...
    async def _acquire_internal_async(
        ...
        cached_id = await asyncio.to_thread(
            self._reuse_in_process_sandbox, thread_id, user_id=user_id
        )
        if cached_id is not None:
            return cached_id
        ...
        reclaimed_id = await asyncio.to_thread(
            self._reclaim_warm_pool_sandbox, thread_id, sandbox_id, user_id=user_id
        )
        if reclaimed_id is not None:
            return reclaimed_id
    ...
    def _discover_or_create_with_lock(
        self, thread_id: str, sandbox_id: str, *, user_id: str | None = None
    ) -> str:
        ...
        paths.ensure_thread_dirs(thread_id, user_id=effective_user_id)
        lock_path = (
            paths.thread_dir(thread_id, user_id=effective_user_id)
            / f"{sandbox_id}.lock"
        )
        ...
        with open(lock_path, "a", encoding="utf-8") as lock_file:
            ...
            try:
                ...
                cached_id = self._recheck_cached_sandbox(
                    thread_id, sandbox_id, user_id=effective_user_id
                )
                if cached_id is not None:
                    return cached_id
                ...
                if discovered is not None:
                    return self._register_discovered_sandbox(
                        thread_id, discovered, user_id=effective_user_id
                    )
        ...
        try:
            ...
            if discovered is not None:
                ...
                return await asyncio.to_thread(
                    self._register_discovered_sandbox,
                    thread_id,
                    discovered,
                    user_id=effective_user_id,
                )
    ...
    def _create_sandbox(
        self, thread_id: str | None, sandbox_id: str, *, user_id: str | None = None
    ) -> str:
        ...
        if not wait_for_sandbox_ready(info.sandbox_url, timeout=60):
            self._backend.destroy(info)
            raise RuntimeError(
                f"Sandbox {sandbox_id} failed to become ready within timeout at {info.sandbox_url}"
            )

        return self._register_created_sandbox(
            thread_id, sandbox_id, info, user_id=effective_user_id
        )
    ...
    async def _create_sandbox_async(
        ...
        provision_lark_cli_runtime = await asyncio.to_thread(
            self._lark_integration_active, effective_user_id
        )
        provision_lark_cli_broker = await asyncio.to_thread(
            self._lark_broker_active, effective_user_id
        )
        ...
        return await asyncio.to_thread(
            self._register_created_sandbox,
            thread_id,
            sandbox_id,
            info,
            user_id=effective_user_id,
    ...
    def release(self, sandbox_id: str) -> None:
        ...
        with self._lock:
            ...
            if info and sandbox_id not in self._warm_pool:
                ...
                self._warm_pool_identity[sandbox_id] = (
                    thread_keys_to_remove[0]
                    if thread_keys_to_remove
        ...
        if sandbox is not None:
            ...
            try:
                ...
                logger.warning(
                    f"Error closing sandbox {sandbox_id} during release: {e}"
                )
        ...
        if info is not None:
            ...
            if not self._refresh_ownership(sandbox_id):
                logger.warning(
                    "Sandbox %s is owned by another instance; releasing it from this warm pool",
                    sandbox_id,
                )
                self._forget_lost_sandbox(sandbox_id, expected_epoch=epoch)

        logger.info(
            f"Released sandbox {sandbox_id} to warm pool (container still running)"
        )
    ...
    def _destroy_tracked(
        ...
        if not self._reserve_local_teardown(sandbox_id, still_reapable):
            logger.info(
                "Skipping destroy of sandbox %s: re-acquired by this instance or already being torn down",
                sandbox_id,
            )
            return
    ...
    def _destroy_reserved(self, sandbox_id: str) -> None:
        ...
        if not self._claim_ownership(sandbox_id, for_destroy=True):
            logger.warning(
                "Refusing to destroy sandbox %s: owned by another instance", sandbox_id
            )
            return
        ...
        if sandbox is not None:
            ...
            try:
                ...
                logger.warning(
                    f"Error closing sandbox {sandbox_id} during destroy: {e}"
                )
    ...
    def shutdown(self) -> None:
        ...
        for sandbox_id in sandbox_ids:
            try:
                ...
                logger.error(
                    f"Failed to destroy sandbox {sandbox_id} during shutdown: {e}"
                )
        ...
        for sandbox_id, (info, _) in warm_items:
            ...
            self._destroy_warm_entry(
                sandbox_id, info, reason="shutdown", still_reapable=lambda: True
            )
        ...
        try:
            ...
            logger.warning(
                f"Error closing sandbox ownership store during shutdown: {e}"
            )

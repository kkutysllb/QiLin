...
def _is_human_clarification_response(additional_kwargs: Any) -> bool:
    ...
    Host-agnostic structural mirror of qilin's ``read_human_input_response``
...
def filter_messages_for_memory(messages: list[Any], *, should_keep_hidden_message: Any = None) -> list[Any]:
    ...
    default via a host-agnostic structural check (mirrors qilin's
    ...
    for msg in messages:
        ...
        if msg_type == "human":
            ...
            if additional_kwargs.get(HIDE_FROM_UI_KEY):
                ...
                # overrides this (production QiLinMem injects one delegating to
...
def detect_correction(messages: list[Any], *, patterns: list[re.Pattern[str]] | None = None) -> bool:
    ...
    already resolved ``QiLinMemConfig.patterns_dir``); ``None`` loads the bundled

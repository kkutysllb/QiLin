...
class MemoryUpdater:
    ...
    def __init__(self, config: QiLinMemConfig, storage: MemoryStorage, llm: Any = None, *, prompts_dir: str | None = None, callbacks: Any = None):
        """Initialize the memory updater with injected config + storage + llm (DI).

        Args:
            config: QiLinMem private configuration.
            storage: Memory storage instance (owned by QiLinMem, injected here).
            llm: The chat model for memory extraction (owned by QiLinMem, injected
            ...
            callbacks: Optional ``MemoryCallbacks`` (owned by QiLinMem, injected
    ...
    def _do_update_memory_sync(
        ...
        *,
        ...
        host-injected ``trace_context_manager`` hook (``None`` when QiLinMem runs
        standalone, outside the qilin factory) binds ``trace_id`` for the
    ...
    def _do_update_memory_sync_impl(
        ...
        *,
        ...
        try:
            ...
            # ``signals`` (detected on the full conversation in QiLinMem) already
            ...
            model = self._llm
            if model is None:
                raise RuntimeError("QiLinMem memory update requested but no LLM is configured (set memory.backend_config.model in config).")

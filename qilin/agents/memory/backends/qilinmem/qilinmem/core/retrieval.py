"""FTS5-based retrieval engine for QiLinMem.
...
def create_fts5_retrieval(config: Any) -> FTS5RetrievalAdapter | None:
    """Build QiLinMem's bundled adapter, using a persistent derived index.

    Standalone ``QiLinMem`` instances with no configured storage root use an
    ...
    try:
        ...
        if db_path == ":memory:":
            logger.warning("SQLite FTS5 is unavailable; QiLinMem will use substring retrieval: %s", exc)
            return None
        ...
        try:
            ...
            logger.warning(
                "SQLite FTS5 index recreation failed; QiLinMem will use substring retrieval: %s",
                retry_exc,
            )
            return None

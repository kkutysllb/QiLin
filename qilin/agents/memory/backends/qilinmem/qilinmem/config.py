"""QiLinMem backend configuration (parsed from ``MemoryConfig.backend_config``).

QiLinMem-private config lives here, NOT on the shared ``MemoryConfig`` (which
only carries host-shared fields: ``enabled`` / ``injection_enabled`` /
``manager_class`` / ``backend_config``). The factory passes ``backend_config``
(a dict) to ``QiLinMem.__init__``, which parses it into a ``QiLinMemConfig``.
Defaults let QiLinMem run with zero ``backend_config``.
...
``memory.backend_config.<field>``). ``model`` is a nested ``QiLinMemModelConfig``
...
QiLinMem default); tracing is via the base ``MemoryManager.callbacks`` field
(``on_memory_llm_call`` before the LLM call), not a QiLinMemConfig slot.
"""
...
class QiLinMemModelConfig(BaseModel):
    """QiLinMem's memory-update LLM config (langchain ``init_chat_model`` params)."""
...
class QiLinMemConfig(BaseModel):
    """QiLinMem-private configuration (self-contained, host-agnostic)."""

    # ── Storage ──────────────────────────────────────────────────────────
    storage_path: str = Field(
        default="",
        description=("QiLinMem data root. Empty = default (``$DEERMEM_DATA_DIR`` or ``~/.qilinmem/``); per-user memory at ``{root}/users/{user_id}/memory.json``. Any value (absolute or relative) is used as the root directory."),
    )
    ...
    retrieval_adapter: str = Field(
        default="fts5",
        description="Retrieval adapter factory: 'fts5' (default), an empty string to disable, or a dotted factory receiving QiLinMemConfig and implementing RetrievalPort.",
    )
    # ── Queue ────────────────────────────────────────────────────────────
    ...
    model: QiLinMemModelConfig = Field(
        default_factory=QiLinMemModelConfig,
        description=(
            ...
            "when ``host_llm`` is also absent (standalone QiLinMem) an update raises "
            "but non-LLM ops still work."
        ),
    )
    # ── Hooks (optional host-injected callables; None = QiLinMem defaults) ──
    ...
    host_llm: Any = Field(
        ...
        description=(
            ...
            "UX). The qilin factory injects its default model here when "
    ...
    trace_context_manager: Any = Field(
        ...
        description=(
            ...
            "trace correlation. None = no binding (QiLinMem standalone; trace_id "
    ...
    def _check_storage_path_is_directory(self) -> QiLinMemConfig:
        """QiLinMem treats ``storage_path`` as a root DIRECTORY (per-user memory
        ...
        fires even when QiLinMem is built standalone / bypassing the factory.
        ...
        if self.storage_path:
            ...
            if resolved.is_file():
                raise ValueError(
                    ...
                    f"resolves to an existing file {resolved}; QiLinMem treats "
    ...
    @classmethod
    def from_backend_config(cls, backend_config: dict[str, Any] | None) -> QiLinMemConfig:
        ...
        if unknown:
            logger.warning(
                "Unknown backend_config keys ignored by QiLinMem; check for typos: %s",
                unknown,
            )
        return cls(**known)

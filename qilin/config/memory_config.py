...
QiLinMem-private fields live in ``backends/qilinmem/config.py`` (``QiLinMemConfig``),
...
makes backends swappable and portable (QiLinMem's knobs do not leak onto the
shared contract).
"""
...
# QiLinMem-private fields that used to live at the top level of `memory:` in
...
_LEGACY_QILINMEM_FIELDS = frozenset(
    {
        "storage_path",
        "storage_class",
        "debounce_seconds",
...
class MemoryConfig(BaseModel):
    ...
    manager_class: str = Field(
        ...
        description=(
            ...
            "e.g. `qilinmem` / `noop`) or a dotted import path to a "
    ...
    backend_config: dict[str, Any] = Field(
        ...
        description=(
            ...
            "self-interprets it (QiLinMem parses it into `QiLinMemConfig`). Values "
...
def load_memory_config_from_dict(config_dict: dict) -> None:
    ...
    ``manager_class`` / ``backend_config``) are read directly. QiLinMem-private
    ...
    for key in list(config_dict.keys()):
        if key in _SHARED_FIELDS:
            continue
        if key in _LEGACY_QILINMEM_FIELDS:
            ...
            if key == "model_name":
                ...
                # "memory.json" was ignored for per-user). QiLinMem now treats it as a
                ...
                logger.warning(
                    "Legacy memory.storage_path=%r looks like a file path; QiLinMem now "
            ...
            logger.warning(
                "Unknown memory config key %r at top level (not a shared field %s nor a known legacy QiLinMem field); ignored.",
                key,

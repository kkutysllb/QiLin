...
with DeepSeekHarness(qilin_home="/absolute/path/to/test-qilin-home") as harness:
...
- Set `qilin_bin` to the absolute built `apps/cli/lib/bin.js` path to exercise the checkout's CLI directly. Supply an explicit `qilin_home`, plus `profile` and ordered `patches` as needed.

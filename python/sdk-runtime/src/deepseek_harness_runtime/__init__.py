...
- **node (dev-only)**: the full deploy closure under ``runtime/node/``
  ...
  runtime/node/node_modules/@qilin/cli/lib/bin.js`` on a
...
def resolve_bundled_launch_args(mode: str | None = None) -> tuple[str, ...]:
    ...
    raise ValueError(
        f"unsupported QiLin runtime mode {selected!r}: expected 'exe' or 'node' "
...
def _current_platform_tag() -> str:
    ...
    if (
        ...
        raise FileNotFoundError(
            "no bundled QiLin SDK runtime exists for this platform "

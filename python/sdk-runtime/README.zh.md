...
DeepSeek Harness Python SDK 的平台运行时 wheel。它把普通 `qilin` CLI 及其封闭的 Node 依赖树打包成原生可执行程序，因此使用 SDK 不需要系统 Node.js。本包只发布 wheel。

## 安装命令与产物

Wheel 会安装 `qilin` 控制台命令和 `deepseek_harness_runtime` Python 模块。`qilin` 将参数转发给内置可执行程序，并要求非空 `QILIN_HOME`；它不会回退到 `~/.qilin`。
...
仓库构建还会物化仅限开发的 `runtime/node/` 载体。它在系统 Node 22.19 或更高版本上运行 `node runtime/node/node_modules/@qilin/cli/lib/bin.js`。系统不会自动选择它，而且 wheel 与 sdist 均不包含它。

两种载体执行相同的 `qilin` 语法与随附 profile，包括独立的 `sdk-minimal` 配置树，以及包含前端产物的完整 `web` profile。私有 `qilin-python-runtime-closure` manifest 定义打包依赖闭包；不存在 Python 专用 Node 应用或检入的默认 `cordis.yml`。

## Python 模块 API

- `bundled_package_dir() -> Path` 返回已安装模块数据根目录，并校验发布元数据。
- `bundled_runtime_path() -> Path` 返回当前平台可执行程序，并校验必需伴随文件。
- `resolve_bundled_launch_args(mode=None) -> tuple[str, ...]` 默认返回可执行程序 argv。显式 `mode="node"` 或 `QILIN_RUNTIME_MODE=node` 会选择仅限仓库使用的 Node 载体。
- `main()` 实现已安装的 `qilin` 控制台命令，并拒绝缺失或空白的 `QILIN_HOME`。在 Windows 上，它让打包进程继承标准流，等待其结束并转发退出状态；在 POSIX 上，它替换 Python 进程。

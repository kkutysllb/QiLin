...
class HarnessConfig:
    """Configuration for launching the local QiLin SDK runtime."""
...
class HarnessClient:
    """Synchronous JSON-RPC client for the QiLin SDK runtime over stdio."""
    ...
    def close(self) -> None:
        ...
        if proc.poll() is None:
            try:
                ...
                proc.wait()
        self._proc = None
        self._fail_waiters(self._runtime_closed_error("QiLin runtime closed"))
            ...
            while True:
                ...
                if deadline is not None:
                    ...
                    if remaining <= 0:
                        ...
                        raise TimeoutError(
                            f"{method} timed out waiting for QiLin runtime{suffix}"
                        )
    ...
    def _write_message(self, message: JsonObject) -> None:
        ...
        if proc is None or proc.stdin is None:
            raise TransportClosedError("QiLin runtime is not running")
        try:
            ...
            raise self._runtime_closed_error("Failed to write to QiLin runtime") from exc
    ...
    def _reader_loop(self) -> None:
        ...
        try:
            ...
            self._fail_waiters(self._runtime_closed_error("QiLin runtime stdout closed"))
        ...
        if self.config.qilin_bin is None:
            try:
                ...
                raise FileNotFoundError(
                    "Unable to locate the bundled QiLin qilin runtime. "

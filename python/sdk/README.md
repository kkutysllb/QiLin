...
Python subprocess SDK for driving QiLin over newline-delimited JSON-RPC on stdio. Install `deepseek-harness-sdk`; it installs the exact same-version `deepseek-harness-runtime-bin` wheel for the current platform.
...
Every launch requires an explicit Harness home. Pass `qilin_home` or provide a non-empty `DSH_HOME` in the child environment. The SDK deliberately never discovers `~/.qilin`.

```py
from deepseek_harness import DeepSeekHarness

with DeepSeekHarness(
    qilin_home="/absolute/path/to/isolated-qilin-home",
...
```sh
export QILIN_HOME=/absolute/path/to/isolated-qilin-home

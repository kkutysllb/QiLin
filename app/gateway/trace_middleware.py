...
from qilin.config.app_config import is_trace_correlation_enabled
...
class TraceMiddleware:
    ...
    ``qilin.config.reload_boundary.STARTUP_ONLY_FIELDS`` because
    ...
    ``X-Trace-Id`` header and Langfuse ``qilin_trace_id`` immediately while
...
def resolve_trace_enabled(config: Any) -> bool:
    ...
    :func:`qilin.config.app_config.is_trace_correlation_enabled`, kept so
    existing gateway callers and tests do not have to switch imports. Both
    the Gateway middleware and the embedded ``QiLinClient`` resolve the

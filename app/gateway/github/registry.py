...
(:mod:`qilin.persistence.agents`), so both the ``file`` backend (per-user
...
on the store's :meth:`~qilin.persistence.agents.base.AgentStore.signature`
...
from qilin.config.agents_config import AgentConfig, GitHubTriggerConfig

...
x-client-bundle-id: com.deepseek.qilin
x-client-locale: zh-CN
x-client-arch: x64
x-client-update-channel: nightly
x-client-bundled-qilin-version: 0.1.3-rc.2
```
...
| `x-client-version` | Full Desktop SemVer, retaining prerelease identifiers; initially equal to bundled qilin |
| `x-client-bundle-id` | Application identity, for example `com.deepseek.qilin`; distinguishes Harness from Chat |
...
| `x-client-bundled-qilin-version` | Full bundled qilin version from release metadata |
...
**Relying on remote business interception.** Local qilin requests need not reach the remote gateway; an independent guest query is necessary.
...
| Equal prerelease Desktop/qilin versions, fixed Nightly | Full SemVer matching, no downgrade or channel-switch requirement |

...
**Share one undifferentiated Office skill.** Format-specific discovery avoids loading spreaqilineet formula guidance for a Word edit or presentation instructions for a cell update. The deterministic checker remains shared because all three formats use the same package relationship rules.

...
The manageable plugin rows are one function (`readProfilePluginRows`): `qilin plugin list` prints them, and the manager behind the Settings surfaces reads the same profile manifest in its `listBundles` answer, so the two cannot disagree about layer order, installed version, source, or removability. `qilin plugin doctor` answers the same compatibility questions one package at a time without executing anything — it reports a package that builds the DSH-era home while never reading `DSH_HOME`/`QILIN_HOME`, installs an engine package (an upstream DSH name, or a name this installation already provides), imports an engine name it never declares as a peer, or injects client module names the compatibility layer does not map. The report is advisory and exits nonzero only when a finding blocks activation.

Profile mutations are owned by `@qilin/plugin-manager` (`packages/boot/plugin-manager`), which took the channel over from the retired `@qilin/host-plugin-manager` ([alignment note](2026-09-17-upstream-dsh-0.1.6-alpha.2-alignment.md)). Its `pluginManager` Remote runs pnpm in the launch profile through the shared package operations, applies bundle activation, bounds retained output, and reports restart-required because the running launch has a frozen bundle composition.

Each row's permissions follow its resolution channel. Shipped layers are never removable, and the profile owns none of them: `PROFILE_OWNED_BUNDLES` is empty now that the built-in plugin channel is retired, so every shipped layer moves with the running installation and is neither upgradable nor removable. Remote method names stay clear of the Client namespace service's own surface, which reserves its fields and members such as `install` and `remove`; the Manager exposes `listPlugins`, `listBundles`, `inspect`, `checkUpdates`, `catalog`, `installBundle`, `setBundleEnabled`, `setPluginEnabled`, `removeBundle`, and `cancelInstall`, and `isRemoteMethodNameAvailable` is the shared predicate that pins the rule.

The Settings surface carries no user-plugins tab: the sidebar Plugins page (`@qilin/client-ui-plugin-manager`) manages the profile's bundles through the manager Remote, and `ui-settings-plugin-inventory` keeps the read-only list of the bundles a shipped composition leaves out. Every string is localized, versions and sources come from the same Remote, and registry update checks plus GitHub `topic:dsh-plugin` discovery run through `checkUpdates` and `catalog` on it.

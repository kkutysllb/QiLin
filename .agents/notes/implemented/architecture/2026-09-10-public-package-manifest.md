...
Plugin authors need npm identity, runtime requirements, and QILIN declarations from one public import. Internal image-packaging, Session catalog, and generated proxy metadata do not define extension points for community plugins. Exposing those fields together makes internal mechanisms appear available to external authors.

## Decision

[`QilinPackageManifest`](../../../../packages/util/package-manifest/src/types.ts) describes the package.json fields QILIN uses, with required `name` and `version`. Its optional `qilin` member uses `QilinManifest` for public composition and author metadata. The type is a selected npm field set, not a complete package.json schema. App-boot adapts it with `Partial` for local profiles, which need no published identity.

Runtime requirements live at top-level `engines`: `qilin`, `node`, and `npm` are optional version strings, and other engine names are allowed. `qilin.manifestVersion` identifies declaration format `1`. Format and QILIN compatibility declarations are not enforced by current installers or loaders.
...
**Put QILIN compatibility under `qilin.engines`.** [VS Code](https://code.visualstudio.com/api/references/extension-manifest) places its host requirement in top-level `engines.vscode`. Top-level `engines.qilin` gives authors one location for runtime requirements; QILIN still owns enforcement of its custom key.

**Use peer dependencies as the sole host requirement.** Peer dependencies constrain installed npm packages, including the CLI package `@qilin/cli`. They do not identify the currently running QILIN process when plugins live in a separate profile project.

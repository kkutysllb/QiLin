...
Each Host process generates a random launch token, retained by the root application context across Connection hot reloads. `dsh-web-app` prints and opens the application entry URL with that token in the query once per process. `frontend-static` asks Connection to authorize index responses: only `GET <entry path>?token=...` exchanges the process token for a cookie, then redirects to the clean entry path (`/workspace`, published as `ctx.connection.entryPath`); the token is not accepted on API paths or in an Authorization header. An obsolete token paired with a valid cookie redirects to the clean entry path. Missing and invalid credentials receive one minimal 401 response. Static non-index assets and the configured public documents remain public.
...
The [local accounts decision](../feature/2026-09-12-qilin-local-accounts-and-landing.md) adds a second authority to the same gates: when the account session authority claims `ctx.connection.session`, gated index documents and every non-public `/api` request require an account session, and this launch token only cleans the printed URL into the entry redirect.

...
The cost is a large one-shot review surface. Re-landing QiLin's behaviour restored two surfaces upstream does not carry: the `settings.trigger` slot in `ui-settings`' slot contract, with the trigger row `ui-settings-general` renders into it, and `@qilin/plugin-manager`'s update check and GitHub package catalog, which the client page mounts on the existing install path instead of a second writer. The `@pluginId` input-trigger source, the creator toolset, and the `cordis_mount` trust paragraph are gone with upstream's creator-mode rewrite; `plugin_manager` replaces them.

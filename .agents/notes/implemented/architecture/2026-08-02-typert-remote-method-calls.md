...
In SRC mode, the decorator records the method name and invocation mode in a versioned descriptor on the Service prototype. The descriptor uses a stable string property name, so `remoteMethods()` can read markers produced by another installed copy of `@qilin/typert-protocol`; it writes nothing to the Service instance, constructor, or method function.

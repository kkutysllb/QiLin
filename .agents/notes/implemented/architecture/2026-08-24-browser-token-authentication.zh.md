...
每个 Host 进程生成随机启动令牌，并由应用根 context 跨 Connection 热重载保留。`dsh-web-app` 每个进程只打印并打开一次 query 中带该令牌的应用入口 URL。`frontend-static` 请求 Connection 授权 index 响应：只有 `GET <入口路径>?token=...` 会把进程令牌交换为 cookie，再重定向到干净的入口路径（`/workspace`，由 `ctx.connection.entryPath` 发布）；API 路径和 Authorization header 都不接受该令牌。过时令牌如果同时带有有效 cookie，会重定向到干净的入口路径。缺失与无效凭据得到同一份最小 401 响应。非 index 静态资产与配置的公开文档保持公开。
...
[本地账户决策](../feature/2026-09-12-qilin-local-accounts-and-landing.zh.md)为同一批门禁增加了第二种 authority：账户会话 authority 认领 `ctx.connection.session` 后，受门禁的 index 文档与每个非公开 `/api` 请求都要求账户会话，而该启动令牌只负责把打印出的 URL 清理为入口重定向。

...
[Runtime preparation](../../../../apps/desktop/scripts/prepare-qilin.ts) materializes the production graph once at build time and ships it through `extraResources/qilin`. The Electron shell stays in ASAR. A bundled upstream Node process runs the private Desktop Host from resources and loads enabled plugins from `$QILIN_HOME/profiles/desktop`.

...
Every supported Node application starts through the `qilin` CLI and one named profile. The shipped profiles are `web`, `headless`, `sdk`, `sdk-minimal`, and `acp`, selected with `qilin --profile <name>` or `qilin <name>`. `plugin` names the management command; a profile with that name requires `--profile plugin`.

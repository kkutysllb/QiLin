...
- Use [qilin-pre-push-checks](../qilin-pre-push-checks/SKILL.md) after the test design is sound to select outgoing validation.
...
- Wait for an explicit readiness event, hanqilinake, state transition, owned promise, or externally observable condition.
...
Before a push, use [qilin-pre-push-checks](../qilin-pre-push-checks/SKILL.md). Report exact commands and observed results; do not describe retries, skipped tests, or pending CI as passing.

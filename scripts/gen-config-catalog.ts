...
export function render(entries: CatalogEntry[]): string {
  ...
  const lines: string[] = [
    ...
    '',
    'Every `config:` block a `kylin.yml` entry can set: for each loadable harness package, the verbatim config declaration (JSDoc included) its `apply` function or service constructor receives, with every referenced type pasted alongside (package-local types) or linked (everything else). The paste is the plugin\'s full declared config type — a field the runtime schema deliberately excludes is a runtime-only seam (its own JSDoc says so) and is not settable from `kylin.yml`. This is the **deployment**-axis reference — the wiring a plugin author works against is the generated Cordis API region on each [subsystem page](subsystems/core.md), the model-facing tool schemas are the [tool catalog](tool-catalog.md), and [subsystems/](subsystems/core.md) documents the types these declarations reference.',
    '',
    ...
    '',
    'A `Requires:` line lists the service keys the plugin `inject`s: its `kylin.yml` tree must also load providers for those services. Scope is the harness tier (`packages/`); the vendored kylin plugins a config tree may also load (`hmr`, the console logger, …) are pinned upstream source ([vendoring policy](../vendor/README.md)) and not catalogued here.',
    '',
  ]
  ...
  lines.push(
    ...
    '',
    'These load from a `kylin.yml` entry with no `config:` block; they declare no configuration API.',
    '',
    ...
    '',
    'Imported as libraries by other packages; a `kylin.yml` cannot load them.',
    '',

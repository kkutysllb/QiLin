...
import { kylinModuleBody } from './kylin-walk.ts'

const root = resolve(import.meta.dirname, '..')
const FENCE = 'ts kylin-catalog'
...
export const CORDIS_CORE_API_PAGES: CordisCoreApiPage[] = [
  {
    ...
    sections: [
      { kind: 'class', file: 'vendor/kylin/src/context.ts', symbol: 'Context', prefix: 'ctx.' },
      { kind: 'context-merge', file: 'vendor/kylin/src/reflect.ts', heading: 'Service store and mixins' },
    ],
  },
  {
    out: 'docs/kylin-api/events.md',
    title: 'Events',
    ...
    sections: [
      { kind: 'context-merge', file: 'vendor/kylin/src/events.ts' },
      { kind: 'decl', file: 'vendor/kylin/src/events.ts', symbol: 'EventOptions' },
      { kind: 'decl', file: 'vendor/kylin/src/events.ts', symbol: 'DispatchMode' },
    ],
  },
  {
    out: 'docs/kylin-api/fiber.md',
    title: 'Fiber',
    ...
    sections: [
      { kind: 'context-merge', file: 'vendor/kylin/src/fiber.ts' },
      { kind: 'class', file: 'vendor/kylin/src/fiber.ts', symbol: 'Fiber', heading: 'The Fiber class' },
      { kind: 'decl', file: 'vendor/kylin/src/fiber.ts', symbol: 'Effect' },
      { kind: 'decl', file: 'vendor/kylin/src/fiber.ts', symbol: 'Disposable' },
      { kind: 'decl', file: 'vendor/kylin/src/fiber.ts', symbol: 'EffectMeta' },
      { kind: 'decl', file: 'vendor/kylin/src/fiber.ts', symbol: 'CordisError' },
      { kind: 'decl', file: 'vendor/kylin/src/fiber.ts', symbol: 'ValidationError' },
    ],
  },
  {
    out: 'docs/kylin-api/registry.md',
    title: 'Registry',
    intro: 'Plugin loading and dependency injection.',
    sections: [
      { kind: 'context-merge', file: 'vendor/kylin/src/registry.ts' },
      { kind: 'decl', file: 'vendor/kylin/src/registry.ts', symbol: 'Plugin' },
      { kind: 'decl', file: 'vendor/kylin/src/registry.ts', symbol: 'Inject' },
    ],
  },
  {
    out: 'docs/kylin-api/service.md',
    title: 'Service',
    intro: 'The base class for context services. A subclass loaded as a plugin registers itself as `ctx.<name>`.',
    sections: [
      { kind: 'class', file: 'vendor/kylin/src/service.ts', symbol: 'Service' },
    ],
  },
]
...
function memberDoc(ctx: RenderContext, where: string, name: string, group: Member[], rel: string): MemberDoc {
  ...
  const first = group[0]
  if (first === undefined) throw new Error(`kylin-core-api: empty member group for ${name}.`)
  ...
  if (docCarrier !== undefined) {
    checkParams(where, 'kylin-core-api', docCarrier.parameters, tags, sf,
...
function contextMergeMembers(ctx: RenderContext, rel: string): MemberDoc[] {
  ...
  const body = kylinModuleBody(sf)
  if (body === null) throw new Error(`kylin-core-api: ${rel} has no Context module merge.`)
...
function classMembers(ctx: RenderContext, rel: string, className: string): {
  ...
  if (cls === undefined) throw new Error(`kylin-core-api: class ${className} not found in ${rel}.`)
...
function declarationPaste(ctx: RenderContext, rel: string, symbol: string): { doc: string; code: string; source: string } {
  ...
  if (first === undefined) throw new Error(`kylin-core-api: declaration ${symbol} not found in ${rel}.`)
...
export function renderCordisCoreApiPage(
  ...
  reportViolations('gen-kylin-catalog', ctx.violations)
  return `${lines.join('\n').replace(/\n{3,}/g, '\n\n').trimEnd()}\n`
}

...
const KIND_TEMPLATES: Readonly<Record<string, string>> = {
  ...
  'persistence-change': '.agents/skills/qilin-doc/templates/persistence-change.md',
  'persistence-release': '.agents/skills/qilin-doc/templates/persistence-release.md',
  'persistence-format': '.agents/skills/qilin-doc/templates/persistence-format.md',
}
...
describe('qilin-doc skill consolidation', () => {
  ...
  it('maps retrospective release comparisons to their dedicated document kind', () => {
    const files = globSync('docs/persistence-changes/releases/qilin-*.md', { cwd: root })
    expect(files.length).toBeGreaterThan(0)
    for (const file of files) {
      const metadata = readFrontmatter(file)

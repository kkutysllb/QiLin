/**
 * Generate the model-facing client slot catalog consumed by `kylin_inspect
...
const CLIENT_NOTES: readonly string[] = [
  ...
  'This catalog is the COMPILE-TIME contract of the shipped web bundle, not a snapshot of one page: a key is registrable only where the owner that declares it is mounted. A failed registration surfaces in the browser-half load report — read it back with `kylin_inspect what:"temporary"`.',
]
...
export function renderClientCatalog(entries: readonly SlotEntry[]): string {
  const lines: string[] = [
    ...
    ' * `kylin_inspect what:"client"` serves it to the model: every SlotMap key a',

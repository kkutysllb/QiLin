...
describe('Cordis core API generation', () => {
  it('renders the five detailed pages from pinned vendor declarations', () => {
    ...
    expect(pages.get('docs/kylin-api/context.md')).toContain('### ctx.extend(meta?)')
    expect(pages.get('docs/kylin-api/events.md')).toContain('## DispatchMode')
    expect(pages.get('docs/kylin-api/fiber.md')).toContain('## EffectMeta')
    expect(pages.get('docs/kylin-api/registry.md')).toContain('## Plugin')
    expect(pages.get('docs/kylin-api/service.md')).toContain('### Service.resolveConfig')

    const fiber = pages.get('docs/kylin-api/fiber.md') ?? ''
  ...
  it('rejects a public core class without source JSDoc', () => {
    const root = mkdtempSync(join(tmpdir(), 'qilin-kylin-core-api-'))
    roots.push(root)
    mkdirSync(join(root, 'vendor/kylin/src'), { recursive: true })
    writeFileSync(join(root, 'vendor/kylin/src/service.ts'), 'export class Service {\n  run(): string { return "ok" }\n}\n')
    const page: CordisCoreApiPage = {
      out: 'docs/kylin-api/service.md',
      title: 'Service',
      intro: 'Service API.',
      sections: [{ kind: 'class', file: 'vendor/kylin/src/service.ts', symbol: 'Service' }],
    }

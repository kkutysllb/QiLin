/**
 * Acceptance-path coverage for the kylin-surface partition backstops
...
import { contextKeyMap, contextMergeFiles, eventNameList } from './kylin-walk.ts'
import { walkPartitionProblems } from './gen-kylin-catalog.ts'
import type { WalkPartitionInput, WalkPartitionMaps } from './gen-kylin-catalog.ts'
...
describe('kylin-walk scan reach', () => {
  it('finds Context keys and Events names in nested Events-only merge files', () => {
    const root = mkdtempSync(join(tmpdir(), 'kylin-walk-'))
    roots.push(root)
  ...
  it('yields every merge block of a multi-block file, double-quoted heads, and .tsx sources', () => {
    ...
    mkdirSync(dir, { recursive: true })
    // The Typert analyzer reads every kylin module block in a file; the

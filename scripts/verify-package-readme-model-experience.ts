...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-theme-brand': { kind: 'none', reason: 'The theme sets browser CSS custom properties only; it contributes no prompt text or session event.' },
  ...
  'packages/code-runtime/code-runtime': { kind: 'indirect', reason: 'The service interface delegates model rendering to PTC mode in qilin-tools.' },
  ...
  'packages/code-runtime/code-runtime-worker-thread': { kind: 'indirect', reason: 'The worker backend delegates model rendering to PTC mode in qilin-tools.' },
  'packages/experimental/code-runtime-python': { kind: 'indirect', reason: 'Explicit source-checkout compositions delegate model rendering to PTC mode in qilin-tools.' },
  ...
  'packages/client/ui-sidebar-tasks': { kind: 'none', reason: 'Browser-only viewer of Session-side work; registers no tool, prompt section, or session event, and both snapshots arrive on the Session list the client already holds.' },
  'packages/client/ui-sidebar-plans': { kind: 'none', reason: 'Browser-only workspace plan list; registers no tool, prompt section, or session event, and a plan document is read only to title its row.' },
  ...
  'packages/e2b/fs-e2b': { kind: 'indirect', reason: 'The provider backend delegates model rendering to qilin-tool-fs.' },
  ...
  'packages/identity/accounts-local': { kind: 'none', reason: 'The account surface gates the browser document and the /api transport; it registers nothing model-facing.' },

...
const MERGE_HEAD = /declare module ['"](?:@qilin\/kylin|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@qilin/kylin'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@qilin/kylin'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@qilin/kylin' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@qilin/kylin'` Context merge
...
/**
 * Every event name a `declare module '@qilin/kylin'` Events merge declares in one

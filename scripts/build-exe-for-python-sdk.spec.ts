...
describe('Python runtime executable builder CLI', () => {
  it('keeps the single-file dispatcher on the Python packaging surface', () => {
    ...
    expect(cliManifest.dependencies).not.toHaveProperty('@qilin/subprocess-local')
    expect(cliManifest.devDependencies).toHaveProperty('@qilin/subprocess-local')
    expect(runtimeManifest.dependencies).toHaveProperty('@qilin/subprocess-local')
    expect(bootstrap).toContain("import('@qilin/cli/lib/bin.js')")
    expect(bootstrap).toContain('await runCli()')
    expect(bootstrap).toContain("import('@qilin/subprocess-local/runner')")
  ...
  it('runs pnpm through its JavaScript entrypoint without a command shell', () => {
    ...
    const deploy = result.stdout.split('\n').find(line => line.includes(' --filter qilin-python-runtime-closure deploy'))

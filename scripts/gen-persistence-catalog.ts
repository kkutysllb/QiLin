...
const SESSION_PACKAGE = '@qilin/session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@qilin/session/types'
 ...
 * top-level declaration (in `@qilin/session`) and any declaration
 * merge inside a `declare module '@qilin/session/types'` block. Both forms
...
export function render(events: AnnotatedLogEventEntry[], envelopeTypes: EventEnvelopeTypeEntry[]): string {
  const lines: string[] = [
    ...
    '',
    'Every event type that can appear in a session\'s durable event log: the complete persisted `SessionEvent` envelope and each member of the merge-extensible `SessionEventMap` — the owning vocabulary in `@qilin/session` plus every plugin declaration merge into `@qilin/session/types` in this repo — with source JSDoc, full payload declaration, surface badge, and declaration site. It complements [session.md](subsystems/session.md) (surface ordering and the `deriveMessages()` projection), [persistence.md](subsystems/persistence.md) (how the log is made durable), and the generated region of [session.md](subsystems/session.md#kylin-surface) (the live bus wiring — a log event is NOT a cordis event; it reaches listeners via the single `session/event` emit).',
    '',
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @qilin/session/known-event-types',
    ' */',
    '',
    '/**',

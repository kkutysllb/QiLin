...
function lockfile() {
  const lock = {
    ...
    packages: {
      ...
      'node_modules/@qilin/cli/node_modules/library': { version: '2.0.0', dependencies: { plugin: '*' } },
...
function fixture(): string {
  const root = mkdtempSync(join(tmpdir(), 'qilin-dependency-catalog-test-'))
  roots.push(root)
...
describe('published npm dependency catalog', () => {
  ...
  it('does not silently produce an incomplete catalog from invalid or unrelated resolutions', () => {
    ...
    input.packages[''].dependencies['@qilin/cli'] = 'next'
    expect(() => collectDependencies(input)).toThrow('must request only @qilin/cli@latest')

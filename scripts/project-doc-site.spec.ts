...
describe('docsPages locale routes', () => {
  it('redirects both locale roots to their locale-relative quick-start page', () => {
    ...
    for (const page of homes) {
      ...
      expect(projected).not.toContain('# QiLin')
    }
  })
  ...
  it('places the shared todo fragment alias on the translated todo section', () => {
    ...
    expect(catalog.match(/<a id="deepseek-aiqilin-tool-todo"><\/a>/g)).toHaveLength(1)
  ...
  it('publishes the Cordis core API under matching locale structures', () => {
    ...
    for (const file of files) {
      const root = docsPages.find(page => page.route === `reference/kylin-api/${file}`)
      const english = docsPages.find(page => page.route === `en/reference/kylin-api/${file}`)
      expect(root?.source).toBe(`docs/kylin-api/${file.replace(/\.md$/, '.zh.md')}`)
      expect(root?.contentLocale).toBe('zh-CN')
      expect(root?.section).toBe('Cordis API')
      expect(english?.source).toBe(`docs/kylin-api/${file}`)
  ...
  it('keeps Cordis inherited on the English fallback in both locales', () => {
    const pages = docsPages.filter(page => page.route.endsWith('reference/kylin-api/inherited.md'))
    expect(pages).toHaveLength(2)
    expect(pages.every(page => page.source === 'docs/kylin-api/inherited.md')).toBe(true)
  ...
  it('projects reviewed generated counterparts into root locale routes', () => {
    ...
    const routes = [
      ...
      'reference/kylin-api/context.md',
      'reference/kylin-api/events.md',
      'reference/kylin-api/fiber.md',
      'reference/kylin-api/registry.md',
      'reference/kylin-api/service.md',
    ]
...
describe('projectedPageContent', () => {
  ...
  it('drops the repository badge every page links from its footer', () => {
    const badge = '[![](https://img.shields.io/badge/powered_by-qilin-4D6BFE?style=flat-square)](https://github.com/deepseek-ai/deepseek-harness)'
    expect(projectedPageContent(`# Guide\n\nBody.\n\n${badge}\n`, page('zh-guide')))
      .toBe('# Guide\n\nBody.\n')
  })
...
describe('raw Markdown projection of the published manifest', () => {
  ...
  it('emits home pages with their bodies instead of the frontmatter stub', () => {
    for (const route of ['index.md', 'en/index.md']) {
      ...
      expect(home, route).toContain('# QiLin')
    }
  })
...
describe('llmsTxt', () => {
  const site = { base: '/x/', title: 'QiLin', description: '插件化 SDK' }

...
describe('CI workflow', () => {
  ...
  it('keeps split native Windows PR jobs with failover, plus a master-only standby', () => {
    ...
    for (const [name, selector, variable, pool, hosted] of [
      ['linux gates', selectors.linux, 'QILIN_CI_FAILOVER_LINUX', ['self-hosted', 'linux', 'x64', 'vm-backup'], 'qilin-ubuntu-24-04-16core'],
      ['linux aggregate', selectors.linuxAggregate, 'QILIN_CI_FAILOVER_LINUX', ['self-hosted', 'linux', 'x64', 'vm-backup'], 'ubuntu-latest'],
      ['windows lanes', selectors.windows, 'QILIN_CI_FAILOVER_WINDOWS', ['self-hosted', 'qilin-win-ci', 'windows'], 'qilin-windows-2025-16core'],
    ] as const) {
  ...
  it('gates standalone keyless blacksmith jobs and benchmark tiers on the failover variables', () => {
    ...
    expect(expectedFilenames['runs-on']).toContain('QILIN_CI_FAILOVER_LINUX')
    ...
    expect(sandbox['runs-on']).toContain('QILIN_CI_FAILOVER_LINUX')
    ...
    for (const name of ['larger-runner-benchmark', 'consolidated-runner-benchmark'] as const) {
      ...
      expect(benchmark['runs-on']).toContain('QILIN_CI_FAILOVER_LINUX')
      expect(benchmark['runs-on']).toContain('QILIN_CI_FAILOVER_WINDOWS')
...
describe('Runtime and LLM e2e Blacksmith routing', () => {
  it('routes DeepSeek e2e only through the Linux Blacksmith switch', () => {
    ...
    for (const mode of ['', 'selfhosted', 'unexpected', 'blacksmith']) {
      expect(evaluateRunsOn(job['runs-on'], { vars: { QILIN_CI_FAILOVER_LINUX: mode, QILIN_CI_FAILOVER_WINDOWS: 'blacksmith' } }))
  ...
  it('keeps native release and dispatch builders hosted while routing x64 CI by platform', () => {
    ...
    for (const [target, runner, variable, blacksmith] of [
      ['node24-linux-x64', 'ubuntu-latest', 'QILIN_CI_FAILOVER_LINUX', 'blacksmith-16vcpu-ubuntu-2404'],
      ['node24-win-x64', 'windows-2025', 'QILIN_CI_FAILOVER_WINDOWS', 'blacksmith-16vcpu-windows-2025'],
      ['node24-linux-arm64', 'ubuntu-24.04-arm', 'QILIN_CI_FAILOVER_LINUX', 'ubuntu-24.04-arm'],
      ['node24-macos-arm64', 'macos-latest', 'QILIN_CI_FAILOVER_LINUX', 'macos-latest'],
      ['node24-macos-x64', 'macos-15-intel', 'QILIN_CI_FAILOVER_LINUX', 'macos-15-intel'],
    ] as const) {
      for (const ci of [false, true]) {
        for (const release of [false, true]) {
          for (const mode of ['', 'selfhosted', 'unexpected', 'blacksmith']) {
            const vars = { QILIN_CI_FAILOVER_LINUX: 'blacksmith', QILIN_CI_FAILOVER_WINDOWS: 'blacksmith', [variable]: mode }
  ...
  it.each(['plan', 'sdk-wheel'])('routes runtime %s only for non-release CI', (name) => {
    ...
    for (const ci of [false, true]) {
      for (const release of [false, true]) {
        for (const mode of ['', 'selfhosted', 'unexpected', 'blacksmith']) {
          expect(evaluateRunsOn(job['runs-on'], { inputs: { ci, release }, vars: { QILIN_CI_FAILOVER_LINUX: mode } }))
...
describe('Documentation site publication', () => {
  it('keeps Pages deployment dispatch-only from a v* tag', () => {
    ...
    // RELEASE_PUBLISH makes release:verify reject every ref that is not a v*

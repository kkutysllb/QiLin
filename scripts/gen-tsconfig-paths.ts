 ...
 * `@qilin/*` listed 49 candidate globs and `@qilin/*\/invariant`
...
interface PackageAlias {
  /** Bare specifier, e.g. `@qilin/session`. */
 ...
 * @returns Every directory whose manifest names a `@qilin/` package and that carries `src`.
 */
 ...
 * `@qilin/<directory>`. Packages named after something other than
 ...
 *
 * @returns Declared names of every `@qilin/` package carrying a `src` directory.
 */
...
export function mappedSpecifiers(text: string): Set<string> {
  ...
  for (const match of text.matchAll(/^\s*"(@qilin\/[^"/]+)":/gm)) {
    const key = match[1]

...
export function fixClientPackageManifests(root: string, facts: ClientPackageFacts): string[] {
  ...
  for (const declaration of facts.declarations.filter(entry => entry.dynamic)) {
    ...
    const qilin = isRecord(target.manifest.qilin) ? target.manifest.qilin : undefined
    const client = isRecord(qilin?.client) ? qilin.client : undefined
...
function readDeclaration(
  ...
  const qilin = isRecord(manifest.qilin) ? manifest.qilin : undefined
  const rawClient = qilin?.client

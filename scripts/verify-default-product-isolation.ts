...
export function verifyDefaultProductIsolation(root: string): ProductIsolationResult {
  ...
  if (directories.get(resolve(root, 'apps/cli'))?.manifest.name !== '@qilin/cli') {
  ...
  if (existsSync(profilePath)) {
    ...
    for (const name of selection.packages) {
      ...
      if (packages.get(name)?.manifest.qilin?.bundle?.patch === undefined) {
        failures.push(`${PROFILE_SOURCE}: default bundle ${name} must declare qilin.bundle.patch`)
      }
    }
  ...
  for (const pkg of queue) {
    ...
    if (manifest.qilin?.bundle?.patch !== undefined) scanConfig(resolve(pkg.directory, manifest.qilin.bundle.patch))
    for (const tree of manifest.qilin?.configTrees ?? []) {

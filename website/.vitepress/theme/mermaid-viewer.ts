...
function button(doc: Document, label: string, icon: keyof typeof icons): HTMLButtonElement {
  ...
  svg.classList.add('qilin-diagram-icon')
...
function openDiagram(
  ...
  dialog.className = 'qilin-diagram-viewer'
  dialog.setAttribute('aria-labelledby', 'qilin-diagram-title')
  const toolbar = doc.createElement('div')
  toolbar.className = 'qilin-diagram-toolbar'
  const title = doc.createElement('span')
  title.className = 'qilin-diagram-title'
  title.id = 'qilin-diagram-title'
  ...
  scaleLabel.className = 'qilin-diagram-scale'
  const fit = button(doc, copy.fit, 'fit')
  fit.className = 'qilin-diagram-fit'
  const close = button(doc, copy.close, 'close')
  close.className = 'qilin-diagram-close'
  close.autofocus = true
  toolbar.append(zoomOut, scaleLabel, zoomIn, fit)
  const helpToggle = button(doc, copy.helpLabel, 'help')
  helpToggle.className = 'qilin-diagram-help-toggle'
  helpToggle.setAttribute('aria-expanded', 'false')
  helpToggle.setAttribute('aria-controls', 'qilin-diagram-help-text')
  const help = doc.createElement('p')
  help.className = 'qilin-diagram-help'
  help.id = 'qilin-diagram-help-text'
  help.hidden = true
  help.textContent = copy.help
  const viewport = doc.createElement('div')
  viewport.className = 'qilin-diagram-viewport'
  const paper = doc.createElement('div')
  paper.className = 'qilin-diagram-paper'
...
export function installMermaidViewer(doc: Document, language: () => string): MermaidViewer {
  ...
  const scan = (): void => {
    ...
    for (const [container, entry] of entries) {
      if (!containers.has(container) || container.querySelector('svg:not(.qilin-diagram-icon)') !== entry.svg || !entry.button.isConnected) {
    ...
    for (const container of containers) {
      ...
      const svg = container.querySelector<SVGSVGElement>('svg:not(.qilin-diagram-icon)')
      if (!svg || !dimensions(svg)) continue
      const trigger = button(doc, copy.open, 'open')
      trigger.className = 'qilin-diagram-open'

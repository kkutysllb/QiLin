...
Bundle names resolve from the qilin installation first, then from the profile directory. In-box bundles (`@qilin/base`, `@qilin/web-app`, `@qilin/headless`, `@qilin/sdk-app`, `@qilin/sdk-minimal`, `@qilin/acp-app`) therefore always come from the same installation as the running `qilin`; out-of-tree bundles come from the profile's pnpm-managed `node_modules`. A bare plugin `name` in any patch row resolves through the profile directory's Node parent walk, which reaches the maintained installation fallback `$QILIN_HOME/profiles/node_modules`. Plain Node installations place one healed symlink there per dependency-closure package. A pkg executable instead places a real ESM proxy that mirrors explicit exports and re-exports the virtual package URL, because operating-system symlinks cannot enter pkg's `/snapshot` filesystem. Every launch also links packages carried only by selected external bundles through a qilin-owned directory into the current profile's `node_modules`; existing pnpm entries win, and each profile owns its links independently.
...
qilin plugin --profile <name> add @qilin/subagent-codex @qilin/subagent-claude-code
...
Install external plugin bundles through `qilin plugin --profile <name> add <package-or-git-spec>`. The installed package owns its dependencies and contributes its declared `cordis.patch.yml` layer. The CLI also ships `@qilin/mcp-client` as a dependency for patch layers, but no MCP server is enabled by default because each server command is trusted executable code outside the agent sandbox.

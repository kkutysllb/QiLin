...
`qilin.profile.bundles` 中列出的组合包先从 qilin 安装目录解析（`@qilin/cli-base`、`@qilin/cli-web-app`、`@qilin/cli-headless`、`@qilin/cli-sdk-app`、`@qilin/cli-sdk-minimal`、`@qilin/cli-acp-app`），再从 profile 自身的 `node_modules` 解析；pnpm 会将树外插件安装到该目录。

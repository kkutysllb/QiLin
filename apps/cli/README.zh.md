...
`dsh.profile.bundles` 中列出的组合包先从 dsh 安装目录解析（`@qilin/base`、`@qilin/web-app`、`@qilin/headless`、`@qilin/sdk-app`、`@qilin/sdk-minimal`、`@qilin/acp-app`），再从 profile 自身的 `node_modules` 解析；pnpm 会将树外插件安装到该目录。

...
Bundles named in `qilin.profile.bundles` resolve from the qilin installation first (`@qilin/cli-base`, `@qilin/cli-web-app`, `@qilin/cli-headless`, `@qilin/cli-sdk-app`, `@qilin/cli-sdk-minimal`, `@qilin/cli-acp-app`), then from the profile's own `node_modules`, where pnpm installs out-of-tree plugins.

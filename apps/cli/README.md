...
Bundles named in `dsh.profile.bundles` resolve from the dsh installation first (`@qilin/base`, `@qilin/web-app`, `@qilin/headless`, `@qilin/sdk-app`, `@qilin/sdk-minimal`, `@qilin/acp-app`), then from the profile's own `node_modules`, where pnpm installs out-of-tree plugins.

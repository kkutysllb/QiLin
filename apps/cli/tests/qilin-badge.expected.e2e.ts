...
const binScript = fileURLToPath(new URL('./fixtures/qilin-badge/snapshot.ts', import.meta.url))
const configPath = fileURLToPath(new URL('./fixtures/qilin-badge/cordis.yml', import.meta.url))
const defaultConfigPath = fileURLToPath(new URL('./fixtures/qilin-badge/default.cordis.yml', import.meta.url))
...
describe('qilin badge assembled snapshot', () => {
  it('advertises and loads the opt-in bundled skill through the shipped app', async () => {
    const disabled = await runLoaderSmoke({
      ...
      tempDirPrefix: 'headless-snapshot-qilin-badge-disabled-',
      binScript,
    ...
    const enabled = await runLoaderSmoke({
      ...
      tempDirPrefix: 'headless-snapshot-qilin-badge-',
      binScript,
      libBinScript: binScript,
      configPath,
      tsconfigPath,
    ...
    expect(enabledSnapshot).toMatchInlineSnapshot(`
      ...
      <available_skills>
      - \`qilin-badge\`: Add the official “powered by qilin” badge to documents, pull requests, merge requests, and other content produced with DeepSeek Harness. Use whenever creating a pull request or merge request. Also use when the user asks for a qilin badge, powered-by-qilin attribution, or a reusable qilin badge asset or snippet.
      </available_skills>
      ...
      - Shields.io image URL: \`https://img.shields.io/badge/powered_by-qilin-4D6BFE?style=flat-square&logo=deepseek&logoColor=white\`
      ...
      \`\`\`markdown
      ![](https://img.shields.io/badge/powered_by-qilin-4D6BFE?style=flat-square&logo=deepseek&logoColor=white)
      \`\`\`

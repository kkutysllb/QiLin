...
it.each([false, true])('settles startup after parent IPC disconnect (boot failure: %s)', async (fail) => {
  ...
  for (const [name, source] of [
    ['qilin-home-paths', `export const resolveQilinHome = () => ${JSON.stringify(root)}`],
    ['qilin-tools', 'export const defineTool = value => value'],
  ] as const) {
  ...
  writeFileSync(join(modules, 'qilin-app-boot', 'package.json'), '{"type":"module","exports":"./index.js"}')
  writeFileSync(join(modules, 'qilin-app-boot', 'index.js'), 'export const loadProfileDirectory = () => ({}); export const loadLayeredEnv = () => ({})')
  writeFileSync(join(modules, 'qilin', 'package.json'), '{"type":"module","exports":{"./profile-boot":"./profile-boot.js"}}')
  writeFileSync(join(modules, 'qilin', 'profile-boot.js'), `

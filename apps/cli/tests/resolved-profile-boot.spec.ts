...
describe('runProfile with an application-owned profile', () => {
  it.each(
    ...
    const home = mkdtempSync(join(tmpdir(), 'qilin-profile-startup-failure-'))
    homes.push(home)
  ...
  it.each([
    ...
    const home = mkdtempSync(join(tmpdir(), 'qilin-resolved-profile-'))
    homes.push(home)
    mkdirSync(join(home, 'runtime'))
    ...
    vi.stubEnv('QILIN_HOME', home)
    vi.stubEnv('QILIN_TELEMETRY_DISABLED', '1')
    vi.spyOn(process, 'on').mockReturnValue(process)

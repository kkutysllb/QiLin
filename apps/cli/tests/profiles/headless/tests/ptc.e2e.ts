...
import NodeRuntime from '@qilin/ptc-runtime-node'
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('PTC mode: real model writes a program over real tools', () => {
  ...
  it('projects nested workspace instructions discovered by an fs sub-call', async () => {
    ...
    await writeFile(join(workdir, 'pkg/AGENTS.md'), `If asked for the PTC mode workspace hanqilinake, reply with exactly ${WORKSPACE_PROBE} and nothing else.\n`)
    ...
    handle.agent.followup(createUserMessage({
      content: [{
        type: 'text',
        text: 'Use one run_code program to call tools.read on pkg/deep/task.txt. After it finishes, answer: PTC mode workspace hanqilinake?',
      }], source: { kind: 'user' } }))

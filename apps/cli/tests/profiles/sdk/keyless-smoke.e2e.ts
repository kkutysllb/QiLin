...
describe('Python SDK qilin profile keyless smoke', () => {
  ...
  it.each([false, true])('exits after startup failure with stdin open (logs blocked: %s)', async (blocked) => {
    const root = await mkdtemp(join(tmpdir(), 'qilin-sdk-startup-exit-'))
    const home = join(root, '.qilin')
    ...
    const child = execa(launch.command, [
      ...
      cwd: repoRoot,
      env: { ...launch.env, QILIN_HOME: home, QILIN_TELEMETRY_DISABLED: '1', DEEPSEEK_API_KEY: 'keyless-no-call' },
      stdin: 'pipe',

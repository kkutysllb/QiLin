...
import { readProfileManifest } from '@qilin/app-boot'
...
it('configures MCP on a live profile, restores it on restart, and removes its tools', async (test) => {
  ...
  await writeFile(join(bundle, 'cordis.patch.yml'), JSON.stringify([{ insert: [{ id: 'demo',
    name: '@qilin/mcp-client', config: { serverName: 'demo', transport: 'streamable-http',
  ...
  const start = async () => {
    const child = spawn(process.execPath, [join(repo, 'apps/cli/lib/bin.js'), '--profile', 'web', '--patch', patch,
      ...
      env: { ...process.env, QILIN_HOME: join(root, 'home'), QILIN_AGENTS_HOME: join(root, 'agents'),
        QILIN_TELEMETRY_DISABLED: '1', DEEPSEEK_API_KEY: 'keyless-no-model-calls' },
    ...
    await expect.poll(() => {
      ...
      return output.includes('qilin web: http://')
  ...
  const saved = readProfileManifest('qilin', join(root, 'home/profiles/web'))
  expect(saved.qilin?.profile?.bundles).toContain('@test/creator-mcp')

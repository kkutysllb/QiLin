...
const builtArtifactsExist = existsSync(qilinBin) && existsSync(frontendIndex)
...
function createFixture(): Fixture {
  const root = mkdtempSync(join(tmpdir(), 'qilin-web-best-effort-'))
...
async function waitForStartup(
  ...
  const finish = (): void => {
    ...
    if (!stderrText.includes('qilin: warning: 6 entries did not activate')) return
  ...
  stdout.on('data', (chunk: string) => {
    stdoutText += chunk
    url ??= /qilin web: (http:\/\/[^\s]+)/u.exec(stdoutText)?.[1]
    finish()
  })
...
describe.skipIf(!builtArtifactsExist)('qilin Web profile best-effort startup', () => {
  it('serves the full Web app while unrelated entries fail to start', async () => {
    ...
    const child = execa(process.execPath, [
      ...
      env: {
        ...
        QILIN_AGENTS_HOME: join(fixture.root, '.agents'),
  ...
  it.each([
    ...
    try {
      ...
      expect(result.stdout).not.toContain('qilin web: http://')
  ...
  it.each([false, true])('fails the full Web profile when its required HTTP server cannot bind (logs blocked: %s)', async (logsBlocked) => {
    const root = mkdtempSync(join(tmpdir(), 'qilin-web-required-bind-'))
    const home = join(root, 'home')
    mkdirSync(home)
    ...
    try {
      ...
      expect(summary).not.toMatch(/qilin: warning:|\[cause\]|at boot \(|at runCli \(|Node\.js v/u)
      let report: string
      if (logsBlocked) {
        expect(result.stderr).toContain('qilin: warning: could not write startup diagnostics:')
      ...
      expect(report).toContain('qilinVersion:')

...
export async function withDefaultWeb(test: TestContext, inspect: (app: DefaultWeb) => Promise<void>): Promise<void> {
  const root = await mkdtemp(join(tmpdir(), 'qilin-web-default-isolation-'))
  ...
  try {
    ...
    try {
      await expect.poll(() => {
        ...
        return /qilin web: (http:\/\/[^\s]+)/u.exec(stdout)?.[1]
      }, { timeout: test.task.timeout }).toBeDefined()
      const url = /qilin web: (http:\/\/[^\s]+)/u.exec(stdout)![1]!

...
import type { StartupError } from '@qilin/app-boot'
...
/**
 * Print the startup summary and save a private, uniquely named report under QILIN_HOME/logs.
...
export async function reportStartupFailure(
  ...
  try {
    ...
    await write(`\nqilin: warning: could not write startup diagnostics: ${String(writeError)}\nFull diagnostics:\n${report}`)
    return
  }

...
export function homePatchPath(): string {
  return join(resolveQilinHome(), PROFILE_PATCH_FILENAME)
}
...
export interface ResolvedProfileRuntime {
  ...
  profile: Profile
  /** Absolute package.json path of the application's qilin installation. */
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  const dispose = (): Promise<void> => disposal ??= (async () => {
    ...
    if (failures.length > 1) throw new AggregateError(failures, 'qilin: profile cleanup failed')
  })()
  try {
    ...
    const profileContext: ProfileContext = {
      ...
      cwd: process.cwd(), home: resolveQilinHome(),
      overlays: composed.overlays, telemetryDisabledEnv: process.env.QILIN_TELEMETRY_DISABLED,
    }
    const ctx = await boot(NAME, rootConfig, readProfilePatches(NAME, profileContext, composed.profile), async (hostCtx) => {
      app.current = hostCtx
      // Before any config-tree entry mounts, so profile-management host plugins
      // receive the same immutable profile facts that composed this Loader tree.
    ...
    try { await dispose() } catch (cleanupError) {
      throw new AggregateError([error, cleanupError], 'qilin: profile startup and cleanup failed')
    }
    throw error
  }
}

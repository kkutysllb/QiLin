...
export class DesktopProjectManager {
  ...
  /** Read the qilin version supplied by this application's verified resources. */
  qilinVersion(): string {
  ...
  canRecoverProfile(): boolean {
    return this.descriptor !== undefined && existsSync(this.runtime.node) && existsSync(this.runtime.qilin)
  }
  ...
  private readRuntime(): DesktopRuntimeDescriptor {
    ...
    return readDesktopRuntime(this.runtime.qilin)
  }
  ...
  private prepareProfile(projectDir: string): void {
    ...
    else linkDesktopHostPackages(projectDir, this.runtime.qilin, runtime)
    validateDesktopPluginGraph(projectDir, this.runtime.qilin, runtime, profilePluginNames(projectDir), resolutionMode)
  }
    ...
    return this.withLock(async () => {
      ...
      if (!existsSync(this.pendingPackages) && previous?.runtimeId === desktopRuntimeId(target)
        ...
        && (this.runtime.profileResolution === 'runtime' || (previous.links.length === target.sharedPackages.length
          ...
          && realpathSync.native(link.target) === realpathSync.native(join(this.runtime.qilin, 'node_modules', link.name)))))) {
        return false
      }
    ...
    await this.withLock(async () => {
      ...
      if (mutation.type === 'plugins-disable-all') {
        ...
        writeJson(join(this.paths.profile, 'package.json'), {
          ...manifest,
          qilin: { ...manifest.qilin, profile: { ...manifest.qilin.profile, bundles: [...DESKTOP_PROFILE_BUNDLES] } },
        })
        this.prepareProfile(this.paths.profile)
        await hooks.afterChange()
        return
      ...
      try {
        ...
        if (packagesChanged) {
          ...
          else linkDesktopHostPackages(this.paths.profile, this.runtime.qilin, runtime)
        }
      }

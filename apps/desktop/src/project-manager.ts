...
export class DesktopProjectManager {
  ...
  constructor(
    ...
    readonly runtime: { readonly qilin: string },
  ) {}
  ...
  async disableAllPlugins(): Promise<string | undefined> {
    return this.withLock(() => sanitizeProfile('qilin', this.paths.profile, WEB_PROFILE.bundles))
  }
  ...
  async applyRelease(production = false): Promise<void> {
    await this.withLock(() => {
      const descriptor = readDesktopRuntime(this.runtime.qilin)
...
export function createRuntimeProjectMetadata(projectDir: string, release: DesktopRelease): void {
  ...
  const manifest = {
    ...
    qilin: { profile: { bundles: [...WEB_PROFILE.bundles] } },
  }

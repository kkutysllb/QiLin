...
function runtimeResources(): RuntimeResources {
  ...
  const qilin = (development ? process.env.QILIN_DESKTOP_QILIN_DIR : undefined)
    ?? (development ? join(app.getAppPath(), '.desktop-build', 'development', 'project') : join(app.getAppPath(), 'qilin'))
  return { node, nodeBin, pnpm, qilin }
}
...
async function main(): Promise<void> {
  const journalDirectory = process.env.QILIN_DESKTOP_UPDATE_JOURNAL_DIR
  ...
  const backend = new DesktopBackendController((onFailure) => {
    ...
    const host = new DesktopHostProcess(resources.node, resources.qilin, activeProject,
  ...
  protocol.handle(SCHEME, (request) => {
    ...
    if (url.hostname === 'app') {
      if (url.pathname === '/' || url.pathname === '/index.html' || url.pathname.startsWith('/assets/')
        ...
        return serveWebDocument(request, join(resources.qilin, 'node_modules', '@deepseek-ai', 'qilin-web-frontend', 'dist'))
      }
  ...
  ipcMain.handle(DESKTOP_IPC.bootFailed, (event, message: unknown) => {
    ...
    if (event.sender !== mainWindow?.webContents || event.senderFrame !== event.sender.mainFrame) {
      throw new Error('qilin desktop: rejected startup failure from a non-primary frame')
    }
    if (typeof message !== 'string') throw new Error('qilin desktop: startup failure must be text')
  ...
  session.defaultSession.webRequest.onBeforeSendHeaders({ urls: ['ws://127.0.0.1/*'] }, (details, callback) => {
    ...
    if (headers.origin !== 'qilin-app://app') { callback({ cancel: true }); return }
  ...
  const developmentPolicy = app.isPackaged ? undefined : process.env.QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG
  const policyInput: unknown = app.isPackaged
    ? ('qilinMandatoryUpdatePolicy' in manifest ? manifest.qilinMandatoryUpdatePolicy : undefined)
  ...
  if (policyConfig !== undefined) {
    ...
    const bundleId = app.isPackaged
      ? ('qilinDesktopAppId' in manifest ? manifest.qilinDesktopAppId : undefined)
      : process.env.QILIN_DESKTOP_APP_ID
    ...
    mandatoryPolicy = new DesktopMandatoryUpdatePolicy(policyConfig, {
      ...
      version: app.getVersion(), bundledDshVersion: app.isPackaged ? readDesktopRuntime(resources.qilin).release.version : app.getVersion(),
  ...
  if (window !== undefined && development && process.env.QILIN_DESKTOP_OPEN_DEVTOOLS !== '0') {

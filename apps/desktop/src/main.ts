...
function runtimeResources(): RuntimeResources {
  ...
  const node = development
    ? process.env.QILIN_DESKTOP_NODE_BINARY
  ...
  const qilin = (development ? process.env.QILIN_DESKTOP_QILIN_DIR : undefined)
    ?? (development ? join(process.resourcesPath, 'qilin') : join(app.getAppPath(), 'qilin'))
  return { node, pnpm, qilin, ...(development ? {} : { profileResolution: 'runtime' }) }
}
...
function createWindow(preload: string, show = false): BrowserWindow {
  ...
  window.webContents.on('will-navigate', (event, url) => {
    ...
    if (action.protocol !== 'qilin-recovery:' || !['restart', 'plugins', 'reset'].includes(action.hostname)) return
...
async function main(): Promise<void> {
  ...
  const backend = new DesktopBackendController((onFailure) => {
    ...
    const host = new DesktopHostProcess(resources.node, development ?? resources.qilin, activeProject,
  ...
  ipcMain.handle(DESKTOP_IPC.pluginsToggle, (event, name: unknown, enabled: unknown) => {
    if (typeof name !== 'string' || typeof enabled !== 'boolean') throw new Error('qilin desktop: invalid plugin activation request')
  ...
  if (quitting) return
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
  if (mainWindow !== undefined && development !== undefined && process.env.QILIN_DESKTOP_OPEN_DEVTOOLS !== '0') {

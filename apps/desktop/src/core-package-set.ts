/** Signed local npm package set that supplies the Desktop-owned qilin runtime and private Host. */
...
/** Private package installed beside qilin to boot the Desktop Host process. */
export const DESKTOP_HOST_PACKAGE = '@qilin/desktop-host'
...
/** Complete union of the first-party package closures rooted at qilin and its private Desktop Host. */
...
const RELEASE_PACKAGES = [QILIN_PACKAGE, DESKTOP_HOST_PACKAGE] as const
 ...
 * @param expectedReleaseVersion - Required qilin and Desktop Host version when validating one release.
...
/** Return the local direct dependency spec for the qilin package. */
export function desktopQilinPackageSpec(packageSet: DesktopCorePackageSet): string {
  const record = packageSet.packages.find(entry => entry.name === QILIN_PACKAGE)
  if (record === undefined) throw new Error(`desktop package set: missing ${QILIN_PACKAGE}`)
 ...
 * @param expectedReleaseVersion - Exact qilin and Desktop Host version bound to Electron.

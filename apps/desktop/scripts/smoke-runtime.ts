 ...
 * @param root - Materialized qilin resources.
...
export async function smokeDesktopRuntime(root: string, node: string, runtime: DesktopRuntimeDescriptor): Promise<void> {
  const home = mkdtempSync(join(tmpdir(), 'qilin-desktop-smoke-'))
  const profile = join(home, 'profiles', 'desktop')
  const host = new DesktopHostProcess(node, root, profile, undefined, { ...process.env, QILIN_HOME: home })
  try {
    createPluginProfile(profile)
    const pluginName = 'desktop-runtime-smoke-plugin'
    const plugin = join(profile, 'node_modules', pluginName)
    mkdirSync(plugin, { recursive: true })
    const cordis = runtime.sharedPackages.find(entry => entry.name === '@qilin/kylin')
    ...
    writeFileSync(join(plugin, 'package.json'), JSON.stringify({
      ...
      peerDependencies: { '@qilin/kylin': cordis.version }, qilin: { bundle: { patch: './bundle.yml' } },
    }))
...
`)
    ...
    manifest.qilin.profile.bundles.push(pluginName)
    ...
    if (ready.qilinVersion !== runtime.release.version) throw new Error('desktop runtime: Host reported another qilin release')
    const response = await host.fetch(new Request('qilin-app://app/'))

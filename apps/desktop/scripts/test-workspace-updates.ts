...
try {
  ...
  await writeFile(join(project, 'desktop-runtime.json'), JSON.stringify({
    ...
    sharedPackages: ['@qilin/cli', '@qilin/desktop-host']
  ...
  const child = spawn(electron, [fileURLToPath(new URL('../tests/fixtures/workspace-updates.mjs', import.meta.url)), '--lang=zh-CN',
    ...
    cwd: root, env: { ...environment, QILIN_HOME: join(root, 'home'), USERPROFILE: root, HOME: root,
      TEMP: root, TMP: root, TMPDIR: root, QILIN_WORKSPACE_UPDATE_ROOT: root, QILIN_WORKSPACE_UPDATE_TOKEN: randomUUID(),
      QILIN_DESKTOP_OPEN_DEVTOOLS: '0' },

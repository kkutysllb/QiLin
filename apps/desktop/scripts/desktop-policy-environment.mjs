...
export function resolveDesktopPolicyEnvironment(environment) {
  ...
  const name = deployment === 'test' ? 'QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN' : 'QILIN_DESKTOP_MANDATORY_UPDATE_PROD_ORIGIN'
  const selected = origin(environment[name], name)
  let settings = {}
  if (environment.QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG !== undefined) {
    try { settings = JSON.parse(environment.QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG) }
    catch { throw new Error('desktop package: QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG must be valid JSON') }
  }

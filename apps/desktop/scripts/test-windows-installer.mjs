...
const sign = process.argv.includes('--signed') ? createWindowsTokenSigner({
  certificateFile: signingEnvironment.QILIN_DESKTOP_WINDOWS_CER_FILE,
  signTool: signingEnvironment.QILIN_DESKTOP_WINDOWS_SIGNTOOL,
  tokenPin: signingEnvironment.QILIN_DESKTOP_WINDOWS_TOKEN_PIN,
  keyContainer: signingEnvironment.QILIN_DESKTOP_WINDOWS_KEY_CONTAINER,
  runDirectory: signingRun.directory,
}) : undefined
let succeeded = false
try {
  Object.assign(process.env, {
    QILIN_DESKTOP_APP_ID: `com.deepseek.harness.installertest.n${id}`,
    QILIN_DESKTOP_TARGET_PLATFORM: 'win32', QILIN_DESKTOP_TARGET_ARCH: 'x64',
    QILIN_DESKTOP_UNSIGNED: '1', CSC_IDENTITY_AUTO_DISCOVERY: 'false', ELECTRON_BUILDER_7Z_FILTER: 'BCJ',
    QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: signingEnvironment.QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN
      ?? 'https://harness-test.deepseek.com',
    ...signingRun ? { QILIN_DESKTOP_PACKAGING_RUN_DIR: signingRun.directory } : {},
  })

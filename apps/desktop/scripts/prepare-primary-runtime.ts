...
export async function preparePrimaryRuntime(options: { deferSmoke?: boolean } = {}): Promise<void> {
  ...
  const staging = mkdtempSync(join(tmpdir(), 'qilin-primary-'))
  try {
  ...
  await prepareOfficeSkillAssets(join(dirname(hostRequire.resolve('@qilin/skill-office/package.json')), 'assets'),

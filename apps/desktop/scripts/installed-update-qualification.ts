...
export async function createInstalledUpdateRun(
  ...
  const run: InstalledUpdateRun = {
    ...
    appId: `com.deepseek.qilin.qualification.q${id}`, productName: `QILIN Update Test ${id}`,
    environment: 'test', origin: 'https://download-test.deepseek.com', bucket: 'bj-toc-download-test-1320056602',
    feedKey: `qilin-desk/feeds/qualification/${id}/win-x64/nightly.yml`,
    binPrefix: `qilin-desk/bin/qualification/${id}/win-x64`,
  }
...
export async function readInstalledUpdateRun(path: string): Promise<InstalledUpdateRun> {
  ...
  if (row.schemaVersion !== 1 || typeof row.id !== 'string' || !/^[a-f0-9]{24}$/u.test(row.id)
    ...
    || row.appId !== `com.deepseek.qilin.qualification.q${row.id}` || row.productName !== `QILIN Update Test ${row.id}`
    || row.feedKey !== `qilin-desk/feeds/qualification/${row.id}/win-x64/nightly.yml`
    || row.binPrefix !== `qilin-desk/bin/qualification/${row.id}/win-x64`
...
export async function collectInstalledUpdateJournals(manifest: string, directory: string): Promise<string> {
  ...
  if (!resolve(directory).replaceAll('\\', '/').endsWith(`/qilin-update-qualification/${run.id}/journals`)) {

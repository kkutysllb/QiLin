...
const QILIN_OUTPUT_ROOT = BUILD_PATHS.qilin
const BUILD_ROOT = mkdtempSync(join(tmpdir(), 'qilin-desktop-runtime-'))
const STORE_ROOT = join(BUILD_ROOT, 'store')
const RUNTIME_ROOT = BUILD_PATHS.runtime
const PNPM_BUILD_STATE = BUILD_PATHS.qilinPnpm
...
function desktopRelease(): DesktopRelease {
  ...
  if (version !== qilinVersion) {
    throw new Error(`desktop runtime: Electron ${version} must bind the same version of @qilin/cli, found ${qilinVersion}`)
  }
    ...
    const child = spawn(NODE, [
      ...
      env: {
        ...Object.fromEntries(Object.entries(process.env).filter(([name]) => (
          name !== 'NODE_OPTIONS' && name !== 'NODE_PATH' && !/^QILIN_DESKTOP_/u.test(name) && !/^(?:npm|pnpm|corepack)_/iu.test(name)
        ))),
...
async function main(): Promise<void> {
  rmSync(QILIN_OUTPUT_ROOT, { recursive: true, force: true })
  ...
  try {
    ...
    mkdirSync(QILIN_OUTPUT_ROOT, { recursive: true })
    cpSync(modules, join(QILIN_OUTPUT_ROOT, 'node_modules'), {
      recursive: true, dereference: true,
      filter: source => desktopRuntimeFileExclusion(relative(modules, source), target, officeEngine) === undefined,
    })
    writeFileSync(join(QILIN_OUTPUT_ROOT, 'package.json'), `${JSON.stringify({
      name: '@qilin/desktop-runtime', private: true, version: release.version, type: 'module',
    ...
    for (const file of DESKTOP_HOST_RUNTIME_FILES) {
      if (!existsSync(join(QILIN_OUTPUT_ROOT, 'node_modules', DESKTOP_HOST_PACKAGE, file))) {
        throw new Error(`desktop runtime: missing private Host file ${file}`)
      }
    }
    if (!existsSync(join(QILIN_OUTPUT_ROOT, 'node_modules', '@deepseek-ai', `libreoffice-kit-${officeEngine}`, 'prebuilds.json'))) {
      throw new Error(`desktop runtime: missing required LibreOffice engine ${officeEngine}`)
    }
    if (process.platform === 'darwin') {
      await signMacOSRuntime(QILIN_OUTPUT_ROOT, resolveDesktopAppId(process.env), resolveMacOSSigningEnvironment(process.env))
    ...
    writeDesktopRuntime(QILIN_OUTPUT_ROOT, release, packageSet.packages.map(entry => entry.name), target)
    const descriptor = await verifyDesktopRuntime(QILIN_OUTPUT_ROOT, release.version, target)
    await new Promise<void>((accept, reject) => {
      execFile(NODE, ['--expose-internals', join(APP_ROOT, 'tests/fixtures/runtime-payload-smoke.mjs'), QILIN_OUTPUT_ROOT],
    ...
    await smokeDesktopRuntime(QILIN_OUTPUT_ROOT, NODE, descriptor)
    await verifyDesktopRuntime(QILIN_OUTPUT_ROOT, release.version, target)
  } catch (error) {
    rmSync(QILIN_OUTPUT_ROOT, { recursive: true, force: true })
    throw error
  } finally {

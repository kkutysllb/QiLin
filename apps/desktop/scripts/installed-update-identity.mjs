 ...
 * @param {{ version?: string, qilinDesktopAppId?: string }} metadata Installed package metadata.
...
export function configureInstalledUpdateIdentity(app, run, metadata, environment) {
  if (!/^[a-f0-9]{24}$/u.test(run.id) || run.versions.length !== 2
    || metadata.qilinDesktopAppId !== `com.deepseek.qilin.qualification.q${run.id}`
  ...
  const root = join(app.getPath('appData'), 'qilin-update-qualification', run.id)
  const paths = { root, userData: join(root, 'user-data'), harnessHome: join(root, 'qilin-home'), journals: join(root, 'journals') }
  ...
  environment.QILIN_HOME = paths.harnessHome
  environment.QILIN_DESKTOP_UPDATE_JOURNAL_DIR = paths.journals
  return paths
}

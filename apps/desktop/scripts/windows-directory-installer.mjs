...
const patched = Symbol.for('@qilin/desktop/directory-installer')
...
!macro extractUsing7za FILE
  !insertmacro qilinExtractPayload "\${FILE}"
!macroend
...
!insertmacro qilinStageApplication`)
  result = replaceOnce(result, '!insertmacro installApplicationFiles', 'Call qilinPromoteDirectories\nIfErrors 0 +4\n  SetErrorLevel 2\n  MessageBox MB_OK|MB_ICONEXCLAMATION "$(appCannotBeClosed)" /SD IDOK\n  Quit')
...
export function installWindowsDirectoryInstaller() {
  ...
  prototype.computeFinalScript = async function (source, ...args) {
    ...
    return `!define QILIN_SEVENZIP_PATH "${tool}"\n!define QILIN_SEVENZIP_LICENSE_DIR "${dirname(dirname(sourceTool))}"\n${await compute.call(this, adapted, ...args)}`
  }
}

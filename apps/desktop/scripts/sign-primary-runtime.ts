...
async function main(): Promise<void> {
  ...
  const certificateFile = process.env.QILIN_DESKTOP_WINDOWS_CER_FILE
  ...
  try {
    await signWindowsPrimaryRuntime(join(resolveDesktopTargetBuildPaths().runtime, 'primary-runtime'), {
      thumbprint,
      sign: createWindowsTokenSigner({ certificateFile, signTool: process.env.QILIN_DESKTOP_WINDOWS_SIGNTOOL,
        keyContainer: process.env.QILIN_DESKTOP_WINDOWS_KEY_CONTAINER, tokenPin: process.env.QILIN_DESKTOP_WINDOWS_TOKEN_PIN }),

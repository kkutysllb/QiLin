/** Build one release target with matching Electron and qilin architecture. */
...
export function desktopElectronBuilderEnvironment(environment: NodeJS.ProcessEnv, unsigned: boolean): NodeJS.ProcessEnv {
  const selected: NodeJS.ProcessEnv = { ...environment, QILIN_DESKTOP_UNSIGNED: unsigned ? '1' : '0' }
  // The bundled NSIS decoder cannot extract 7-Zip's automatic ARM64-filtered entries.
  if (environment.QILIN_DESKTOP_TARGET_PLATFORM === 'win32') selected.ELECTRON_BUILDER_7Z_FILTER = 'BCJ'
...
export async function packageTarget(
  ...
  await execute(['run', 'release:pack', '--family', 'qilin', '--out', buildPaths.packedQilin], buildEnv, REPOSITORY_ROOT)
  await execute([
    '--dir',
    'apps/desktop-host',
    'pack',
  ...
  await execute(['run', 'prepare:qilin'], targetEnv)

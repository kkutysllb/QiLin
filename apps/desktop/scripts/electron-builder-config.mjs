 ...
 * @param {string | undefined} preparedRuntime - Verified private qilin tree for installed-update qualification; ordinary releases use the target tree.
...
export function createElectronBuilderConfig(
  ...
  if (env.QILIN_DESKTOP_UNSIGNED !== undefined && !['0', '1'].includes(env.QILIN_DESKTOP_UNSIGNED)) {
    throw new Error('desktop package: QILIN_DESKTOP_UNSIGNED must be 0 or 1')
  }
  const unsigned = env.QILIN_DESKTOP_UNSIGNED === '1'
  ...
  const windowsSigner = packagesWindows && !unsigned
    ? createWindowsTokenSigner({
        ...
        preserveSignature: async path => primaryRuntimeDestination === undefined ? false : preserveWindowsRuntimeSignature(path, {
          ...
          runDirectory: env.QILIN_DESKTOP_PACKAGING_RUN_DIR,
        }),
      })
    : undefined
  ...
  if (preparedRuntime !== undefined) buildPaths.qilin = preparedRuntime
  return {
    appId,
    extraMetadata: { qilinDesktopAppId: appId, qilinMandatoryUpdatePolicy: policy },
    productName: 'QiLin',
    ...
    files: [
      ...
      'renderer/**/*',
      'package.json',
      { from: buildPaths.qilin, to: 'qilin', filter: ['**/*'] },
      // electron-builder excludes a source directory's root node_modules.
      { from: join(buildPaths.qilin, 'node_modules'), to: 'qilin/node_modules', filter: ['**/*'] },
    ],
    asarUnpack: [
    ...
    mac: {
      ...
      signIgnore: ['/Contents/Resources/app\\.asar\\.unpacked/qilin(?:/|$)', '/Contents/Resources/runtime/primary-runtime(?:/|$)', '\\.pak$'],
      notarize: true,
      target: ['dmg', 'zip'],
    },
    dmg: {
    ...
    afterPack: async context => {
      ...
      if (resolvedPlatform === 'win32' && !unsigned) {
        ...
        const prepared = await verifyDesktopRuntime(buildPaths.qilin,
          context.packager.appInfo.version, { platform: resolvedPlatform, arch: resolvedArch })
        writeDesktopRuntime(buildPaths.qilin, prepared.release, prepared.sharedPackages.map(entry => entry.name),
          { platform: resolvedPlatform, arch: resolvedArch })
      }
      await verifyDesktopRuntime(buildPaths.qilin,
        context.packager.appInfo.version, { platform: resolvedPlatform, arch: resolvedArch })
    },
    ...
    win: {
      ...
      signtoolOptions: {
        sign: windowsSigner,
        publisherName: windowsSigner === undefined ? undefined : resolveWindowsUpdatePublisher(env.QILIN_DESKTOP_WINDOWS_CER_FILE),

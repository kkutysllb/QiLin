...
const SIGNING_FIELDS = new Set(['QILIN_DESKTOP_WINDOWS_CER_FILE', 'QILIN_DESKTOP_WINDOWS_SIGNTOOL',
  'QILIN_DESKTOP_WINDOWS_KEY_CONTAINER', 'QILIN_DESKTOP_WINDOWS_TOKEN_PIN'])
 ...
 */
export async function assertInstalledUpdateSigningClear(stateFile = join(homedir(), '.qilin-desktop-signing', 'attempt.json')): Promise<void> {
...
async function inputHashes(manifest: string, version: string, environment: NodeJS.ProcessEnv): Promise<object> {
  ...
  const paths = [manifest, join(run.root, 'application/result.json'), join(run.root, version, 'qilin/desktop-runtime.json'),
    ...
    environment.QILIN_DESKTOP_WINDOWS_CER_FILE!, environment.QILIN_DESKTOP_WINDOWS_SIGNTOOL!]
  const files = []

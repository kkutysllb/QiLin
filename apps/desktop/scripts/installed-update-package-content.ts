...
export async function verifyInstalledUpdatePackageContent(manifest: string, version: string, payload: string, publisher: string) {
  ...
  const policy = resolveDesktopPolicyConfig(metadata.qilinMandatoryUpdatePolicy)
  if (metadata.name !== `qilin-update-test-${run.id}` || metadata.version !== version
    || metadata.qilinDesktopAppId !== run.appId || metadata.main !== 'qualification-bootstrap.mjs'
  ...
  const inspect = (node: AsarNode, path = ''): void => {
    ...
    if (node.link !== undefined || (node.unpacked === true && !path.startsWith('qilin/'))) {
    ...
    } else if (!path.startsWith('qilin/') && !expectedPaths.has(path)) {
      throw new Error('installed update: application archive contains additional files')
    }
  }
  inspect(archive.header)
  if (archive.header.files?.qilin?.files === undefined) throw new Error('installed update: application archive lacks the qilin runtime')
  ...
  if (update.provider !== 'generic' || update.url !== url || update.channel !== 'nightly'
    || update.updaterCacheDirName !== `qilin-update-test-${run.id}-updater`
  ...
  const prepared = await verifyDesktopRuntime(join(run.root, version, 'qilin'), version, { platform: 'win32', arch: 'x64' })
  const runtime = readDesktopRuntime(join(extracted, 'qilin'))
  ...
  const actualFiles = inventoryDesktopRuntime(join(extracted, 'qilin'))
  ...
  for (const [index, file] of prepared.files.entries()) {
    ...
    const transformed = file.path.startsWith('node_modules/') && file.path.endsWith('/package.json')
      ? await transform(runtimePath(join(run.root, version, 'qilin'), file.path)) : null
  ...
  return { version, appId: run.appId, applicationFiles: inventory.files.length, dependencies, dependenciesFrozen: false,
    ...
    resignedExecutables: runtime.files.filter(file => file.path.endsWith('.exe')).map((file) => {
      if (archive.getFile(join('qilin', file.path), false).unpacked !== true) {
        throw new Error('installed update: executable runtime file must be outside ASAR')
      }
      return join(payload, 'resources/app.asar.unpacked/qilin', file.path)
    }) }
}

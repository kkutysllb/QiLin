...
export async function preflightWindowsSigning(options: SigningPreflightOptions): Promise<void> {
  ...
  const state = options.stateDirectory ?? join(homedir(), '.qilin-desktop-signing')
  ...
  const certificate = await readFile(environment.QILIN_DESKTOP_WINDOWS_CER_FILE!)
  ...
  record({ type: 'signing-preflight-static', compiler,
    ...
    signToolSha256: createHash('sha256').update(await readFile(environment.QILIN_DESKTOP_WINDOWS_SIGNTOOL!)).digest('hex'),
...
async function main(): Promise<void> {
  ...
  const runDirectory = process.env.QILIN_DESKTOP_PACKAGING_RUN_DIR

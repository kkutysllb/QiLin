...
export function prepareDevelopmentProject(options: DevelopmentProjectOptions): string {
  ...
  if (cliManifest.name !== '@qilin/cli' || cliManifest.version !== options.release.version) {
    throw new Error(
      `desktop development: apps/cli must be @qilin/cli@${options.release.version}, found `
  ...
  if (hostManifest.name !== '@qilin/desktop-host' || hostManifest.version !== options.release.version) {
    throw new Error(
      `desktop development: apps/desktop-host must be @qilin/desktop-host@${options.release.version}, found `
  ...
  const qilinLink = join(destinationModules, '@qilin', 'cli')
  removeOwnedPath(qilinLink)
  linkDirectory(options.cliDir, qilinLink)
  const hostLink = join(destinationModules, '@qilin', 'desktop-host')
  removeOwnedPath(hostLink)
  linkDirectory(options.hostDir, hostLink)
  const sharedPackages = [...new Set([...names, '@qilin/cli', '@qilin/desktop-host'])].flatMap((name) => {

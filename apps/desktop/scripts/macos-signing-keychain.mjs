...
export async function withMacOSSigningKeychain(environment, action, run = execute) {
  ...
  const directory = mkdtempSync(join(tmpdir(), 'qilin-macos-signing-'))
  ...
  try {
    ...
    run('/usr/bin/codesign', ['--force', '--sign', `Developer ID Application: ${environment.QILIN_DESKTOP_MACOS_SIGNING_IDENTITY}`, '--keychain', keychain, '--timestamp', '--options', 'runtime', probe])

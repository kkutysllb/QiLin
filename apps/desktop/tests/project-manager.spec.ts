...
if (command !== 'rebuild') {
  ...
  for (const [name, version] of Object.entries(manifest.dependencies)) {
    ...
    writeFileSync(join(packageRoot, 'package.json'), JSON.stringify({name, version,
      peerDependencies: {'@qilin/kylin': '^1.0.0'}, qilin: {bundle: {patch: './bundle.yml'}}}))
...
function setup(): { root: string; manager: DesktopProjectManager } {
  ...
  const qilin = join(root, 'resources', 'qilin')
  runtimeFixture(qilin)
  return { root, manager: new DesktopProjectManager(resolveDesktopPaths(join(root, '.qilin')), { node: process.execPath, pnpm: writeFakePnpm(root), qilin }) }
}
...
describe('desktop external plugin profile', () => {
  it('changes runtime generations without deleting legacy host links', async () => {
    ...
    const qilin = join(root, 'next-runtime', 'qilin')
    runtimeFixture(qilin, '1.1.0')
  ...
  it('disables every third-party bundle without reading a broken plugin patch declaration', async () => {
    ...
    await manager.mutate({ type: 'plugins-disable-all' }, hooks({ afterChange: async () => {
      ...
      }).qilin.profile.bundles).not.toContain('plugin')
    } }))
    expect((JSON.parse(readFileSync(join(manager.paths.profile, 'package.json'), 'utf8')) as {
      qilin: { profile: { bundles: string[] } }
    }).qilin.profile.bundles).not.toContain('plugin')
  ...
  it('resets the entire profile without backups while retaining its lock and shared data', async () => {
    ...
    const task = join(root, '.qilin', 'task-sentinel')
    const homeEnvironment = join(root, '.qilin', '.env')
  ...
  it('reports damaged application metadata as a reinstall failure', async () => {
    const { manager } = setup()
    writeFileSync(join(manager.runtime.qilin, 'desktop-runtime.json'), '{broken')
    await expect(manager.applyRelease()).rejects.toThrow()
  ...
  it('retries installation after an interrupted runtime rebuild removed plugin files', async () => {
    ...
    await manager.applyRelease()
    await manager.mutate({ type: 'plugin-add', spec: 'plugin@1.0.0' }, hooks())
    const qilin = join(root, 'new-node')
    runtimeFixture(qilin, '1.1.0', '24.18.0')
    const failing = join(root, 'fail-install.mjs')
    writeFileSync(failing, 'process.exitCode = 1')
    const worker = new DesktopProjectManager(manager.paths, { ...manager.runtime, qilin, pnpm: failing })
    ...
    const retry = new DesktopProjectManager(manager.paths, { ...manager.runtime, qilin })
  ...
  it.each(['plugin-add', 'runtime-change'] as const)('retries failed rebuild after %s across manager instances', async (operation) => {
    const { root, manager } = setup()
    await manager.applyRelease()
    let qilin = manager.runtime.qilin
    if (operation === 'runtime-change') {
      await manager.mutate({ type: 'plugin-add', spec: 'plugin@1.0.0' }, hooks())
      qilin = join(root, 'new-node')
      runtimeFixture(qilin, '1.1.0', '24.18.0')
    }
  ...
  it('initializes and restarts offline without executing pnpm', async () => {
    ...
    expect(realpathSync(join(manager.paths.profile, 'node_modules/@qilin/kylin'))).toBe(realpathSync(join(manager.runtime.qilin, 'node_modules/@qilin/kylin')))
  ...
  it('repairs a removed managed link without running pnpm', async () => {
    const { root, manager } = setup()
    await manager.applyRelease()
    unlinkSync(join(manager.paths.profile, 'node_modules/@qilin/kylin'))
    await expect(manager.applyRelease()).resolves.toBe(true)
    expect(calls(root)).toEqual([])
  })
  ...
  it.skipIf(process.platform !== 'win32')('reuses the profile when the launch path changes only Windows letter casing', async () => {
    const { manager } = setup()
    await manager.applyRelease()
    const relaunched = new DesktopProjectManager(manager.paths, { ...manager.runtime, qilin: manager.runtime.qilin.toUpperCase() })
  ...
  it.each(['changed', 'same-size', 'extra', 'missing'])('starts and reuses a profile without checking %s runtime bytes', async (operation) => {
    const { root, manager } = setup()
    if (operation === 'changed') writeFileSync(join(manager.runtime.qilin, 'package.json'), '{}')
    if (operation === 'same-size') writeFileSync(join(manager.runtime.qilin, 'package.json'), '{"type":"Module"}\n')
    if (operation === 'extra') writeFileSync(join(manager.runtime.qilin, 'extra'), '')
    if (operation === 'missing') unlinkSync(join(manager.runtime.qilin, 'package.json'))
    await expect(manager.applyRelease()).resolves.toBe(true)
  ...
  it('installs only plugins and checks the graph before running lifecycle scripts', async () => {
    ...
    await expect(manager.mutate({ type: 'plugin-add', spec: '@qilin/kylin' }, hooks())).rejects.toThrow(/host-owned/u)
    await expect(manager.applyRelease()).resolves.toBe(false)
    expect(calls(root)).toHaveLength(2)
  })
  ...
  it('keeps plugin files and patches through a compatible release and application relocation', async () => {
    ...
    const nextRoot = join(root, 'relocated', 'qilin')
    runtimeFixture(nextRoot, '1.1.0')
    const next = new DesktopProjectManager(manager.paths, { ...manager.runtime, qilin: nextRoot })
    ...
    expect(calls(root)).toHaveLength(2)
    expect(realpathSync(join(manager.paths.profile, 'node_modules/@qilin/kylin'))).toBe(realpathSync(join(nextRoot, 'node_modules/@qilin/kylin')))
  ...
  it('reinstalls the locked plugin graph when bundled Node changes', async () => {
    ...
    const qilin = join(root, 'new-node')
    runtimeFixture(qilin, '1.1.0', '24.18.0')
    const next = new DesktopProjectManager(manager.paths, { ...manager.runtime, qilin })
  ...
  it('allows incompatible plugins to be disabled in recovery without deleting them', async () => {
    ...
    await manager.applyRelease()
    await manager.mutate({ type: 'plugin-add', spec: 'plugin@1.0.0' }, hooks())
    const qilin = join(root, 'next-major')
    runtimeFixture(qilin, '2.0.0')
    const next = new DesktopProjectManager(manager.paths, { ...manager.runtime, qilin })
  ...
  it('keeps partial package changes and restores host links after pnpm fails', async () => {
    ...
    expect(realpathSync(join(manager.paths.profile, 'node_modules/@qilin/kylin')))
      .toBe(realpathSync(join(manager.runtime.qilin, 'node_modules/@qilin/kylin')))
    await manager.mutate({ type: 'plugin-remove', name: 'plugin' }, hooks())
    expect(manager.listPlugins()).toEqual([])
  })

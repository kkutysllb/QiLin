...
import { readProfilePlugins } from '@deepseek-ai/qilin-app-boot'
...
function seedPlugin(manager: DesktopProjectManager): void {
  ...
  manifest.qilin.profile.bundles.push('plugin')
  writeFileSync(path, JSON.stringify(manifest))
  const directory = join(manager.paths.profile, 'node_modules/plugin')
  mkdirSync(directory, { recursive: true })
  writeFileSync(join(directory, 'package.json'), JSON.stringify({ name: 'plugin', version: '1.0.0', qilin: { bundle: { patch: 'bundle.yml' } } }))
  writeFileSync(join(directory, 'bundle.yml'), '[]\n')
}
function plugins(manager: DesktopProjectManager) {
  return readProfilePlugins({ binName: 'qilin', profileDir: manager.paths.profile,
    installAnchor: join(manager.runtime.qilin, 'node_modules/@deepseek-ai/qilin/package.json') }).dependencies
...
function setup(): { root: string; manager: DesktopProjectManager } {
  ...
  return { root, manager: new DesktopProjectManager(resolveDesktopPaths(join(root, '.qilin')), { qilin }) }
}
afterEach(() => {
...
describe('desktop external plugin profile', () => {
  it('cleans application packages only when preparing a production launch', async () => {
    ...
    const name = '@deepseek-ai/qilin-web-app'
  ...
  it('disables plugins before runtime initialization and backs up the patch while preserving package files', async () => {
    ...
    const uninitialized = new DesktopProjectManager(manager.paths, { ...manager.runtime, qilin: 'missing-runtime' })
    ...
    expect(manifest.qilin.profile.bundles).not.toContain('plugin')
    expect(manifest.qilin.profile.bundles).toContain('@deepseek-ai/qilin-web-app')
    await manager.applyRelease()

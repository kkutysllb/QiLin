...
import type { DesktopUpdateState, QilinDesktopProductApi } from '../src/ipc.ts'
...
it('keeps the Electron preload assignable to the product consumer', () => {
  const assign = (bridge: QilinDesktopProductApi['updates']): DesktopUpdateBridge => bridge
...
it('records visible ordinary-update states without authorizing downloads or installation', async () => {
  ...
  const states = phases.map(phase => presentDesktopUpdate({ phase, version: '3.0.0', percent: 58 }))

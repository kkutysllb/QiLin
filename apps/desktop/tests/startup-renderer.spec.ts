...
import type { QilinDesktopStartupApi } from '../src/ipc.ts'
...
function startup(locale = 'en', status: Promise<DesktopBackendState> = Promise.resolve({ phase: 'starting' })) {
  ...
  const api: QilinDesktopStartupApi = {
    protocolVersion: 1,
  ...
  Object.defineProperty(dom.window, 'qilinDesktop', { value: api })
...
it('keeps emergency diagnostics inert without shell assets', () => {
  ...
  expect([...dom.window.document.querySelectorAll('form')].map(form => form.action)).toEqual([
    'qilin-recovery://restart', 'qilin-recovery://plugins', 'qilin-recovery://reset',
  ])
  dom.window.close()
})
...
it('offers only restart before emergency profile recovery is available', () => {
  ...
  expect([...dom.window.document.querySelectorAll('form')].map(form => form.action)).toEqual(['qilin-recovery://restart'])
  dom.window.close()
})

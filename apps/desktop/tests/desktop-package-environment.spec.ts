...
const POLICY = { QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: 'https://policy.example.com' }
const RELEASE = { ...POLICY, QILIN_DESKTOP_APP_ID: 'com.example.desktop', DOWNLOAD_TEST_ORIGIN: 'https://updates.example.com' }
const MAC_IDENTITY = { QILIN_DESKTOP_MACOS_SIGNING_IDENTITY: 'Example Company (TEAMID1234)', QILIN_DESKTOP_MACOS_TEAM_ID: 'TEAMID1234' }
...
describe('Desktop local packaging configuration', () => {
  it('selects the platform file, preserves literal secrets, and excludes stale ambient release settings', async () => {
    await withDirectory(async (directory) => {
      await writeFile(join(directory, '.env.windows'), '\uFEFFQILIN_DESKTOP_APP_ID=com.example.windows\r\nQILIN_DESKTOP_WINDOWS_TOKEN_PIN=" #!$%&literal "\r\nQILIN_DESKTOP_WINDOWS_CER_FILE="keys/public certificate.cer"\r\n')
      await writeFile(join(directory, '.env.macos'), 'QILIN_DESKTOP_APP_ID=com.example.mac\nAPPLE_KEYCHAIN_PROFILE=release\nCSC_LINK=keys/signing.p12\nCSC_KEY_PASSWORD=" # literal "\n')
      const parent = {
        PATH: 'build-tools', QILIN_DESKTOP_APP_ID: 'com.stale.desktop',
        QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG: '{"origin":"https://stale.example.com"}',
        qilin_desktop_mandatory_update_config: 'stale-policy',
        QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: 'https://stale.example.com',
        qilin_desktop_mandatory_update_prod_origin: 'https://stale.example.com',
        QILIN_DESKTOP_WINDOWS_TOKEN_PIN: 'stale-pin', APPLE_ID: 'stale-apple-id',
        CSC_LINK: 'stale-certificate', DOWNLOAD_TEST_ORIGIN: 'https://stale.example.com',
        qilin_desktop_windows_key_container: 'case-insensitive-stale-container',
      }
      expect(loadDesktopPackageEnvironment('win32', parent, directory)).toEqual({
        PATH: 'build-tools', QILIN_DESKTOP_APP_ID: 'com.example.windows',
        QILIN_DESKTOP_WINDOWS_TOKEN_PIN: ' #!$%&literal ',
        QILIN_DESKTOP_WINDOWS_CER_FILE: join(directory, 'keys', 'public certificate.cer'),
      })
      expect(loadDesktopPackageEnvironment('darwin', parent, directory)).toEqual({
        PATH: 'build-tools', QILIN_DESKTOP_APP_ID: 'com.example.mac', APPLE_KEYCHAIN_PROFILE: 'release',
        CSC_LINK: join(directory, 'keys/signing.p12'), CSC_KEY_PASSWORD: ' # literal ',
      })
      expect(parent.QILIN_DESKTOP_WINDOWS_TOKEN_PIN).toBe('stale-pin')
      expect(parent.qilin_desktop_mandatory_update_config).toBe('stale-policy')
    })
  })
  ...
  it('loads mandatory update origins and options only from the platform file without changing its parent', async () => {
    await withDirectory(async (directory) => {
      ...
      const origins = { QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: 'https://test.example.invalid',
        QILIN_DESKTOP_MANDATORY_UPDATE_PROD_ORIGIN: 'https://prod.example.invalid' }
      const lines = Object.entries(origins).map(([key, value]) => `${key}=${value}\n`).join('')
      await writeFile(join(directory, '.env.windows'), `${lines}QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG='${windowsPolicy}'\n`)
      await writeFile(join(directory, '.env.macos'), `${lines}QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG='${macPolicy}'\n`)
      const parent = { QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG: 'stale-policy' }
      expect(loadDesktopPackageEnvironment('win32', parent, directory)).toEqual({ ...origins, QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG: windowsPolicy })
      expect(loadDesktopPackageEnvironment('darwin', parent, directory)).toEqual({ ...origins, QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG: macPolicy })
      expect(parent).toEqual({ QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG: 'stale-policy' })
    })
  })
  ...
  it('checks application and update configuration before Windows credentials while preserving unsigned and preparation modes', () => {
    ...
    }).toThrow(/QILIN_DESKTOP_APP_ID/u)
    expect(() => {
      validateDesktopPackageEnvironment({ QILIN_DESKTOP_APP_ID: 'invalid' }, WINDOWS)
    }).toThrow(/reverse-DNS/u)
    expect(() => {
      validateDesktopPackageEnvironment({ ...POLICY, QILIN_DESKTOP_APP_ID: RELEASE.QILIN_DESKTOP_APP_ID }, WINDOWS)
    }).toThrow(/DOWNLOAD_TEST_ORIGIN/u)
    expect(() => {
      validateDesktopPackageEnvironment(RELEASE, WINDOWS)
    }).toThrow(/QILIN_DESKTOP_WINDOWS_CER_FILE/u)
    expect(() => {
      validateDesktopPackageEnvironment({ ...POLICY, QILIN_DESKTOP_APP_ID: RELEASE.QILIN_DESKTOP_APP_ID }, WINDOWS, { unsigned: true })
    }).not.toThrow()
    expect(() => {
      validateDesktopPackageEnvironment({ ...POLICY, QILIN_DESKTOP_APP_ID: RELEASE.QILIN_DESKTOP_APP_ID }, WINDOWS, { prepareOnly: true })
    }).not.toThrow()
  })
  ...
  it('rejects incomplete macOS identity and credentials and checks referenced files without contacting Apple', async () => {
    ...
    }).toThrow(/QILIN_DESKTOP_MACOS_SIGNING_IDENTITY/u)
    expect(() => {

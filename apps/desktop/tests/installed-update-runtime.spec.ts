...
async function fixture<T>(body: (directory: string) => Promise<T>): Promise<T> {
  const directory = await mkdtemp(join(tmpdir(), 'qilin-update-materials-'))
  ...
  it('retags only release-owned packages and dependencies while preserving source and third-party versions', async () => {
    await fixture(async (directory) => {
      ...
      for (const version of versions) {
        const runtime = join(run.root, version, 'qilin')
        const descriptor = await verifyDesktopRuntime(runtime, version)
        expect(descriptor.sharedPackages.find(entry => entry.name === '@qilin/kylin')?.version).toBe(source.version)
        expect(descriptor.sharedPackages.find(entry => entry.name === '@qilin/cli')?.version).toBe(version)
        const metadata = JSON.parse(await readFile(join(runtime, 'node_modules/@qilin/cli/package.json'), 'utf8')) as {
          dependencies: Record<string, string>
        }
        expect(metadata.dependencies['@qilin/desktop-host']).toBe(version)
        expect(metadata.dependencies['@qilin/kylin']).toBe(source.version)
      }

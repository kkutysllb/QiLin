...
const source = { version: '3.0.0', commit: 'a'.repeat(40), dirtyFiles: [] }

async function fixture<T>(body: (directory: string) => Promise<T>): Promise<T> {
  const directory = await mkdtemp(join(tmpdir(), 'qilin-update-bootstrap-'))
...
console.log(JSON.stringify({ paths: app.paths, home: process.env.QILIN_HOME, journals: process.env.QILIN_DESKTOP_UPDATE_JOURNAL_DIR }))
`)
      const results = []
      for (const version of versions) {
        await writeFile(join(bootstrap, 'package.json'), JSON.stringify({ type: 'module', version, qilinDesktopAppId: run.appId }))
        const { stdout, stderr } = await promisify(execFile)(process.execPath, [join(bootstrap, 'qualification-bootstrap.mjs')], {
          windowsHide: true, env: { SystemRoot: process.env.SystemRoot, PATH: process.env.PATH,
            QILIN_HOME: 'must-not-be-used', QILIN_DESKTOP_UPDATE_JOURNAL_DIR: 'must-not-be-used' },
        })
        expect(stderr).toBe('')
        results.push(JSON.parse(stdout))
      }
      const root = join(directory, 'application-data/qilin-update-qualification', run.id)
      expect(results).toEqual([0, 1].map(() => ({ paths: { userData: join(root, 'user-data'), sessionData: join(root, 'user-data') },
        home: join(root, 'qilin-home'), journals: join(root, 'journals') })))
      expect((await readdir(root)).sort()).toEqual(['qilin-home', 'journals', 'user-data'])
  ...
  it('refuses foreign metadata before creating any data directory or changing the environment', async () => {
    await fixture(async (directory) => {
      ...
      expect(() => configureInstalledUpdateIdentity(app, { id: 'a'.repeat(24), versions },
        { version: versions[0], qilinDesktopAppId: 'com.deepseek.qilin' }, env)).toThrow('identity')
      expect(await readdir(directory)).toEqual([])
      expect(env).toEqual({ QILIN_HOME: 'existing' })
    })
  })

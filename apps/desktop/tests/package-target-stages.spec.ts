...
const environment = { QILIN_DESKTOP_APP_ID: 'com.example.test', QILIN_DESKTOP_AUTO_UPDATE_ENV: 'test',
  DOWNLOAD_TEST_ORIGIN: 'https://updates.example.com', QILIN_DESKTOP_WINDOWS_TOKEN_PIN: 'fixture-pin' }
...
it('requires one signing preflight before building, then records only the complete release', async () => {
  ...
  expect(run.run.mock.calls[0]![3]).toMatchObject({ env: { QILIN_DESKTOP_WINDOWS_TOKEN_PIN: 'fixture-pin' }, timeoutMs: 60_000 })
  expect(run.run.mock.calls[1]![3].env).not.toHaveProperty('QILIN_DESKTOP_WINDOWS_TOKEN_PIN')
  expect(writeFileSync).toHaveBeenCalledOnce()
})

it.each(['preflight:windows-signing', 'run build:official', 'run sign:primary-runtime', 'run prepare:qilin',
...
it.each(['--unsigned', '--prepare-only'])('keeps %s hardware-free and creates no release record', async (mode) => {
  ...
  for (const call of run.run.mock.calls) expect(call[3].env).not.toHaveProperty('QILIN_DESKTOP_WINDOWS_TOKEN_PIN')
  expect(writeFileSync).not.toHaveBeenCalled()
})

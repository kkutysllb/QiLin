...
process.send({ type: 'ready', protocolVersion: 3, qilinVersion: '1.0.0' })
...
`)
    ...
    try {
      await host.start()
      await expect(host.fetch(new Request('qilin-app://app/'))).rejects.toThrow('plugin unavailable')
      await host.stop()
  ...
  it('loads the resource entry with a separate profile and scrubs Node resolution overrides', async () => {
    const runtime = projectWithHost(`
process.send({ type: 'ready', protocolVersion: 3, qilinVersion: 'split-runtime' })
function onRequestFrame(frame) {
...
`)
    ...
    try {
      const response = await host.fetch(new Request('qilin-app://app/environment'))

...
const versions = ['3.0.0.20260916.1', '3.0.0.20260916.2'] as const
const source = { version: '3.0.0', commit: 'a'.repeat(40), dirtyFiles: [' M apps/desktop/example.ts'] }
...
async function fixture<T>(body: (directory: string) => Promise<T>): Promise<T> {
  const directory = await mkdtemp(join(tmpdir(), 'qilin-installed-update-'))
...
describe('installed-update qualification materials', () => {
  it('allocates independent test-only namespaces and does not produce packages or overwrite an earlier manifest', async () => {
    await fixture(async (directory) => {
      ...
      expect(first.feedKey).toBe(`qilin-desk/feeds/qualification/${first.id}/win-x64/nightly.yml`)
  ...
  it('collects exact validated journal bytes in independent snapshots without copying other files or declaring acceptance', async () => {
    await fixture(async (directory) => {
      ...
      const journals = join(directory, 'qilin-update-qualification', run.id, 'journals')

...
The examples use base version `3.0.0`. Before creating new material, substitute the actual base, Asia/Shanghai date, and unused index according to the [release version rules](../../README.md#release-versions).

```powershell
node --import tsx apps/desktop/scripts/prepare-installed-update.ts init 3.0.0.20260916.1 3.0.0.20260916.2
```
...
```powershell
node --import tsx apps/desktop/scripts/package-installed-update.ts "<run.json>" 3.0.0.20260916.1 --check
```
...
Before the operator starts, supply both verified installers, generated metadata and blockmaps, packaging and signature records, the exact installed executable path, and fixed test feed URL. Both packages must use the same test identity, isolated data directories, and absolute external `QILIN_DESKTOP_UPDATE_JOURNAL_DIR` on every launch, including an installer-triggered restart. A variable set only in the original launch terminal is insufficient. The [Windows signing rules](../../README.md#windows-ev-signing) still apply; a manifest never clears a signing interlock.
...
```powershell
node --import tsx apps/desktop/scripts/verify-installed-update-package.ts "<run.json>" 3.0.0.20260916.1 "<public.cer>" "<reviewed-7za.exe>"
```
...
```powershell
node --import tsx apps/desktop/scripts/prepare-installed-update.ts inspect 3.0.0.20260916.1 3.0.0.20260916.2 "<journal-directory>"
```
...
To retain a local journal snapshot and its report, use `collect` with the matching installed application's `qilin-update-qualification/<run-id>/journals` directory. It validates all selected records before creating a fresh `evidence/collection-*` under the material run. The copied journals and `report.json` describe the same in-memory snapshot, with SHA-256 values and `operatorAcceptance: pending`; original files are unchanged. Other files, settings, conversations, build output, and credentials are not collected. Keep their separately reviewed evidence alongside the report, not inside the journal directory.

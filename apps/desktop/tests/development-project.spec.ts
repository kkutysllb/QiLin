...
describe('desktop development project', () => {
  it('manages development plugins without modifying the linked workspace packages', async () => {
    ...
    mkdirSync(join(dependencies, '@qilin', 'cli'), { recursive: true })
    writeFileSync(join(cli, 'package.json'), '{"name":"@qilin/cli","version":"1.2.3"}\n')
    writeFileSync(join(host, 'package.json'), '{"name":"@qilin/desktop-host","version":"1.2.3"}\n')
    writeFileSync(join(host, 'lib', 'index.js'), '')
    writeFileSync(join(dependencies, '@qilin', 'cli', 'package.json'), '{}\n')
    ...
    expect(realpathSync(join(project, 'node_modules', '@qilin', 'cli'))).toBe(realpathSync(cli))
    expect(realpathSync(join(project, 'node_modules', '@qilin', 'desktop-host'))).toBe(realpathSync(host))
    ...
    expect(manifest.dependencies['@qilin/cli']).toBe('1.2.3')
    expect(manifest.dependencies['@qilin/desktop-host']).toBe('1.2.3')
    const manager = new DesktopProjectManager(resolveDesktopPaths(join(root, 'home')), {
      qilin: project,
    })
    await manager.applyRelease()
    await manager.disableAllPlugins()
    expect(readFileSync(join(cli, 'package.json'), 'utf8')).toBe('{"name":"@qilin/cli","version":"1.2.3"}\n')
  ...
  it('rejects a CLI package from another release', () => {
    ...
    writeFileSync(join(cli, 'package.json'), '{"name":"@qilin/cli","version":"2.0.0"}\n')
    writeFileSync(join(host, 'package.json'), '{"name":"@qilin/desktop-host","version":"1.2.3"}\n')
    writeFileSync(join(host, 'lib', 'index.js'), '')
    expect(() => prepareDevelopmentProject({
      ...
      dependencyDir: dependencies,
      release: release(),
    })).toThrow(/must be @qilin\/cli@1\.2\.3/u)
  })
})

...
async function withCertificate(subject: unknown, action: (file: string, signTool: string) => Promise<void>): Promise<void> {
  const directory = await mkdtemp(join(tmpdir(), 'qilin-update-publisher-'))
  try {
...
describe('Windows update publisher', () => {
  beforeAll(() => {
    vi.stubEnv('QILIN_DESKTOP_APP_ID', 'com.example.publisher-test')
    vi.stubEnv('QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN', 'https://policy.example.com')
    vi.stubEnv('QILIN_DESKTOP_TARGET_PLATFORM', 'win32')
    vi.stubEnv('QILIN_DESKTOP_UNSIGNED', '1')
  })
  ...
  it.each([true, false])('supplies publisher metadata with explicit Windows target = %s', async (explicitTarget) => {
    ...
    await withCertificate({ CN: 'Publisher', O: 'Company', C: 'CN' }, async (file, signTool) => {
      const config = createElectronBuilderConfig({
        QILIN_DESKTOP_APP_ID: 'com.example.publisher-test',
        QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: 'https://policy.example.com',
        QILIN_DESKTOP_MANDATORY_UPDATE_PROD_ORIGIN: 'https://policy.example.com',
        ...(explicitTarget ? { QILIN_DESKTOP_TARGET_PLATFORM: 'win32' } : {}),
        QILIN_DESKTOP_WINDOWS_CER_FILE: file,
        QILIN_DESKTOP_WINDOWS_SIGNTOOL: signTool,
        QILIN_DESKTOP_WINDOWS_KEY_CONTAINER: 'test-container',
        QILIN_DESKTOP_WINDOWS_TOKEN_PIN: 'test-pin',

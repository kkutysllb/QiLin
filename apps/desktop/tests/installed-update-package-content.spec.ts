...
async function fixture(body: (context: {
  ...
  const root = await mkdtemp(join(tmpdir(), 'qilin-package-content-'))
  try {
    const run = await createInstalledUpdateRun(root, versions, { version: '3.0.0', commit: 'a'.repeat(40), dirtyFiles: [] })
    ...
    await writeFile(join(source, 'package.json'), JSON.stringify({ name: `qilin-update-test-${run.id}`, version,
      qilinDesktopAppId: run.appId, main: 'qualification-bootstrap.mjs', type: 'module',
      qilinMandatoryUpdatePolicy: { origin: 'https://policy.example.com', allowedPageOrigins: ['https://policy.example.com'],
    ...
    const descriptor = runtimeFixture(qilin, version)
    await writeFile(join(qilin, 'tool.exe'), 'inert executable fixture')
    ...
    reseal(qilin)
    await cp(qilin, join(source, 'qilin'), { recursive: true })
    ...
    await writeFile(join(payload, 'resources/app-update.yml'), JSON.stringify({ provider: 'generic', channel: 'nightly',
      ...
      updaterCacheDirName: `qilin-update-test-${run.id}-updater` }))
    const seal = async () => {
...
describe('installed update archive contents', () => {
  it.each(versions)('reads real ASAR and runtime inventories for %s without claiming installation', async (version) => {
    await fixture(async ({ manifest, payload }) => {
      expect(await verifyInstalledUpdatePackageContent(manifest, version, payload, publisher)).toMatchObject({
        ...
        resignedExecutables: [join(payload, 'resources/app.asar.unpacked/qilin/tool.exe')],
      })
    }, version)
  })

  it.each(['name', 'version', 'qilinDesktopAppId', 'main', 'type', 'qilinMandatoryUpdatePolicy'])(
    'rejects mismatched packaged %s', async (field) => {
      await fixture(async ({ manifest, source, payload, version, seal }) => {
        ...
        await expect(verifyInstalledUpdatePackageContent(manifest, version, payload, publisher))
          .rejects.toThrow(field === 'qilinMandatoryUpdatePolicy' ? 'desktop policy' : 'identity')
      })
    })
  ...
  it('accepts only the builder transformation of runtime dependency manifests', async () => {
    await fixture(async ({ manifest, source, payload, version, seal }) => {
      const preparedRoot = join(dirname(manifest), version, 'qilin')
      const packagePath = 'node_modules/@qilin/cli/package.json'
      ...
      await cp(join(preparedRoot, 'desktop-runtime.json'), join(source, 'qilin/desktop-runtime.json'))
      const packagedPath = join(source, 'qilin', packagePath)
      const packaged = { ...data }
  ...
  it.each(['unsealed', 'resealed-change', 'resealed-addition'])(
    'rejects %s runtime content even when metadata is regenerated', async (failure) => {
      await fixture(async ({ manifest, source, payload, version, seal, resealRuntime }) => {
        await writeFile(join(source, 'qilin', failure === 'resealed-addition' ? 'extra.js' : 'package.json'), '{}')
  ...
  it('identifies changed executable resources for separate signature verification', async () => {
    await fixture(async ({ manifest, source, payload, version, resealRuntime }) => {
      const executable = join(payload, 'resources/app.asar.unpacked/qilin/tool.exe')
      await writeFile(join(source, 'qilin/tool.exe'), 'inert changed executable, not a signature')
      await resealRuntime()

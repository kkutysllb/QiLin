...
const identity = { platform: 'desktop-win', arch: 'x64', version: '0.1.5-rc.1', bundledQilinVersion: '0.1.5-rc.1',
  bundleId: 'com.deepseek.qilin', locale: 'zh-CN' } as const
...
describe('mandatory update policy', () => {
  ...
  it('sends an anonymous independent request with installed release headers and fixed Nightly', async () => {
    ...
    expect(options).toMatchObject({ credentials: 'omit', cache: 'no-store', redirect: 'error', headers: {
      'x-client-platform': 'desktop-win', 'x-client-version': '0.1.5-rc.1', 'x-client-bundle-id': 'com.deepseek.qilin',
      'x-client-locale': 'zh-CN', 'x-client-arch': 'x64', 'x-client-update-channel': 'nightly',
      'x-client-bundled-qilin-version': '0.1.5-rc.1',
    } })
  })

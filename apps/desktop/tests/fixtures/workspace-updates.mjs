...
const root = process.env.QILIN_WORKSPACE_UPDATE_ROOT
assert.ok(root)
...
process.env.QILIN_DESKTOP_APP_ID = 'com.deepseek.qualification'
process.env.QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG = JSON.stringify({ origin: new URL(server.url).origin,
...
async function control(action) {
  ...
  const response = await fetch(`${url}/${action}`, { method: 'POST', headers: { 'x-qualification-token': process.env.QILIN_WORKSPACE_UPDATE_TOKEN } })
...
async function dialogWith(message) {
  ...
  await waitFor(async () => {
    for (const window of BrowserWindow.getAllWindows()) {
      if (window.webContents.getURL() !== 'qilin-app://shell/update-dialog.html') continue
      try {
...
async function qualify() {
  ...
  try {
    ...
    await documentReady(mainWindow, `document.querySelector('[class*="frame"]') && window.qilinDesktop?.updates`)
    ...
    assert.equal(BrowserWindow.getAllWindows().some(window => window.webContents.getURL() === 'qilin-app://shell/update-dialog.html'), false)
    ...
    const mandatory = await windowAt('qilin-app://shell/mandatory-update.html')
    ...
    checkMenu.click()
    await waitFor(async () => (await mandatory.webContents.executeJavaScript('window.qilinMandatoryUpdate.status()')).policy.error === 'unavailable', 'retained policy failure')
    ...
    server.policy('force')
    checkMenu.click()
    const forcedRecovery = await windowAt('qilin-app://shell/mandatory-update.html')

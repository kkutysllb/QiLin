...
const root = process.env.QILIN_SIGNED_UPDATE_TEST_ROOT
...
async function main() {
  await app.whenReady()
  const publisher = resolveWindowsUpdatePublisher(process.env.QILIN_SIGNED_UPDATE_CERTIFICATE)
  const server = await createArtifactUpdateServer({
    signed: process.env.QILIN_SIGNED_UPDATE_SIGNED,
    unsigned: process.env.QILIN_SIGNED_UPDATE_UNSIGNED,
    ...(process.env.QILIN_SIGNED_UPDATE_OLD ? {
      old: process.env.QILIN_SIGNED_UPDATE_OLD,
      oldBlockmap: `${process.env.QILIN_SIGNED_UPDATE_OLD}.blockmap`,
      signedBlockmap: `${process.env.QILIN_SIGNED_UPDATE_SIGNED}.blockmap`,
    } : {}),
  })

...
const root = process.env.QILIN_LOCAL_UPDATE_TEST_ROOT
...
protocol.registerSchemesAsPrivileged([{ scheme: 'qilin-app', privileges: { standard: true, secure: true, supportFetchAPI: true } }])
const cases = []
const presentations = []
...
async function main() {
  ...
  try {
    ...
    await scenario('mandatory-real-window-policy-download-retry-and-clear', async () => {
      protocol.handle('qilin-app', async request => {
      ...
      const policy = new DesktopMandatoryUpdatePolicy(config, { platform: 'desktop-win', arch: 'x64',
        version: '1.0.0', bundledQilinVersion: '1.0.0', bundleId: 'com.deepseek.qilin', locale: 'zh-CN' }, () => { modal?.sync() })
      ...
      try {
        ...
        async function until(expression) {
          try { return await window.webContents.executeJavaScript(`new Promise((resolve, reject) => {
            ...
            console.error('Modal condition:', expression, await window.webContents.executeJavaScript('({ url: location.href, text: document.body?.innerText, bridge: typeof window.qilinMandatoryUpdate })'))
            throw error
          }
        }

...
const { LlmAdapter, createUserMessage } = await import(pathToFileURL(require.resolve('@qilin/llm')).href)
const observed = []
const { runProfile } = await import(pathToFileURL(require.resolve('@qilin/cli/profile-boot')).href)
const { loadLayeredEnv, loadProfileDirectory } = await import(pathToFileURL(require.resolve('@qilin/app-boot')).href)
const installAnchor = require.resolve('@qilin/cli/package.json')
const running = await runProfile({ environment: loadLayeredEnv('qilin'), profile: 'desktop',
  resolvedProfile: { profile: loadProfileDirectory('qilin', project, installAnchor), installAnchor },

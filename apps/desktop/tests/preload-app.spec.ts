...
import { DESKTOP_IPC, type QilinDesktopStartupApi } from '../src/ipc.ts'
...
it.each(['qilin-app://app/index.html', 'https://shell/startup.html'])('exposes only the carrier marker to %s', async (url) => {
...
it('provides startup controls and a removable state subscription to shell documents', async () => {
  vi.stubGlobal('location', new URL('qilin-app://shell/startup.html'))
  await import('../src/preload-app.ts')
  const api = electron.contextBridge.exposeInMainWorld.mock.calls[0]?.[1] as QilinDesktopStartupApi
  await api.locale()

...
describe('Windows token signing', () => {
  it('preserves verified copies without hardware and rejects the entire queue after preservation failure', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'qilin-windows-copy-signature-'))
    try {
  ...
  it('stops concurrent and subsequent signing tasks after a PIN failure', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'qilin-windows-pin-failure-'))
    try {
      const certificateFile = join(directory, 'server.cer')
      const signTool = join(directory, 'signtool.exe')
      ...
      validateDesktopPackageEnvironment({
        QILIN_DESKTOP_APP_ID: 'com.example.desktop', DOWNLOAD_TEST_ORIGIN: 'https://updates.example.com',
        QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: 'https://policy.example.com',
        QILIN_DESKTOP_WINDOWS_CER_FILE: certificateFile, QILIN_DESKTOP_WINDOWS_SIGNTOOL: signTool,
        QILIN_DESKTOP_WINDOWS_TOKEN_PIN: 'fixture-pin', QILIN_DESKTOP_WINDOWS_KEY_CONTAINER: 'fixture-container',

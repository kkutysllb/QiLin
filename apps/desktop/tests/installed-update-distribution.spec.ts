...
async function fixture<T>(body: (manifest: string) => Promise<T>): Promise<T> {
  const directory = await mkdtemp(join(tmpdir(), 'qilin-update-distribution-'))
  try {
    const run = await createInstalledUpdateRun(directory, versions, { version: '3.0.0', commit: 'a'.repeat(40), dirtyFiles: [] })
...
describe('qualification distribution file planning', () => {
  it('keeps two binary sets separate from successive bodies at one fixed feed URL without publication authority', async () => {
    await fixture(async (manifest) => {
      ...
      expect(next!.binaries.every(file => file.key.startsWith('qilin-desk/bin/qualification/'))).toBe(true)
      expect(next!.feed.key).toMatch(/^qilin-desk\/feeds\/qualification\/[a-f0-9]{24}\/win-x64\/nightly.yml$/u)

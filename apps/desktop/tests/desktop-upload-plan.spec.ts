...
async function fixture(
  ...
  await writeFile(join(artifactsRoot, `${target}-release.json`), `${JSON.stringify({
    ...
    version,
    environment,
    publicUrl: `${origin}/qilin-desk/feeds/${target}/`,
  })}\n`)
...
describe('desktop upload plan', () => {
  it('publishes fixed feeds referencing versioned binaries without overriding CDN cache policy', async () => {
    ...
    expect(plan.artifacts.map(artifact => artifact.key)).toEqual([
      'qilin-desk/bin/win-x64/deepseek-harness-1.2.3-win-x64.exe',
      'qilin-desk/bin/win-x64/deepseek-harness-1.2.3-win-x64.exe.blockmap',
      'qilin-desk/feeds/win-x64/nightly.yml',
      'qilin-desk/feeds/win-x64/latest.yml',
    ])
    expect(load(plan.artifacts[2]!.contents!)).toMatchObject({
      version: '1.2.3',
      files: [{
        url: 'https://download.deepseek.com/qilin-desk/bin/win-x64/deepseek-harness-1.2.3-win-x64.exe',
  ...
  it('validates macOS artifacts and puts channel metadata last', async () => {
    ...
    expect(plan).toMatchObject({
      environment: 'test',
      version: '1.2.3',
      publicUrl: 'https://desktop-updates.example.com/qilin-desk/feeds/mac-arm64/',
      bucket: TEST_BUCKET,
    })
  ...
  it('validates the Windows installer with the emitted external blockmap and production destination', async () => {
    ...
    expect(plan).toMatchObject({
      publicUrl: 'https://download.deepseek.com/qilin-desk/feeds/win-x64/',

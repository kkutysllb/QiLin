  ...
  class FakeWindow extends EventEmitter {
    ...
    readonly webContents = Object.assign(new EventEmitter(), {
      ...
      send: vi.fn((channel: string, state: { phase?: string }) => {
        if (channel === 'qilin-desktop:backend-state' && state.phase === 'error') errorPublished.resolve()
      }),
    })
    readonly show = vi.fn()
    ...
    async loadURL(url: string) {
      this.urls.push(url)
      if (url === 'qilin-app://app/index.html') navigated.resolve()
    }
...
function invoke(channel: string): unknown {
  ...
  return handler({ senderFrame: { url: 'qilin-app://shell/startup.html' } })
}
...
beforeEach(() => {
  ...
  vi.useFakeTimers()
  harness.reset()
  vi.stubEnv('QILIN_DESKTOP_NODE_BINARY', 'test-node')
...
describe('desktop main startup', () => {
  ...
  it('withholds profile recovery after application resources fail to load', async () => {
    ...
    expect(html).toContain('qilin-recovery://restart')
    expect(html).not.toContain('qilin-recovery://reset')
    expect(html).not.toContain('qilin-recovery://plugins')
  })
  ...
  it('reloads a crashed startup renderer in the same window', async () => {
    ...
    await harness.errorPublished.promise
    expect(window.urls).toEqual(['qilin-app://shell/startup.html', 'qilin-app://shell/startup.html'])
  ...
  it.each(['plugins', 'reset'])('runs %s recovery from a document with a broken preload', async (action) => {
    ...
    const event = { preventDefault: vi.fn() }
    window.webContents.emit('will-navigate', event, `qilin-recovery://${action}/?`)
    ...
    expect(window.urls.at(-1)).toBe('qilin-app://app/index.html')
  })
  ...
  it('shows the loading window before profile preparation and starts one actual Host', async () => {
    ...
    expect(window.urls).toEqual(['qilin-app://shell/startup.html'])
    ...
    expect(harness.windows).toHaveLength(1)
    expect(window.urls).toEqual(['qilin-app://shell/startup.html', 'qilin-app://app/index.html'])
    expect(invoke(DESKTOP_IPC.backendStatus)).toEqual({ phase: 'ready' })
  })
  ...
  it('keeps startup errors and a successful retry in the same window', async () => {
    ...
    expect(harness.windows[0]!.urls).toEqual(['qilin-app://shell/startup.html'])
    const nextStarted = harness.nextHostStart()
    ...
    await retry
    expect(harness.windows).toHaveLength(1)
    expect(harness.windows[0]!.urls.at(-1)).toBe('qilin-app://app/index.html')
    expect(harness.dialog.showErrorBox).not.toHaveBeenCalled()
  })

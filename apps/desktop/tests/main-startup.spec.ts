...
const harness = await vi.hoisted(async () => {
  ...
  class FakeWindow extends EventEmitter {
    ...
    async loadURL(url: string) {
      ...
      if (url === 'qilin-app://app/') navigated.resolve()
    }
...
vi.mock('node:fs/promises', async (importOriginal) => {
  ...
  return { ...original, readFile: vi.fn((path: Parameters<typeof original.readFile>[0], encoding?: 'utf8') => {
    if (path === join('desktop-test-app', 'package.json')) {
      return Promise.resolve(JSON.stringify({ qilinDesktopAppId: 'com.deepseek.qilin', qilinMandatoryUpdatePolicy: harness.embeddedPolicy }))
    }
...
function invoke(channel: string, origin = channel === DESKTOP_IPC.boot ? 'app' : 'shell', ...args: unknown[]): unknown {
  ...
  return handler({ senderFrame: { url: `qilin-app://${origin}/index.html` } }, ...args)
}
...
beforeEach(() => {
  ...
  vi.stubEnv('QILIN_DESKTOP_PNPM_ENTRY', 'test-pnpm')
  vi.stubEnv('QILIN_DESKTOP_QILIN_DIR', 'test-runtime')
  vi.stubGlobal('process', { ...process, platform: 'win32', resourcesPath: 'desktop-test-resources' })
  vi.stubEnv('QILIN_DESKTOP_HOST_INSPECT_PORT', undefined)
  vi.stubEnv('QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG', undefined)
  vi.stubEnv('QILIN_DESKTOP_UPDATE_JOURNAL_DIR', undefined)
})
...
describe('desktop main startup', () => {
  ...
  it('persists opt-in update evidence from the real main entry without private diagnostics', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'qilin-main-update-journal-'))
    try {
      vi.stubEnv('QILIN_DESKTOP_UPDATE_JOURNAL_DIR', directory)
      await readyForUpdate()
  ...
  it.each(['darwin', 'win32', 'linux'] as const)('limits native titlebar styling to macOS on %s', async (platform) => {
    ...
    expect(window.urls).toEqual(['qilin-app://app/'])
    if (platform === 'darwin') {
  ...
  it('follows the Windows primary document language and palette without trusting other frames', async () => {
    ...
    listener({ ...event, senderFrame: { url: 'qilin-app://app/' } }, 'zh-CN', '#ffffff', '#000000')
    ...
    window.setTitleBarOverlay.mockClear()
    window.webContents.mainFrame.url = 'qilin-app://unowned/index.html'
  ...
  it('attaches Host socket credentials only to the owned application origin and window', async () => {
    ...
    const details = { url: 'ws://127.0.0.1:3080/api/remote.mux', webContentsId: 42, requestHeaders: { Origin: 'qilin-app://app' } }
  ...
  it('holds boot injections until the Host is ready and rejects foreign boot callers', async () => {
    ...
    let settled = false
    const boot = Promise.resolve(handler({ senderFrame: { url: 'qilin-app://app/' } })).then((value) => { settled = true; return value })
    await Promise.resolve()
    expect(settled).toBe(false)
    expect(harness.windows[0]!.urls).toEqual(['qilin-app://app/'])
    harness.prepared.resolve()
    await harness.hostStarted.promise
    expect(settled).toBe(false)
    harness.hosts[0]!.ready.resolve()
  ...
  it('opens message links externally while retaining same-origin application navigation', async () => {
    ...
    window.webContents.emit('will-navigate', internal, 'qilin-app://app/session/task-1')
  ...
  it('shares the failed-check deadline across focus and resume, while explicit checks reset polling', async () => {
    vi.stubEnv('QILIN_DESKTOP_UPDATE_CHECK_INTERVAL_MS', '1000')
    vi.stubEnv('QILIN_DESKTOP_UPDATE_CHECK_MAX_BACKOFF_MS', '4000')
    vi.stubEnv('QILIN_DESKTOP_UPDATE_CHECK_JITTER', '0')
  ...
  it('blocks subsequent product operations without stopping the Host and clears only on a fresh no-force policy', async () => {
    harness.embeddedPolicy = { origin: 'https://policy.example.com',
      allowedPageOrigins: ['https://downloads.example.com'], intervalMs: 10_000, jitter: 0 }
    vi.stubEnv('QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG', '{invalid environment override}')
    ...
    const unowned = [
      ...
      { ...owned, senderFrame: { url: 'qilin-app://app/index.html' } },
    ...
    expect(host.stop).not.toHaveBeenCalled()
    expect(request.mock.calls[0]![1]!.headers).toMatchObject({ 'x-client-bundle-id': 'com.deepseek.qilin', 'x-client-version': '1.0.0' })
  })
  ...
  it.each([false, true])('restores a cleanly stopped Host after installer failure and retains mandatory blocking: %s', async (mandatory) => {
    ...
    await vi.waitFor(() => { expect(harness.windows[0]!.urls).toEqual(['qilin-app://app/', 'qilin-app://app/']) })
  ...
  it('ignores subresource failures and navigation cancellation but reports a failed main document', async () => {
    ...
    await harness.preparing.promise
    const window = harness.windows[0]!
    window.webContents.emit('did-fail-load', {}, -2, 'failed', 'qilin-app://app/image.png', false)
    window.webContents.emit('did-fail-load', {}, -3, 'aborted', 'qilin-app://app/', true)
    expect(harness.dialog.showMessageBox).not.toHaveBeenCalled()
    window.webContents.emit('did-fail-load', {}, -2, 'failed', 'qilin-app://app/', true)
  ...
  it('shows the loading window before profile preparation and starts one actual Host', async () => {
    ...
    expect(harness.hosts[0]).toMatchObject({
      ...
      runtime: join(harness.app.getAppPath(), 'qilin'),
  ...
  it('prepares an independent plugin profile for the unpackaged Host', async () => {
    ...
    vi.stubEnv('QILIN_DESKTOP_QILIN_DIR', undefined)
    await import('../src/main.ts')
    await harness.preparing.promise
    harness.prepared.resolve()
    await harness.hostStarted.promise
  ...
  it('keeps startup errors in the existing window without a retry handler', async () => {
    ...
    expect(harness.windows[0]!.urls).toEqual(['qilin-app://app/'])
    expect(harness.dialog.showMessageBox).toHaveBeenCalledOnce()
    expect(harness.handlers.has('qilin-desktop:backend-retry')).toBe(false)
    expect(harness.hosts).toHaveLength(1)
  })

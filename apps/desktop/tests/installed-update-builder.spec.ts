...
const environment = { QILIN_DESKTOP_AUTO_UPDATE_ENV: 'test', DOWNLOAD_TEST_ORIGIN: 'https://download-test.deepseek.com',
...
async function fixture<T>(body: (manifest: string, source: string) => Promise<T>): Promise<T> {
  const root = await mkdtemp(join(tmpdir(), 'qilin-update-builder-'))
  try {
    ...
    for (const version of versions) {
      ...
      const runtime = runtimeFixture(qilin, version)
      writeDesktopRuntime(qilin, runtime.release, runtime.sharedPackages.map(entry => entry.name), { platform: 'win32', arch: 'x64' })
    }
  ...
  it('keeps signed NSIS hooks and one isolated identity/feed while selecting each private runtime and output', async () => {
    await fixture(async (manifest, source) => {
      ...
      for (const [index, config] of [first!, second!].entries()) {
        ...
        const qilin = join(manifest, '..', versions[index]!, 'qilin')
        expect(config.files[2]).toMatchObject({ from: qilin, to: 'qilin' })
        expect(config.files[3]).toMatchObject({ from: join(qilin, 'node_modules'), to: 'qilin/node_modules' })
        ...
        expect(config.publish[0]!.url)
          .toMatch(/^https:\/\/download-test\.deepseek\.com\/qilin-desk\/feeds\/qualification\/[a-f0-9]{24}\/win-x64\/$/u)
      }
    })
  })

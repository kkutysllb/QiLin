...
describe('installer preparation preserves application dependencies', () => {
  it.each(['win32', 'darwin'] as const)('rejects a missing production policy before signing on %s', async (platform) => {
    ...
    expect(() => createElectronBuilderConfig({ QILIN_DESKTOP_APP_ID: 'com.example.installer',
      QILIN_DESKTOP_AUTO_UPDATE_ENV: 'production',
      QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: 'https://harness-test.deepseek.com',
    }, platform, 'x64')).toThrow('QILIN_DESKTOP_MANDATORY_UPDATE_PROD_ORIGIN')
  })
  it.each(['win32', 'darwin'] as const)('keeps electron-builder responsible for node_modules on %s', async (platform) => {
    execute.mockClear()
    const env = {
      QILIN_DESKTOP_APP_ID: 'com.example.installer',
      QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: 'https://policy.example.com',
      QILIN_DESKTOP_TARGET_PLATFORM: platform,
      QILIN_DESKTOP_TARGET_ARCH: 'x64',
      QILIN_DESKTOP_UNSIGNED: platform === 'win32' ? '1' : '0',

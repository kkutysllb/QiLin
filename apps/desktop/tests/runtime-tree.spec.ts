...
function fixture(): string {
  ...
  roots.push(root)
  runtimeFixture(join(root, 'qilin'))
  return root
}
...
it('verifies a runtime after relocation without depending on build paths', async () => {
  const root = fixture()
  const before = await verifyDesktopRuntime(join(root, 'qilin'), '1.0.0')
  cpSync(join(root, 'qilin'), join(root, 'moved'), { recursive: true })
...
it.each(['changed', 'same-size', 'extra', 'missing'])('checks %s runtime bytes only during build verification', async (operation) => {
  const qilin = join(fixture(), 'qilin')
  const before = readDesktopRuntime(qilin)
  if (operation === 'changed') writeFileSync(join(qilin, 'package.json'), '{}')
  if (operation === 'same-size') writeFileSync(join(qilin, 'package.json'), '{"type":"Module"}\n')
  if (operation === 'extra') writeFileSync(join(qilin, 'extra'), '')
  if (operation === 'missing') rmSync(join(qilin, 'package.json'))
  expect(readDesktopRuntime(qilin)).toEqual(before)
  await expect(verifyDesktopRuntime(qilin, '1.0.0')).rejects.toThrow(/integrity/u)
})
it('rejects filesystem links and incompatible targets', async () => {
  const qilin = join(fixture(), 'qilin')
  await expect(verifyDesktopRuntime(qilin, '1.0.0', { platform: process.platform, arch: 'wrong' })).rejects.toThrow(/incompatible/u)
  symlinkSync(join(qilin, 'node_modules'), join(qilin, 'outside'), process.platform === 'win32' ? 'junction' : 'dir')
  expect(readDesktopRuntime(qilin).release.version).toBe('1.0.0')
  await expect(verifyDesktopRuntime(qilin, '1.0.0')).rejects.toThrow(/unsupported filesystem/u)
})
it('reads file inventory records unchanged during startup', () => {
  const qilin = join(fixture(), 'qilin')
  const path = join(qilin, DESKTOP_RUNTIME_FILE)
  ...
  expect(readDesktopRuntime(qilin).files).toEqual(descriptor.files)
})
it.each(['missing', 'directory'])('checks a %s Host entry only during build verification', async (operation) => {
  const qilin = join(fixture(), 'qilin')
  const path = join(qilin, 'node_modules', DESKTOP_HOST_PACKAGE, DESKTOP_HOST_RUNTIME_FILES[0])
  rmSync(path)
...
it('checks the shell version only during build verification', async () => {
  const qilin = join(fixture(), 'qilin')
  expect(readDesktopRuntime(qilin).release.version).toBe('1.0.0')
  await expect(verifyDesktopRuntime(qilin, '2.0.0')).rejects.toThrow(/does not match Electron/u)
})
it.each([
  { schemaVersion: 2 },
  { platform: 'other' },
  ...
  const qilin = join(fixture(), 'qilin')
  const path = join(qilin, DESKTOP_RUNTIME_FILE)
  const original = readDesktopRuntime(qilin)
  const descriptor = { ...original, ...patch, release: { ...original.release, ...patch.release } }
  writeFileSync(path, JSON.stringify(descriptor))
  expect(readDesktopRuntime(qilin)).toEqual(descriptor)
  await expect(verifyDesktopRuntime(qilin, '1.0.0')).rejects.toThrow(/invalid|incompatible/u)
})
it.each(['missing', 'invalid-json', 'mismatched'])('checks %s shared manifests only during build verification', async (operation) => {
  const qilin = join(fixture(), 'qilin')
  const before = readDesktopRuntime(qilin)
  const path = join(qilin, 'node_modules', DESKTOP_HOST_PACKAGE, 'package.json')
  if (operation === 'missing') rmSync(path)
  else writeFileSync(path, operation === 'invalid-json' ? '{' : '{}')
  expect(readDesktopRuntime(qilin)).toEqual(before)
  await expect(verifyDesktopRuntime(qilin, '1.0.0')).rejects.toThrow()
})
it('rejects a descriptor that maps a shared package outside node_modules', async () => {
  ...
  writeFileSync(path, JSON.stringify(descriptor))
  await expect(verifyDesktopRuntime(qilin, '1.0.0')).rejects.toThrow(/shared package record/u)
})
it('verifies recorded executable permissions only on Unix', async () => {
  ...
  writeFileSync(path, JSON.stringify(descriptor))
  if (process.platform === 'win32') await expect(verifyDesktopRuntime(qilin, '1.0.0')).resolves.toMatchObject(descriptor)
  else await expect(verifyDesktopRuntime(qilin, '1.0.0')).rejects.toThrow(/integrity/u)
})

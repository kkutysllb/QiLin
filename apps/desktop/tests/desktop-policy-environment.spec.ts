...
const origins = { QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: 'https://harness-test.deepseek.com',
  QILIN_DESKTOP_MANDATORY_UPDATE_PROD_ORIGIN: 'https://harness.deepseek.com' }

it.each(['test', 'production'] as const)('selects the %s policy and authentication together', (deployment) => {
  const policy = resolveDesktopPolicyEnvironment({ ...origins, QILIN_DESKTOP_AUTO_UPDATE_ENV: deployment })
  const origin = deployment === 'test' ? origins.QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN : origins.QILIN_DESKTOP_MANDATORY_UPDATE_PROD_ORIGIN
...
it('requires only the selected origin, defaults to test, and accepts explicit page restrictions', () => {
  const policy = resolveDesktopPolicyEnvironment({
    QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: origins.QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN,
    QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG: JSON.stringify({ allowedPageOrigins: ['https://download.deepseek.com'], intervalMs: 5000 }) })
  expect(policy).toMatchObject({ authentication: 'feishu-test', intervalMs: 5000, allowedPageOrigins: ['https://download.deepseek.com'] })
  expect(() => resolveDesktopPolicyEnvironment({ ...origins, QILIN_DESKTOP_AUTO_UPDATE_ENV: 'prod' })).toThrow('production')
})
...
it.each([undefined, '', 'http://harness-test.deepseek.com', 'https://user:secret@harness-test.deepseek.com',
  ...
  expect(() => resolveDesktopPolicyEnvironment({ QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: origin })).toThrow('HTTPS origin')
  expect(() => resolveDesktopPolicyEnvironment({ QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: origin })).not.toThrow('secret=value')
})
...
it.each(['{', 'null', '[]', '{"origin":"https://old.example.com"}', '{"authentication":"anonymous"}',
  ...
  expect(() => resolveDesktopPolicyEnvironment({ ...origins, QILIN_DESKTOP_MANDATORY_UPDATE_CONFIG: options })).toThrow()
})

it.each([{ unsigned: true }, { prepareOnly: true }, {}])('fails before signing/preparation when policy is absent in %j', (options) => {
  for (const platform of ['win32', 'darwin'] as const) {
    expect(() => { validateDesktopPackageEnvironment({ QILIN_DESKTOP_APP_ID: 'com.example.test' }, { platform, arch: 'x64' }, options) })
      .toThrow('QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN')
  }
})

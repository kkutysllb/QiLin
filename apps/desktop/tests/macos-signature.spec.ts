...
const RELEASE_ENVIRONMENT = {
  ...
  QILIN_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: 'https://policy.example.com',
...
describe('desktop macOS release signature', () => {
  ...
  it('loads release identifiers from the environment and requires code signing', async () => {
    ...
    const [qilinFiles, qilinNodeModules] = config.files.slice(-2)
    if (!qilinFiles || !qilinNodeModules || typeof qilinFiles === 'string' || typeof qilinNodeModules === 'string') {
      throw new Error('desktop QILIN resources must use electron-builder file mappings')
    }
    expect(portablePath(qilinFiles.from)).toContain('/.desktop-build/targets/mac-arm64/qilin')
    expect(qilinFiles.to).toBe('qilin')
    expect(portablePath(qilinNodeModules.from)).toContain('/.desktop-build/targets/mac-arm64/qilin/node_modules')
    expect(qilinNodeModules.to).toBe('qilin/node_modules')
    ...
    expect(config).toMatchObject({
      ...
      publish: [{
        provider: 'generic',
        url: 'https://desktop-updates.example.com/qilin-desk/feeds/mac-arm64/',
        channel: 'nightly',
      }],
    })
  ...
  it('rejects unsigned macOS builds and malformed signing modes', async () => {
    const { createElectronBuilderConfig } = await import('../electron-builder.config.mjs')
    expect(() => createElectronBuilderConfig({ ...RELEASE_ENVIRONMENT, QILIN_DESKTOP_UNSIGNED: '1' }))
      .toThrow(/unsigned builds require Windows/u)
    expect(() => createElectronBuilderConfig({ ...RELEASE_ENVIRONMENT, QILIN_DESKTOP_UNSIGNED: 'yes' }))

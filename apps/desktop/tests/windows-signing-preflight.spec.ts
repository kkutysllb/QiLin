...
async function fixture() {
  ...
  const environment = { SystemRoot: root,
    QILIN_DESKTOP_WINDOWS_CER_FILE: join(root, 'public.cer'),
    QILIN_DESKTOP_WINDOWS_SIGNTOOL: join(root, 'signtool.exe'),
    QILIN_DESKTOP_WINDOWS_KEY_CONTAINER: 'fixture-container', QILIN_DESKTOP_WINDOWS_TOKEN_PIN: 'fixture-pin' }
  await writeFile(environment.QILIN_DESKTOP_WINDOWS_CER_FILE, 'inert certificate')
  await writeFile(environment.QILIN_DESKTOP_WINDOWS_SIGNTOOL, 'inert signer')
...
('rejects static %s failure without compiling or signing', async (failure) => {
  ...
  else if (failure === 'certificate') await rm(options.environment.QILIN_DESKTOP_WINDOWS_CER_FILE)
  else if (failure === 'signTool') await rm(options.environment.QILIN_DESKTOP_WINDOWS_SIGNTOOL)
  else if (failure === 'pin') options.environment.QILIN_DESKTOP_WINDOWS_TOKEN_PIN = ''
  else {

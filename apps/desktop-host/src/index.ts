...
import { loadLayeredEnv, loadProfileDirectory } from '@qilin/app-boot'
import { runProfile } from '@qilin/cli/profile-boot'
...
async function main(): Promise<void> {
  ...
  const installAnchor = join(runtimeDir, 'node_modules', '@deepseek-ai', 'qilin', 'package.json')
  const profile = loadProfileDirectory('qilin', projectDir, installAnchor)
    ...
    ...(process.argv[6] === undefined ? {} : {
      packageManager: {
        ...
        env: {
          ...
          QILIN_DESKTOP_NODE_EXECUTABLE: process.execPath,
  ...
  await ctx.plugin(desktopOffice, {
    ...
    root: join(resolveQilinHome(), 'qilin-runtimes', 'qilin-primary-runtime'),
  })

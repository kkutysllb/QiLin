...
const DEPENDENCY_ANALYSIS_PLUGIN = 'qilin-browser-dependency-analysis'
...
export function productWebBundleIsolation(repository: string, webRoot: string): Plugin[] {
  ...
  return [{
    name: 'qilin-product-web-chunk-inputs',
    apply: 'build',
    generateBundle: {
      order: 'pre',
    ...
    name: 'qilin-product-web-bundle-isolation',
    apply: 'build',
    enforce: 'post',
    config(config) {
      ...
      return {
        ...
        worker: {
          plugins: () => [...workerPlugins?.() ?? [], {
            name: 'qilin-worker-build-inputs',
            generateBundle: {
              order: 'post',

...
export function realOfficeBytes(extension: 'doc' | 'docx' | 'xls' | 'xlsx' | 'ppt' | 'pptx', font = 'Liberation Sans'): Uint8Array {
  ...
  switch (extension) {
    ...
    case 'xlsx':
      ...
      parts = {
        [main]: 'application/vnd.openxmlformats-officedocument.spreaqilineetml.sheet.main+xml',
        'xl/worksheets/sheet1.xml': 'application/vnd.openxmlformats-officedocument.spreaqilineetml.worksheet+xml',
      }
      files[main] = `<workbook xmlns="http://schemas.openxmlformats.org/spreaqilineetml/2006/main" xmlns:r="${OFFICE_REL_NS}"><sheets><sheet name="Preview" sheetId="1" r:id="rId1"/></sheets></workbook>`
      ...
      files['xl/worksheets/sheet1.xml'] = `<worksheet xmlns="http://schemas.openxmlformats.org/spreaqilineetml/2006/main"><cols><col min="1" max="1" width="45" customWidth="1"/></cols><sheetData><row r="1"><c r="A1" t="inlineStr"><is><t>${TEXT}</t></is></c></row></sheetData></worksheet>`
      break
    case 'pptx':

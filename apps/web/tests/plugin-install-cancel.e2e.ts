...
it('cancels installation through the UI, restores files, and offers the spec again', async () => {
  const scratch = await mkdtemp(join(tmpdir(), 'qilin-install-cancel-'))
  ...
  try {
    ...
    try {
      ...
      await writeFile(join(profile, 'view'), 'console.log(JSON.stringify({ name: "slow-package", version: "1.0.0", qilin: { bundle: { patch: "./cordis.patch.yml" } } }))\n')
      ...
      await writeFile(join(profile, 'add'), `
        ...
        fs.writeFileSync('node_modules/slow-package/package.json', JSON.stringify({ name: 'slow-package', version: '1.0.0', qilin: { bundle: { patch: './cordis.patch.yml' } } }));

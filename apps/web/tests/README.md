...
the deliberate composition divergences from `qilin web` — are documented in
...
`@qilin/goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@qilin/client-web` and the boot-manifest
type from `@qilin/client-modules/client`: booting the real shell is what

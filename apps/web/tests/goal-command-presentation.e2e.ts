...
describe('web e2e: /goal human transcript presentation', () => {
  ...
  it('completes with Tab and shows the bare input and result without a model turn', async () => {
    ...
    // The empty-state hero is up before the command: the conversation root
    // reports the blank-draft phase, whatever copy the locale gives it.
    await expect.poll(() => page.locator('div[data-phase="hero"]').count(), {
      timeout: 15_000,
    }).toBe(1)

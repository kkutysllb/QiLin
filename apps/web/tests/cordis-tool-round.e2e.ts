...
describe.skipIf(MODE === 'record')('web e2e: historical Kylin cards', () => {
  ...
  it('renders recorded definition source and lifecycle results without registering retired tools', async () => {
    ...
    expect(names).not.toEqual(expect.arrayContaining(['kylin_define']))
    const define = page.locator('[data-tool="kylin_define"]').first()
    ...
    const stop = page.locator('[data-tool="kylin_stop"]').first()

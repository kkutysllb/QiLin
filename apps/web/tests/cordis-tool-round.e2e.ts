// Web e2e scenario for the opt-in Kylin tools. Record mode drives a real
// model through inspect, define, run, and stop; replay pins the same shipped Web
// composition, durable calls, Kylin-owned rows, the define card's own source view,
...
const UI_EXPECTED = fileURLToPath(new URL('../../../snapshots/web/kylin-tool-round/ui.expected.md', import.meta.url))
const MODE = webSnapshotMode()
const CORDIS_TOOLS = ['kylin_inspect_self', 'kylin_define', 'kylin_run', 'kylin_stop'] as const
...
const PROMPT = 'Use only Kylin tools. First call kylin_inspect_self with no arguments. '
  + 'Then call kylin_define with plugin kind "new", idPrefix "snap", name "snapshot noop", '
  ...
  + 'Read its returned pluginId and packageId, then call kylin_run with those exact IDs and mode "run". '
  + 'After the run request returns, reply exactly CORDIS_UI_READY and stop.'
const STOP_PROMPT = 'Use only Kylin tools. Call kylin_stop with pluginId "snap-1". '
  + 'After it succeeds, reply exactly CORDIS_UI_DONE and stop.'

function assertCompleteKylinLifecycle(events: readonly SessionEvent[]): void {
...
describe('web e2e: Kylin tools use their owned cards', () => {
  ...
  it('drives the recorded Kylin lifecycle to a settled turn (all modes)', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-kylin-drive'))
    if (MODE !== 'record') {
    ...
    const approve = page.locator('[data-kylin-approve]').first()
    ...
    await expect.poll(() => {
      const stop = sessionEvents.find(
        (event): event is Extract<SessionEvent, { type: 'tool/call' }> =>
          event.type === 'tool/call' && event.data.name === 'kylin_stop',
      )
    ...
    if (MODE === 'record') {
      assertCompleteKylinLifecycle(sessionEvents)
  ...
  it.skipIf(MODE === 'record')('the durable log carries one complete Kylin lifecycle', () => {
    assertCompleteKylinLifecycle(sessionEvents)
  })

  it.skipIf(MODE === 'record')('renders localized Kylin lifecycle cards', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-kylin-rows'))
    await expect.poll(() => page.getByText('CORDIS_UI_DONE', { exact: true }).count(), { timeout: 15_000 })
      .toBeGreaterThanOrEqual(1)

    const inspectRow = page.locator('[data-tool="kylin_inspect_self"]').filter({ hasText: 'Inspect' }).first()
    await expandOwningTurnProcess(page, inspectRow)
    await inspectRow.waitFor({ timeout: 10_000 })

    // kylin_define does NOT go through the generic row: ui-kylin registers a
    ...
    const defineRow = page.locator('[data-tool="kylin_define"]').filter({ hasText: 'Kylin Plugin' }).first()
    ...
    const runRow = page.locator('[data-tool="kylin_run"]').filter({ hasText: 'Run Kylin Plugin' }).first()
    ...
    const stopRow = page.locator('[data-tool="kylin_stop"]').filter({ hasText: 'Stop Kylin Plugin' }).first()
  ...
  it.skipIf(MODE === 'record')('matches the conversation aria golden', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-kylin-aria'))

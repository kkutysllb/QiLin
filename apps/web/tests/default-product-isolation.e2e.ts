...
async function readClientRoster() {
  const observation = Reflect.get(globalThis, '__qilinIsolationObservation') as ClientObservation
...
function experimentalClientReferences(roster: Awaited<ReturnType<typeof readClientRoster>>): string[] {
  return [
    ...
    ...roster.modules,
  ].filter(name => name.startsWith('@qilin/experimental-'))
}
  ...
  await withDefaultWeb(test, async ({ url, request }) => {
    ...
    try {
      ...
      await page.addInitScript(() => {
        ...
        Reflect.set(globalThis, '__qilinIsolationObservation', observation)
        Object.defineProperty(globalThis, '__ModuleLoader__', {
          ...
          set(target: ClientModuleLoaderTarget) {
            ...
            target.create = function (options) {
              const cordis = options.staticModules['@qilin/kylin'] as { RegistryService: typeof RegistryService }
      ...
      expect(roster.plugins.some(plugin => plugin.modules.includes('@qilin/client-ui-layout'))).toBe(true)

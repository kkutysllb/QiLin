...
it('isolates replay skill discovery from every ambient host root', async () => {
  ...
  try {
    ...
    try {
      ...
      expect(names).not.toContain('ambient-qilin')
    ...
    try {
      ...
      if (originalQilinHome === undefined) delete process.env.QILIN_HOME
      else process.env.QILIN_HOME = originalQilinHome

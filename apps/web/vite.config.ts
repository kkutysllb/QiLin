...
const STANDALONE_ERROR = 'apps/web is not a standalone application: bare Vite cannot inject window.__QILIN_BOOT__. '
...
const DEFAULT_CLIENT_TITLE = 'QiLin Local Build'
...
function clientDocumentTitle(): Plugin {
  ...
  return {
    ...
    transformIndexHtml(html) {
      return html.replace('<title>QiLin Local Build</title>', `<title>${title}</title>`)
    },
  }
}

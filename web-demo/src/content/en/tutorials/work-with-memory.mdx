...
description: This tutorial shows you how to enable and use QiLin's memory system so the agent remembers important information about you across multiple sessions.
---
...
This tutorial shows you how to enable and use QiLin's memory system so the agent remembers important information about you across multiple sessions.
...
Memory is stored in `backend/.qilin/memory.json`:

```bash
cat backend/.qilin/memory.json
```
...
```
backend/.qilin/agents/{agent_name}/memory.json
```
...
- [Deploy Your Own QiLin](/docs/tutorials/deploy-your-own-kworks)

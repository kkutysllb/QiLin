...
QiLin ships with the following public skills:
...
`parser.py` reads each `SKILL.md` file and extracts structured metadata: name, description, `allowed-tools`, and any tool or resource requirements.
...
The `.skill` installer safely extracts the ZIP archive, validates `SKILL.md`, and scans `SKILL.md`, `references/`, `templates/`, and `scripts/` files that can enter prompts or execution.
...
- **Through the QiLin App UI**: the skills panel lets you toggle skills on and off.
...
QiLin includes an optional **skill evolution** feature that allows the agent to autonomously create and improve skills in the `skills/custom/` directory:

...
QiLin ships with three built-in subagents:
...
A read-only quality-assurance agent that validates the correctness, completeness, and risk profile of other agents' output. Use it as a final verification step before synthesizing multi-agent results.

- **Default timeout**: 900 seconds (15 minutes)
- **Default max turns**: 50
- **Tools**: read-only (`read_file`, `ls`, `glob`, `grep`, `bash`, `web_search`, `web_fetch`) — cannot modify files
- **Role in multi-agent flow**: completes the execute → review → synthesize loop
...
In addition to the built-in subagents, QiLin supports delegating to external agents through the **Agent Connect Protocol (ACP)**. ACP allows QiLin to invoke agents running as separate processes (including third-party CLI tools wrapped with an ACP adapter).
...
<Callout type="tip">
  ACP agents run as child processes managed by QiLin. They communicate over
...
Custom agents created through the QiLin App UI can also be invoked as subagents using the `task` tool. When you specify `agent="my-custom-agent"`, the runtime loads that agent's configuration (skills, tool groups, model) and runs it as a subagent for the delegated task.

---
title: Memory
description: Memory is a runtime feature of the Qilin Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
---
...
<Callout type="info" emoji="💾">
  Memory lets QiLin carry useful information across sessions. The agent
...
Memory is a runtime feature of the Qilin Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
...
memory:
  ...
  # Default: {base_dir}/memory.json  (resolves to backend/.qilin/memory.json)
  ...
  storage_class: qilin.agents.memory.storage.FileMemoryStorage
...
QiLin supports two levels of memory:
...
- Base directory: `backend/.qilin/`
- Global memory: `backend/.qilin/memory.json`
- Per-agent memory: `backend/.qilin/agents/{agent_name}/memory.json`

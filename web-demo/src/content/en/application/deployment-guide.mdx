...
description: "This guide covers all supported deployment methods for QiLin App: local development, Docker Compose, and production with Kubernetes-managed sandboxes."
---
...
This guide covers all supported deployment methods for QiLin App: local development, Docker Compose, and production with Kubernetes-managed sandboxes.

## Local development deployment

The local workflow is the fastest way to run QiLin. All services run as native processes on your machine.
...
```bash
# Set the absolute path to your QiLin repo root
export QILIN_ROOT=/path/to/QiLin
...
# .env
OPENAI_API_KEY=sk-...
QILIN_ROOT=/absolute/path/to/QiLin
...
Thread data is stored in `backend/.qilin/threads/`. In Docker deployments, this directory is bind-mounted into the langgraph container.

To avoid data loss when containers are recreated:

1. Set `QILIN_ROOT` to the absolute repo root path (or a stable host path).
...
USERDATA_PVC_NAME=kworks-userdata-pvc
SKILLS_PVC_NAME=kworks-skills-pvc
```
...
QiLin App uses [Better Auth](https://www.better-auth.com/) for session management. In production:

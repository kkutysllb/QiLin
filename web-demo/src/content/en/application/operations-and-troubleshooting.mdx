...
description: This page covers day-to-day operational tasks and solutions to common problems when running QiLin App.
---
...
This page covers day-to-day operational tasks and solutions to common problems when running QiLin App.
...
| File                 | Service                              |
| -------------------- | ------------------------------------ |
| `logs/langgraph.log` | LangGraph / Qilin Harness runtime |
...
When you pull a new version of QiLin, the `config_version` in `config.example.yaml` may be higher than your `config.yaml`. To merge new fields without losing your customizations:
...
- Check that the storage path is writable: `ls -la backend/.qilin/`.
- Look for memory update errors in `logs/langgraph.log` (search for "memory").

## Data backup

Thread data and memory are stored under `backend/.qilin/`:
...
In Docker deployments, the bind-mounted host path (`$QILIN_ROOT/backend/.qilin/`) is the source of truth — back up the host path.

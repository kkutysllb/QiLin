...
`parser.py` 读取每个 `SKILL.md` 文件并提取结构化元数据：名称、描述、`allowed-tools` 以及任何工具或资源需求。
...
`.skill` 安装器会安全解压 ZIP 包、验证 `SKILL.md`，并扫描 `SKILL.md`、`references/`、`templates/` 和 `scripts/` 中会进入提示词或执行环境的文件。
...
- **通过 QiLin 应用界面**：技能面板允许你切换技能的启用/禁用状态。
...
QiLin 包含一个可选的**技能进化**功能，允许 Agent 在 `skills/custom/` 目录中自主创建和改进技能：

...
只读质量审查 Agent，校验其他 Agent 产出的正确性、完整性和风险。在多 Agent 工作流中作为最终验证环节，完成「执行 → 审查 → 综合」闭环。

- **默认超时**：900 秒（15 分钟）
- **默认最大轮次**：50
- **工具**：只读权限（`read_file`、`ls`、`glob`、`grep`、`bash`、`web_search`、`web_fetch`）——不可修改文件
- **多 Agent 流中的角色**：补全执行 → 审查 → 综合的闭环
...
除内置子 Agent 外，QiLin 还通过 **Agent Connect Protocol (ACP)** 支持委派给外部 Agent。ACP 允许 QiLin 调用作为独立进程运行的 Agent（包括用 ACP 适配器包装的第三方 CLI 工具）。
...
<Callout type="tip">
  ACP Agent 作为 QiLin 管理的子进程运行，通过 ACP 协议通信。标准 CLI

---
title: 运维与排障
description: 本页面涵盖运行 QiLin 应用的操作信息：日志记录、常见问题和维护任务。
---
...
本页面涵盖运行 QiLin 应用的操作信息：日志记录、常见问题和维护任务。

## 日志

QiLin 应用在 `logs/` 目录中写入每个服务的日志：
...
```bash
# 检查线程用户数据目录是否存在且可写
ls -la backend/.qilin/threads/
```

**解决**：

1. 确保 `backend/.qilin/` 对运行 QiLin 的进程可写。
2. 在 Docker 部署中，验证卷挂载路径正确（`QILIN_ROOT` 设置为绝对路径）。
...
- **线程数据**：`backend/.qilin/threads/` — 每个线程的上传文件、输出和工作区文件
- **检查点**：取决于检查点器配置（SQLite：`backend/.qilin/checkpoints.db`，Redis：外部存储）
- **记忆**：`backend/.qilin/memory.json`（以及 `agents/*/memory.json`）

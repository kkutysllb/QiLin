...
// from inside Streamdown's lexing useMemo; on larger stacks the same input
// instead goes quadratic and exhausts the heap. Each list level requires at
// least ~2 columns of indentation, so capping leading whitespace at 200 columns
// bounds the effective nesting near 100 levels — far beyond any legitimate
// content while keeping marked safe. Anything indented past this is pathological
// nesting, not prose or code.
 ...
 *
 * Backend-injected markers like ``<memory>…</memory>`` can occasionally reach
 * the UI renderer (e.g. when a ``hide_from_ui`` reminder leaks through the
 * filter). React's DOM renderer logs "unrecognized tag" console errors for
 * unknown HTML elements. This function strips the tag markers while preserving
 * the inner content.
 *
 * Code-aware: tags inside fenced code blocks and indented code blocks are left
 * untouched, so user-written meta-discussions about the memory system are not
 * silently stripped.
 */
  ...
  return lines
    .map((line) => {
      ...
      if (fenceMatch) {
        ...
        // Otherwise: different fence type or shorter run inside a fence — stay inside.
        return line;
      }

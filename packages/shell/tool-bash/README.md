...
`qilin-tool-bash` lets an agent run one-shot `bash` commands and receive stdout, stderr, and exit markers. Each call uses a fresh shell, so cwd, variables, and functions do not persist; `run_in_background` starts long-running work that the agent can inspect with `job_output` and stop with `job_kill`. Commands receive the managed `QILIN_*` environment, and sandbox denials can be retried once with wider `sandbox_permissions`, a `justification`, and user approval. Non-zero exits are reported as results, so the agent decides how to respond; use an executor such as `qilin-bash-local` or `qilin-bash-sandbox` and load `qilin-shell-env`.
...
Load this plugin in any composition where the agent should run bash commands: it registers the `bash` tool once an executor provider and the `qilin-shell-env` registry are mounted, and stays pending until the `tools`, `shell`, `systemPrompt`, and `shellEnv` services exist.
...
The generated [configuration catalog](../../../docs/config-catalog.md#deepseek-aiqilin-tool-bash) is the exhaustive source for every accepted field and its JSDoc; the generated [tool catalog](../../../docs/tool-catalog.md#deepseek-aiqilin-tool-bash) carries the full argument schema.
...
- **Request from named args only.** The tool never exposes `stdin`, `env`, or `stdoutMaxBytes`; it builds each request from command/workdir/timeout/signal fields plus the registry-collected `qilinEnv`, so model-supplied keys cannot replace managed values.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#deepseek-aiqilin-tool-bash) — the exact `bash` argument schema.
- [Generated configuration catalog](../../../docs/config-catalog.md#deepseek-aiqilin-tool-bash) — every accepted config field and its source declaration.
...
The model sees the generated [`bash` schema](../../../docs/tool-catalog.md#deepseek-aiqilin-tool-bash). `run_in_background` appears only when this producer enables it; `sandbox_permissions` and `justification` appear only when the mounted executor advertises sandboxing. Agent-scoped tool restrictions can remove the definition for that agent.

...
`qilin-tool-pwsh-persistent` 为每个 agent（智能体）提供 `pwsh` 工具，跨调用保留其当前目录、环境变量、函数与后台任务。同一 agent 的命令串行运行，不同 agent 维护相互隔离的 shell 状态。多步 PowerShell 工作应选择本包；若每条命令都应从干净状态开始，请使用 `qilin-tool-pwsh`，需要交互 stdin 时则使用 terminal 工具。请配置支持 pwsh 的后端和单条命令超时；超时或显式 `exit` 会丢弃 shell，因此下次调用从全新状态开始。
...
生成的[配置目录](../../../docs/config-catalog.zh.md#deepseek-aiqilin-tool-pwsh-persistent)是每个受支持字段及其 JSDoc 的穷尽式真源。
...
- **`qilin-tool-bash-persistent` 的刻意孪生。** 会话注册表、轮询循环与重置约定按设计镜像持久 bash 工具（[pwsh 持久 PTY Agent Note](../../../.agents/notes/archived/architecture/2026-08-11-pwsh-persistent-pty.md)）。
...
- [生成的工具目录](../../../docs/tool-catalog.zh.md#deepseek-aiqilin-tool-pwsh-persistent)——`pwsh` 参数 schema 的确切内容。
- [生成的配置目录](../../../docs/config-catalog.zh.md#deepseek-aiqilin-tool-pwsh-persistent)——每个受支持配置字段及其源声明。
...
生成的 [`pwsh` schema](../../../docs/tool-catalog.zh.md#deepseek-aiqilin-tool-pwsh-persistent)，包括配置的 `description`。本插件不贡献独立的系统提示词区段；人设与环境指引由部署方负责。

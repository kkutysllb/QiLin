---
description: "Automation-only ACP (Agent Client Protocol) server for programmatic clients and maintainers driving QiLin agents over JSON-RPC stdio."
...
The generated [configuration catalog](../../../docs/config-catalog.md#deepseek-aiqilin-acp) is the exhaustive source for every accepted field and its JSDoc.
...
- **Standard semantic updates only.** The wire carries committed messages and thoughts, generic tool lifecycle, configuration, and context usage; raw provider deltas, retry attempts, QILIN presentation data, and unsupported content stay off the wire.
...
- **MCP tools only** — MCP resources and prompts have no QILIN consumer.

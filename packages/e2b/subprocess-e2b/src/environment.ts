...
import { e2bControlEnvs } from '@qilin/e2b'
import type { Sandbox } from '@qilin/e2b'
import { SENSITIVE_ENV_PATTERN } from '@qilin/subprocess'
...
export async function readRemoteEnvironment(sandbox: Sandbox, signal?: AbortSignal): Promise<string> {
  ...
  const result = await sandbox.commands.run(
    'set -o pipefail; qilin_e2b_passwd="$(getent passwd "$(id -u)")"; IFS=: read -r _ _ _ _ _ qilin_e2b_home _ <<<"$qilin_e2b_passwd"; test -n "$qilin_e2b_home" -a -d "$qilin_e2b_home"; printf \'%s\' "$qilin_e2b_home" | base64 -w 0; printf \'\\n\'; env -0 | base64 -w 0',

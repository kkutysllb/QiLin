...
async function harness(): Promise<{ ctx: Context; agent: Agent }> {
  ...
  await writeFile(join(workdir, 'AGENTS.md'), `If the user asks for the workspace context hanqilinake, reply with exactly this string and nothing else: ${PROBE}.\n`)
  ctx = new Context()
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('workspace context e2e: real model sees AGENTS.md baseline', () => {
  it('obeys a probe instruction loaded from the workspace', async () => {
    ...
    live.agent.followup(createUserMessage({ content: [{ type: 'text', text: 'Workspace context hanqilinake?' }], source: { kind: 'user' } }))
  ...
  it('loads a nested AGENTS.md after the real read tool touches a descendant file', async () => {
    ...
    await writeFile(join(workdir!, 'pkg/AGENTS.md'), `If the user asks for the nested instruction hanqilinake, reply with exactly this string and nothing else: ${NESTED_PROBE}.\n`)
    await writeFile(join(workdir!, 'pkg/deep/file.txt'), 'This file exists only to trigger nested workspace instructions.\n')

    live.agent.followup(createUserMessage({ content: [{ type: 'text', text: 'Use the read tool to inspect pkg/deep/file.txt. After reading it, answer: nested instruction hanqilinake?' }], source: { kind: 'user' } }))
    await waitForIdle(live.ctx, live.agent)
  ...
  it('appends changed baseline instructions after a real file-tool touch without rewriting the frozen prefix', async () => {
    ...
    live.agent.followup(createUserMessage({ content: [{ type: 'text', text: 'Workspace context hanqilinake?' }], source: { kind: 'user' } }))
    await waitForIdle(live.ctx, live.agent)
    await writeFile(join(workdir!, 'AGENTS.md'), `The old workspace hanqilinake no longer applies. If the user asks for the updated workspace context hanqilinake, reply with exactly this string and nothing else: ${UPDATED_PROBE}.\n`)

    live.agent.followup(createUserMessage({ content: [{ type: 'text', text: 'You must use the read tool to inspect trigger.txt. After reading it, answer: updated workspace context hanqilinake?' }], source: { kind: 'user' } }))
    await waitForIdle(live.ctx, live.agent)

...
async function discoverInstructionFiles(
  ...
  const userGlobal = join(config.qilinHome, USER_GLOBAL_FILE)
  ...
  switch (userGlobalProbe.kind) {
    case 'present':
      addFile({
        ...
        displayPath: userGlobalDisplayPath(config.qilinHome),
...
export async function probeScopeInstruction(
  ...
  const file: ProbedInstructionFile = {
    absolutePath,
    displayPath: directory === USER_GLOBAL_DIRECTORY ? userGlobalDisplayPath(resolved.qilinHome) : relativeDisplay(projectRoot, absolutePath),
    target,
...
function userGlobalDisplayPath(qilinHome: string): string {
  return `${qilinHomeDisplay(qilinHome)}/AGENTS.md`
}

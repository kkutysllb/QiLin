...
vi.mock('@qilin/atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@qilin/atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@qilin/atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }

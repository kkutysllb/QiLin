...
const KEY = credentialRef('QILIN_CRED_TEST')
const OTHER = credentialRef('QILIN_CRED_OTHER')
...
describe('resolveSpec', () => {
  it('defaults to .credentials.yaml under the harness home with watching on', () => {
    const spec = resolveSpec({ qilinHome: '/custom/home' })
  ...
  it('lets an explicit path win over the home', () => {
    const spec = resolveSpec({ path: '/etc/qilin/creds.yaml', qilinHome: '/ignored', watch: false, debounceMs: 5 })
    expect(spec).toEqual({ filename: resolve('/etc/qilin/creds.yaml'), watch: false, debounceMs: 5 })
  })
})
...
describe('layering and reads', () => {
  ...
  it('serves file entries alongside comments and quoted values', async () => {
    ...
    await writeCredentials(path, 'version: 1\nrefs:\n  # notes\n  QILIN_CRED_TEST: plain\n  QILIN_CRED_OTHER: "with space"\n')
  ...
  it('lets a non-empty process environment win read-only over the file', async () => {
    const dir = await tempDir()
    const path = join(dir, '.credentials.yaml')
    await writeCredentials(path, 'version: 1\nrefs:\n  QILIN_CRED_TEST: from-file\n')
    const ctx = await boot({ path, watch: false })
    vi.stubEnv('QILIN_CRED_TEST', 'from-env')
...
describe('layer ladder', () => {
  // inherited process env > .credentials.yaml > $QILIN_HOME/.env, and the
  ...
  async function bootLayered(
    ...
    const ctx = new Context()
    ctx.provide(QILIN_LAUNCH_ENVIRONMENT_KEY, createLaunchEnvironmentSnapshot(layers))
...
describe('document validation', () => {
  ...
  it.each([
    ...
    ['a sequence root', '- QILIN_CRED_TEST\n', /must be a mapping/],
    ...
    ['the flat layout with a non-string value', 'QILIN_CRED_TEST: [nope]\n', /nest the existing 1 entry under/],
    ['the flat layout with several entries and a non-string value', 'QILIN_CRED_TEST: a\nQILIN_CRED_OTHER: [b]\n',
      /nest the existing 2 entries under/],
    ['the flat layout with an empty value', 'QILIN_CRED_TEST: ""\n', /pre-release flat layout/],
    ...
    ['malformed yaml', 'QILIN_CRED_TEST: "unterminated\n', /invalid document/],
  ...
  it('never puts a credential value in a diagnostic', async () => {
    ...
    await writeCredentials(path, `QILIN_CRED_TEST: "${secret}\n`)
    let failure: unknown
    try {
...
describe('document writes', () => {
  ...
  it('patches one entry, preserving comments and every untouched entry', async () => {
    const dir = await tempDir()
    const path = join(dir, '.credentials.yaml')
    await writeCredentials(path, 'version: 1\nrefs:\n  # deployment notes\n  QILIN_CRED_OTHER: keep\n\n  # the one under edit\n  QILIN_CRED_TEST: old\n')
    const ctx = await boot({ path, watch: false })
    await ctx.credentials.set(KEY, 'new value!')
    expect(await readFile(path, 'utf8')).toBe(
      'version: 1\nrefs:\n  # deployment notes\n  QILIN_CRED_OTHER: keep\n\n'
      + '  # the one under edit\n  QILIN_CRED_TEST: new value!\n',
    )
  })
  ...
  it('unsets only the owning entry, with its own annotation, and keeps an absent unset silent', async () => {
    ...
    await writeCredentials(path, 'version: 1\nrefs:\n  # about the doomed one\n  QILIN_CRED_TEST: gone\n  # about the survivor\n  QILIN_CRED_OTHER: stays\n')
    const ctx = await boot({ path, watch: false })
    const seen = updates(ctx)
  ...
  it('rejects empty values and writes the environment would shadow', async () => {
    ...
    vi.stubEnv('QILIN_CRED_TEST', 'shadowing')
  ...
  it('fails a write loud when the on-disk document became invalid', async () => {
    ...
    await writeCredentials(path, 'QILIN_CRED_TEST: "unterminated\n')
...
describe('real hot reload', () => {
  it('publishes external edits, replaces the snapshot wholesale, and suppresses self-writes', async () => {
    ...
    await writeCredentials(path, 'version: 1\nrefs:\n  QILIN_CRED_TEST: boot\n')
    const ctx = await boot({ path, debounceMs: 10 })
    const seen = updates(ctx)

    await writeCredentials(path, 'version: 1\nrefs:\n  QILIN_CRED_TEST: live\n  QILIN_CRED_OTHER: extra\n')

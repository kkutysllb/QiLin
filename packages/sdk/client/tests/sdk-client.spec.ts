...
describe('DeepSeekHarness', () => {
  ...
  it('sends the configured cwd/provider/model/reasoningEffort/maxTokens in the hanqilinake exactly once', async () => {
  ...
  it('resolves a relative launch cwd to an absolute workspace before the hanqilinake', async () => {
    ...
    // ...and the hanqilinake wire cwd went out ABSOLUTE, so the child cannot
  ...
  it('propagates a JSON-RPC error response from initialize and closes the runtime', async () => {
    ...
    // The failed hanqilinake reset lets a later start retry instead of wedging.
  ...
  it('does not replace the client after terminal close wins a failed hanqilinake', async () => {
  ...
  it('retries a failed hanqilinake with a fresh runtime process', async () => {
    ...
    // First start: the scripted runtime fails the hanqilinake and is reaped.
    ...
    await harness.close()
    // close() is terminal: a hanqilinake failure after it must not respawn.

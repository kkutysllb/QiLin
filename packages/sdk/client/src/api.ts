...
/**
 * Reusable SDK for running QiLin agent turns in a runtime
...
export class DeepSeekHarness implements AsyncDisposable {
  ...
  constructor(options: DeepSeekHarnessOptions = {}, clientFactory?: () => HarnessClient) {
    ...
    // Absolute before the hanqilinake: the child spawns relative to THIS
   ...
   * hanqilinake swaps in a fresh instance only after cleanup proves the runtime
  ...
  /**
   * Start the subprocess and perform the `initialize` hanqilinake once. On
   ...
   * @returns settlement of the (memoized) hanqilinake.
   */
  start(): Promise<void> {
      ...
      try {
        ...
        try {
          ...
          throw new AggregateError(
            [error, cleanupError],
            'QiLin initialization and cleanup failed',
          )
        }
   ...
   * a closed harness no longer retries a failed hanqilinake.
...
export class HarnessSession {
  /**
   * @param harness - the owning harness (supplies the client and hanqilinake).

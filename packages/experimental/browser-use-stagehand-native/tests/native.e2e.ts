...
it.skipIf(process.env.QILIN_STAGEHAND_E2E !== '1')('launches and attaches to installed Chromium while preserving the external browser', { timeout: 120_000, retry: 0 }, async ({ signal }) => {
  const executable = process.env.QILIN_BROWSER_EXECUTABLE
  if (executable === undefined) throw new Error('QILIN_STAGEHAND_E2E requires QILIN_BROWSER_EXECUTABLE')
  ...
  const root = await mkdtemp(join(tmpdir(), 'qilin-stagehand-live-'))
  ...
  try {
    ...
    await ctx.plugin(LocalAttachmentStore, { qilinHome: root })
...
it.skipIf(process.env.QILIN_STAGEHAND_E2E !== '1' || !process.env.QILIN_STAGEHAND_MODEL || !process.env.QILIN_STAGEHAND_MODEL_API_KEY)('extracts a controlled heading using an independently configured native Stagehand model', { timeout: 120_000, retry: 0 }, async ({ signal }) => {
  const server = createServer((_request, response) => {
    response.setHeader('content-type', 'text/html')
  ...
  try {
    ...
    await mountAgentLoopTestDependencies(ctx)
    await ctx.plugin(BrowserUseRegistry)
    await ctx.plugin(Provider, { model: stagehandModelSchema.parse({ modelName: process.env.QILIN_STAGEHAND_MODEL, apiKey: process.env.QILIN_STAGEHAND_MODEL_API_KEY }), mode: 'launch', ...process.env.QILIN_BROWSER_EXECUTABLE === undefined ? {} : { executablePath: process.env.QILIN_BROWSER_EXECUTABLE } })
    const harness = await mountAgentLoopTestHarness(ctx)
...
it.skipIf(process.env.QILIN_STAGEHAND_E2E !== '1' || process.platform === 'win32')('scrubs real Chromium environment and closes it during canceled Stagehand initialization', { timeout: 120_000, retry: 0 }, async () => {
  const executable = process.env.QILIN_BROWSER_EXECUTABLE
  if (executable === undefined) throw new Error('QILIN_STAGEHAND_E2E requires QILIN_BROWSER_EXECUTABLE')
  const root = await mkdtemp(join(tmpdir(), 'qilin-stagehand-acquire-'))
  ...
  vi.stubEnv('QILIN_BROWSER_FIXTURE_ID', 'do-not-forward')
...
it.skipIf(process.env.QILIN_STAGEHAND_E2E !== '1')('drains canceled navigation before reconnecting to the same Chromium tabs', { timeout: 120_000, retry: 0 }, async () => {
  const executable = process.env.QILIN_BROWSER_EXECUTABLE
  if (executable === undefined) throw new Error('QILIN_STAGEHAND_E2E requires QILIN_BROWSER_EXECUTABLE')

...
import Llm, { LlmAdapter, ToolCallId, createUserMessage } from '@qilin/llm'
...
import { bindScopeParent } from '@qilin/scope'
...
async function load(exclusive = false, mode?: string, toolCallTimeoutMs?: number, toolOrder?: string[]) {
  const root = await mkdtemp(join(tmpdir(), 'qilin-browser-mcp-'))
  roots.push(root)

...
import BrowserUse from '@qilin/browser-use'
import SystemPrompt from '@qilin/system-prompt'
import Tools from '@qilin/tools'
import Llm from '@qilin/llm'
import { ToolCallId } from '@qilin/llm'
import Sessions, { SessionId } from '@qilin/session'
import Agents from '@qilin/agent'
import AgentLoop from '@qilin/agent-loop'
import Projections from '@qilin/session-projection'
...
export async function verifyMcpBrowser(
  ...
  const root = await mkdtemp(join(tmpdir(), 'qilin-browser-upstream-'))
  const observed = new Set<string>()
  const server = createServer((request, response) => {
    observed.add(request.url!)
    response.writeHead(200, { 'Content-Type': 'text/html' })
    response.end('<!doctype html><title>QILIN browser fixture</title><h1>Browser integration works</h1>')
  })

...
it('loads both tarballs through plain Node and mounts their base image and overlay', { retry: 0 }, async (test) => {
  ...
  for (const name of packages) {
    ...
    const directory = join(root, 'node_modules/@deepseek-ai', `qilin-experimental-${name}`)
  ...
  const script = `
    ...
    assert.ok(vfs.existsSync('/qilin/' + packer.MANIFEST_PATH))
    mkdirSync('overlay')
    ...
    runtime.loadVfsOverlay(await runtime.inflateImage(overlay.image, 'packed overlay'), '/qilin', vfs)
    assert.equal(vfs.readFileSync('/qilin/workspace/hello.txt', 'utf8'), 'packed worker pair\\n')

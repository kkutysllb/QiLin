...
import * as FsObservationPolicy from '@qilin/fs-observation-policy'
...
import * as ShellEnv from '@qilin/shell-env'
...
const REAL = process.env.QILIN_AUTO_REVIEW_CERTIFICATION === '1'
...
async function mount(ctx: Context, workspace: string, qilinHome: string): Promise<void> {
  ...
  await ctx.plugin(ShellEnv, { qilinHome })
...
function orchestrate(ctx: Context, real: boolean) {
  ...
  ctx.on('llm/stream', (options, next) => {
    ...
    if (options.system?.startsWith('REVIEW_POLICY\n') === true
      && source?.kind === 'plugin' && source.plugin === 'qilin-experimental-auto-review') {
...
it('certifies eight Auto risk/authorization cases with zero retries and zero skipped', {
  ...
  const root = await mkdtemp(join(tmpdir(), 'qilin-auto-review-'))
  const ctx = new Context()
  try {
    await chmod(root, 0o700)
    const workspace = join(root, 'workspace')
    const qilinHome = join(root, 'qilin-home')
    await mkdir(workspace)
    await mkdir(qilinHome, { mode: 0o700 })
    vi.stubEnv('QILIN_HOME', qilinHome)
    await mount(ctx, workspace, qilinHome)
...
it.each(['native', 'ptc-inner'] as const)('feeds denial back, re-reviews a new call and accepts narrowed work through %s', {
  ...
  const root = await mkdtemp(join(tmpdir(), 'qilin-auto-review-recovery-'))
  const ctx = new Context()
  try {
    const qilinHome = join(root, 'home')
    await mkdir(qilinHome)
    vi.stubEnv('QILIN_HOME', qilinHome)
    await mount(ctx, root, qilinHome)

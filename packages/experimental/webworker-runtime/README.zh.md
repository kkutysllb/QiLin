...
- **worker 部署不提供账号界面**：页面半从页面自身 origin 读 `/api/auth/status`，而静态宿主对它的回答是 404，因此账号入口渲染为未登录。账号属于服务端部署——那里 `/login` 与 `/setup` 是静态文档——而本部署回答 `/api` 所用的共享 fetch handler 从不查询已安装的会话权限。

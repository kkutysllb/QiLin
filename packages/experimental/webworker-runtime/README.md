...
- **The worker deployment serves no account surface**: the page half reads `/api/auth/status` from the page's own origin, which a static host answers 404, so the account door renders signed out. Accounts belong to a server deployment — `/login` and `/setup` are static documents there — and the shared fetch handler this deployment answers `/api` on never consults an installed session authority.

...
import { readProfileManifest, readProfilePatches, reconcileProfilePatches, PROFILE_PATCH_FILENAME } from '@qilin/app-boot'
  ...
  async* [Service.init](): AsyncGenerator<() => Promise<void>, void, unknown> {
    ...
    if (profile !== undefined) {
      ...
      const refresh = async (manifestOnly: boolean): Promise<void> => {
        const bundles = JSON.stringify(readProfileManifest('qilin', profile.dir).qilin?.profile?.bundles ?? [])
        ...
        const patches = readProfilePatches('qilin', profile)
        const warnings = await reconcileProfilePatches(this.ownerContext.root, patches, 'qilin')
        lastInputs = inputs
        lastBundles = bundles

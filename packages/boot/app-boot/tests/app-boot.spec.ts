...
describe('auditStartupEntries', () => {
  ...
  it('rejects a required entry pending on an injected service', async () => {
    await expect(auditStartupEntries(ctxWith([{
      ...
      'headless-runner (@qilin/headless): pending (waiting for service: headlessStartup)',
    )
  })
})
...
describe('boot', () => {
  ...
  it('disposes successful entries and rejects when a required entry fails', async () => {
    ...
    writeFileSync(join(dir, 'good.mjs'), [
      'export function apply(ctx) {',
      '  globalThis.__QILIN_REQUIRED_TEST_DISPOSED__ = false',
      '  ctx.effect(() => () => { globalThis.__QILIN_REQUIRED_TEST_DISPOSED__ = true })',
      '}',
      '',
    ].join('\n'))
    ...
    disposed = (globalThis as { __QILIN_REQUIRED_TEST_DISPOSED__?: boolean }).__QILIN_REQUIRED_TEST_DISPOSED__ ?? false
    delete (globalThis as { __QILIN_REQUIRED_TEST_DISPOSED__?: boolean }).__QILIN_REQUIRED_TEST_DISPOSED__
    expect(disposed).toBe(true)
  })
...
describe('addHarnessSourceSection', () => {
  ...
  it('distinguishes the source path from the current workdir after reusable instructions', async () => {
    ...
    try {
      ...
      // through `-1 < n`.
      const identityAt = rendered.indexOf('You are an AI agent powered by QiLin.')

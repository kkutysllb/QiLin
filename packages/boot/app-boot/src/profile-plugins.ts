...
export interface ProfilePluginLocation {
  ...
  /** Absolute package.json path of the owning qilin installation. */
...
export function writeProfileBundles(
  ...
  const updated = { ...manifest, qilin: { ...manifest.qilin, profile: { ...manifest.qilin?.profile, bundles: [...bundles] } } }
...
export function reconcileProfilePlugins(options: ProfilePluginLocation & {
  ...
  const disabled = new Set(options.preserveDisabled
    ...
    : [])
  const previous = after.manifest.qilin?.profile?.bundles ?? []

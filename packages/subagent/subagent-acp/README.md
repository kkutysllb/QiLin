...
The generated [configuration catalog](../../../docs/config-catalog.md#deepseek-aiqilin-subagent-acp) is the exhaustive source for every accepted field and its JSDoc.
...
A start resolves the child's working directory (the configured `cwd` override, else the parent session's cwd), spawns the command through the subprocess seam, performs the ACP `initialize` and `newSession` hanqilinake, and only then publishes the run. Fulfillment means a remote session is ready and ownership has transferred to the caller. Disposal is idempotent: it closes stdin and waits a configured grace for cooperative quiescence, then escalates through SIGTERM to SIGKILL and awaits whole-range exit. Cleanup failures remain observable as ordered safe facts and never claim quiescence.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#deepseek-aiqilin-subagent-acp) — every accepted config field and its source declaration.
...
- **A fresh process per run** — there is no process pooling; each delegation pays the full spawn and ACP hanqilinake cost.

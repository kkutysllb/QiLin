...
describe('CodexAppServerWire', () => {
  it('sends the fixed hanqilinake, thread, and turn payloads and keeps final_answer', async () => {
    const child = fakeChild()
    const wire = defaultWire(child)
  ...
  it('rejects invalid hanqilinake, thread, and turn response shapes', async () => {
    {
      const child = fakeChild()
      const wire = defaultWire(child)
      wire.start()

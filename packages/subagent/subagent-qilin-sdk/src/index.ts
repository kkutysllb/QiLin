...
export const name = 'subagent-qilin-sdk'
...
type ResolvedConfig = Required<Omit<Config, 'cwd' | 'maxTokens' | 'qilinBin'>> & Pick<Config, 'cwd' | 'maxTokens' | 'qilinBin'>
...
function resolveConfiguredFile(field: string, value: string): string {
  ...
  throw new TypeError(`subagent-qilin-sdk ${field} must name an existing file: ${path}`)
}

/** QILIN SDK can apply Agent route options while the other start features remain child-owned. */
 ...
 * process-wide hanqilinake; output schema, depth, tool filter, and persona stay
...
class SdkSubagentProvider implements SubagentProvider {
  ...
  start(request: SubagentStartRequest) {
    ...
    let cwd: string
    try {
      cwd = resolveChildCwd('subagent-qilin-sdk', this.config.cwd, request.parent.session.header.cwd)
    } catch (error: unknown) {
      const failure = sdkConfigurationFailure(error)
      this.ctx.logger.warn(`subagent-qilin-sdk "${this.name}": child start failed: %o`, error)
      throw failure
    }
    const route = resolveSdkRoute(this.config, request.agentOptions)
    const spec: SdkRunSpec = {
      ...this.config.qilinBin === undefined ? {} : { qilinBin: this.config.qilinBin },
      profile: this.config.profile,
      patches: this.config.patches,
      qilinHome: this.config.qilinHome,
      cwd,
      ...route,
      env: this.config.env,
      ...
      onError: (error, stopReason) => {
        ...
        this.ctx.logger.warn(`subagent-qilin-sdk "${this.name}": child run failed (${stopReason}): ${error.message}`)
      },
    }
...
export function apply(ctx: Context, config: Config): void {
  ...
  assertPositiveFinite('subagent-qilin-sdk', 'shutdownTimeoutMs', resolved.shutdownTimeoutMs)
  assertPositiveFinite('subagent-qilin-sdk', 'disposeEofGraceMs', resolved.disposeEofGraceMs)
  assertPositiveFinite('subagent-qilin-sdk', 'disposeGraceMs', resolved.disposeGraceMs)
  if (resolved.maxTokens !== undefined && (!Number.isSafeInteger(resolved.maxTokens) || resolved.maxTokens <= 0)) {
    throw new TypeError('subagent-qilin-sdk maxTokens must be a positive safe integer')
  }
  if (!isAbsolute(resolved.qilinHome)) throw new TypeError('subagent-qilin-sdk qilinHome must be an absolute path')
  const launchPaths: ResolvedConfig = {
    ...resolved,
    patches: resolved.patches.map((path, index) => resolveConfiguredFile(`patches[${String(index)}]`, path)),
    ...resolved.qilinBin === undefined ? {} : { qilinBin: resolveConfiguredFile('qilinBin', resolved.qilinBin) },
  }
  ...
  const configuredCwd = validateConfiguredCwd('subagent-qilin-sdk', resolved.cwd)

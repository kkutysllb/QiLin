 ...
 * hanqilinake, maps child failures to stop reasons, and tears down to
...
export interface SdkRunSpec {
  ...
  model: string
  /** Optional adapter-owned reasoning effort sent in the child runtime's initialize hanqilinake. */
  reasoningEffort?: ReasoningEffortId
  /** Optional per-request output-token cap sent in the child runtime's initialize hanqilinake. */
  maxTokens?: number
  /**
...
class SdkRunFailure extends Error {
  constructor(readonly facts: SdkFailureFacts, cause: unknown) {
    super(`subagent-qilin-sdk: ${failureDiagnostic(facts)}`, { cause })
 ...
 * @returns an Error whose message contains only the fixed QILIN SDK failure line.
 */
...
/**
 * Start and publish one SDK runtime child after its `initialize` hanqilinake.
...
export async function startSdkRun(request: SubagentStartRequest, spec: SdkRunSpec): Promise<SubagentRun> {
  ...
  const harness = internals.createHarness({
    ...spec.qilinBin === undefined ? {} : { qilinBin: spec.qilinBin },
    profile: spec.profile,
    patches: spec.patches,
    qilinHome: spec.qilinHome,
    processCwd: spec.cwd,
  ...
  // Establish the child hanqilinake before publishing a handle. Any failure
  ...
  try {
    ...
    // the microtask drain between hanqilinake fulfillment and this continuation,

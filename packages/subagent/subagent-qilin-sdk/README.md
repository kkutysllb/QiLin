...
Mount this provider when a delegation should run as a complete Harness runtime in its own process. The common path is explicit: mount the seam, mount this provider, configure `qilinBin` when needed, and select the child `profile` and ordered `patches`.
...
| `qilinBin` | SDK dependency | Explicit qilin CLI module, resolved and checked at plugin load; omission uses the SDK dependency |
| `profile` | `sdk` | Named child profile |
| `patches` | `[]` | Ordered per-launch profile patch files, resolved and checked at plugin load |
| `qilinHome` | required | Absolute isolated Harness home for every nested child process |
...
The generated [configuration catalog](../../../docs/config-catalog.md#qilinsubagent-qilin-sdk) is the exhaustive source for every accepted field and its JSDoc.
...
```yaml
- id: subagent-qilin-sdk
  name: '@qilin/subagent-qilin-sdk'
  config:
    providerName: qilin-sdk
    profile: sdk
    patches: ['./profiles/research-child.cordis.yml']
    qilinHome: !!js qilinHomePath('children')
    maxTokens: 49152
    env:
...
- [Generated configuration catalog](../../../docs/config-catalog.md#qilinsubagent-qilin-sdk) — every accepted config field and its source declaration.

 ...
 * through the REAL backend over real stdio JSON-RPC, so the hanqilinake, the
...
describe('qilin-subagent-qilin-sdk provider', () => {
  ...
  it('cancelling between hanqilinake and publish rejects start after reap', async ({ task }) => {
    // The abort lands while the child is INSIDE initialize (ready-file
    // hanqilinake window): the fake touches READY, we abort, then GO lets the
    // hanqilinake complete — so the post-race `flags.cancelled` recheck must
    // reject even though the hanqilinake itself succeeded.
    ...
    try {
      ...
      try {
        ...
        controller.abort('mid-hanqilinake')
        writeFileSync(go, 'go\n')
  ...
  it('rejects after reaping when the child dies before the hanqilinake', async () => {
  ...
  it('cancelling mid-hanqilinake rejects start after reaping the child', async () => {
    const controller = new AbortController()
    const spec: SdkRunSpec = {
      profile: 'sdk',
      patches: [],

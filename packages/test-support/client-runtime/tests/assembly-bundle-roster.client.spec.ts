...
class Scratch {
  ...
  bundle(name: string, patch: string): void {
    this.pkg(name, { qilin: { bundle: { patch: './cordis.patch.yml' } } }, { 'cordis.patch.yml': patch })
  }

  web(name: string, client: Record<string, unknown> = {}): void {
    this.pkg(name, { qilin: { client: { platform: 'web', ...client } } })
  }
...
describe('bundleRoster on a scratch installation', () => {
  ...
  it('applies the layers in order and keeps enabled browser rows once, with their qilin.client declaration', () => {
    scratch.web('@t/a', { inject: ['@t/b'], immediately: true })
    scratch.web('@t/b')
    scratch.web('@t/c')
    scratch.web('plain')
    scratch.pkg('@t/host', { qilin: {} })
    scratch.pkg('@t/node', { qilin: { client: { platform: 'node' } } })

...
import LlmRuntime, { createAssistantMessage, createSystemMessage } from '@qilin/llm'
...
import FileSettings from '@qilin/settings-file'
...
async function context() {
  const home = await mkdtemp(join(tmpdir(), 'qilin-messages-test-'))
...
describe('Cordis provider composition', () => {
  it('resolves an attachment service loaded after the adapter and maps its read-only path', async () => {
    ...
    await ctx.plugin(LocalAttachments, { qilinHome: home })
  ...
  async function boot(...args: Parameters<typeof server>) {
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@qilin/credentials-local', LocalCredentials], ['@qilin/settings-file', FileSettings],

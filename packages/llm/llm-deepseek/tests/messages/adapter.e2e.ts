...
import LlmRuntime, { BlockAssembler, createSystemMessage, createToolResultMessage, ReasoningEffortId } from '@qilin/llm'
...
async function boot(models?: Messages.Config['models']) {
  const home = await mkdtemp(join(tmpdir(), 'qilin-messages-e2e-'))
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('DeepSeek Messages real API', () => {
  ...
  it.each([false, true])('submits Loader package inventory and records HTTP acceptance with session-log enabled=%s', async (enabled) => {
    ...
    vi.stubGlobal('fetch', async (input: string | URL | Request, init?: RequestInit) => {
      ...
      expect(body).toMatchObject({ qilin_plugin_packages: {
        version: 1, packages: [{ name: packageIdentity.name, version: packageIdentity.version }],
      } })
      if (enabled) {
        expect(body).toMatchObject({ qilin_session_log: {
      ...
      } else expect(body).not.toHaveProperty('qilin_session_log')

...
declare module '@qilin/deepseek-llm-api-extensions' {
  interface DeepSeekLlmApiExtensionMap {
    qilin_messages_test: { value: string }
  }
}
...
async function boot() {
  const home = await mkdtemp(join(tmpdir(), 'qilin-messages-extensions-'))
...
describe('Messages request extensions', () => {
  it('prepares the native body and accepts its contribution before yielding content', async () => {
    ...
    ctx.deepseekLlmApiExtensions.register('qilin_messages_test', {
      prepare: (value) => {
        request = value
    ...
    expect(request?.body).not.toHaveProperty('qilin_messages_test')
    ...
    expect(JSON.parse(body)).toMatchObject({ qilin_messages_test: { value: 'inventory' } })
  })

  it('rejects preparation before dispatch', async () => {
    const ctx = await boot()
    ctx.deepseekLlmApiExtensions.register('qilin_messages_test', { prepare() { throw new Error('inventory unavailable') } })
  ...
  it.each(['http', 'transport', 'stream'] as const)('records acceptance only for HTTP success despite a later %s failure', async (failure) => {
    const ctx = await boot()
    const accept = vi.fn()
    ctx.deepseekLlmApiExtensions.register('qilin_messages_test', { prepare: () => ({ value: { value: 'log' }, accept }) })

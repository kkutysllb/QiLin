...
The generated [configuration catalog](../../../docs/config-catalog.md#deepseek-aiqilin-llm-deepseek) is the exhaustive source for every accepted field and its JSDoc.
...
For either protocol, when `ctx.deepseekLlmApiExtensions` is present, the adapter prepares its registered top-level fields from the exact serialized base request before `fetch`. Preparation or field collisions fail before HTTP; after a 2xx response, the adapter accepts every captured contribution before consuming SSE. Transport and non-2xx failures do not accept them. Shipped compositions use this for the default-on incremental `qilin_session_log` field and the default-on active `qilin_plugin_packages` inventory; both stay outside model input.
...
- [Session-log upload](../../session/session-log-deepseek/README.md) — the default-on incremental `qilin_session_log` contribution.
- [Plugin package inventory](../plugin-package-inventory-deepseek/README.md) — the default-on `qilin_plugin_packages` contribution.

...
`servers` must contain at least one entry with non-empty ids; timer budgets must be positive integers within Node's timer range, and byte caps must be positive. The generated [configuration catalog](../../../docs/config-catalog.md#deepseek-aiqilin-lsp-stdio) is the exhaustive source for every accepted field.
...
| [`src/instance.ts`](src/instance.ts) | One server process: initialize hanqilinake, serialized transient-open queries, bounded teardown |

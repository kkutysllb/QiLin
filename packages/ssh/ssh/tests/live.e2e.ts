...
import * as LspStdio from '@qilin/lsp-stdio'
import { SessionProjectionRegistry } from '@qilin/session-projection'
import { SandboxPolicyService } from '@qilin/sandbox-policy'
import { SshFileSystem } from '@qilin/fs-ssh'
import { SshSubprocessRuntime } from '@qilin/subprocess-ssh'
import { SshSandboxProvider } from '@qilin/sandbox-ssh'
...
const configPath = process.env.QILIN_SSH_TEST_CONFIG
const bootstrap = process.env.QILIN_SSH_TEST_BOOTSTRAP
const languageServer = process.env.QILIN_SSH_TEST_LSP
...
describe.skipIf(!enabled)('POSIX SSH runtime acceptance', () => {
  ...
  it('preserves binary fd 7 through native ownership and read-only confinement', async () => {
    ...
    try {
      ...
      if (test.hello.platform === 'linux') {
        ...
        expect(facts.cgroup).toMatch(/qilin-subprocess-[^\n]+\.scope/)
      }
    } finally { await removeOwned(test) }
  }, 60_000)
  ...
  it('reports unknown outcomes on disconnect and stops the remote managed descendants without replay', async () => {
    ...
    try {
      ...
      expect(scope).toMatch(/qilin-subprocess-[^\n]+\.scope$/)

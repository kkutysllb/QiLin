...
使用 `QilinPackageManifest` 描述包元数据、`QilinManifest` 描述 `qilin` 下的公共字段，以及 `QilinClientManifest` 等成员类型描述单个领域。各读取方负责 JSON 解析、校验和默认值解析。
...
`QilinPackageManifest` 描述 DSH 使用的 package.json 字段，其中 `name` 和 `version` 必填；它不是完整的 npm schema（模式）。本地 profile 读取方使用 `Partial<QilinPackageManifest>`，因为 profile 无需发布版本。`QilinManifest` 仅描述 `qilin` 下的公共作者字段。TypeScript 检查示例并删除 `import type`；这些接口不解析 JSON，也不写入文件。

以下元数据字段均可选。省略时，格式版本或兼容的宿主版本保持未声明状态；读取方不推断默认值。

| 字段 | 含义 |
|---|---|
| `qilin.manifestVersion` | manifest（元数据清单）格式标识；声明的格式为 `1`，独立于 npm 包版本和 Session 格式版本。 |
| `engines.qilin` | 作者声明的兼容 DSH 版本，使用 SemVer 范围，也可填写精确的预发布版本。此字段与 `engines.node`、`engines.npm` 并列；engines 对象可省略 `qilin`。 |
...
- **兼容性仅作声明。** 当前安装器和加载器不强制检查 `qilin.manifestVersion` 或 `engines.qilin`；声明范围不会拒绝不兼容的宿主，也不会校验 SemVer 语法。

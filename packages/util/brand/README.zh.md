...
为跨包边界且可能被混淆的值添加品牌——`qilin-llm` 中的 `ToolCallId`、`qilin-session` 中由 agent（智能体）和会话共享的 `SessionId`、`qilin-jobs` 中的 `JobId`，以及 `qilin-session` 中的 `SessionSeq` 与 `SessionLogOffset`。保持局部或无法混淆的值不需要这种抽象。

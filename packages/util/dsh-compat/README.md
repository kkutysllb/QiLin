...
None, as this utility only maps DSH-era manifest keys and module names; it changes no prompt, tool schema, session event, or token budget.

#### KV Cache effect

None; it adds nothing to a request and subscribes to no cached prefix.

## Known Limitations and Deferred Work

<a id="known-limitations-and-deferred-work"></a>

- **Engine-specific DSH plugins still need per-plugin validation** — plugins that depend on engine-specific services, resource URL schemes, or Electron bridges are out of scope. This package handles manifest and module identity compatibility only; it does not emulate API drift between DSH and QiLin.

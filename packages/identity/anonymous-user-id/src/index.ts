 ...
 * harness home resolved by {@link resolveQilinHome} (`$QILIN_HOME` > `~/.qilin`),
 ...
 * machine: every process sharing one `$QILIN_HOME` reports the same id, and
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `QILIN_HOME`; defaults to `process.env`. */
...
export function getOrCreateAnonymousUserId(options: AnonymousUserIdOptions = {}): AnonymousUserId {
  const file = join(resolveQilinHome(undefined, options.env ?? process.env), ANONYMOUS_USER_ID_FILE_NAME)

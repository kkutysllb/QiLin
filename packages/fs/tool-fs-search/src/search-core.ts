 ...
 *
 * @module @qilin/tool-fs-search/search-core
 */
...
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@qilin/subprocess'
 ...
 * `@qilin/tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`@qilin/tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@qilin/tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@qilin/tool-fs-search/presentation} `grepSearchMeta`)

...
describe('ripgrep resolution', () => {
  ...
  it('uses the unpacked executable path for an Electron ASAR dependency', async () => {
    ...
    dependency.rgPath = '/Applications/DeepSeek Harness.app/Contents/Resources/app.asar/qilin/node_modules/@vscode/ripgrep/bin/rg'
    const { resolveRgPath } = await import('@qilin/tool-fs-search')

    await expect(resolveRgPath()).resolves.toBe(
      '/Applications/DeepSeek Harness.app/Contents/Resources/app.asar.unpacked/qilin/node_modules/@vscode/ripgrep/bin/rg',
    )
  })
})

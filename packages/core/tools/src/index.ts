...
import type { ToolCallId, ContentBlock, ToolSchema } from '@qilin/llm'
import { HarnessError } from '@qilin/llm'
import type { Agent } from '@qilin/agent'
import type { UserMessage } from '@qilin/session'
import { assertNever, deepFreeze, snapshotJsonValue, type JsonValue } from '@qilin/util-values'
import type { PromptSection, ToolProviderResult } from '@qilin/system-prompt'
import type { PtcRuntime } from '@qilin/ptc-runtime'
 ...
 * instead of deriving them: the seam's `qilin-ptc-runtime` README pair, its
...
// lives in its own UI-facing module; re-export it so `@qilin/tools`
...
export class ToolRuntime extends Service {
  ...
  private requirePtcTransport(): ToolDefinition {
    this.ptcTransport ??= createRunCodeTool(this, {
      ...
      resolveSandboxPolicy: (exec) => {
        ...
        if (policy === undefined) throw new Error('qilin-tools: confined PTC runtime requires sandboxPolicy')
  ...
  private requirePtcRuntime(mode: ToolPresentationMode): PtcRuntime {
    ...
    if (!runtime) {
      throw new Error(`qilin-tools: mode "${mode}" requires a PTC runtime — load a ctx.ptcRuntime implementation (e.g. @qilin/ptc-runtime-node) or set tools mode to "native"`)
    }

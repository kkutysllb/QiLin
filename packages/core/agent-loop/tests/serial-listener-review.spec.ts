...
import LocalFileReference from '@qilin/file-reference-local'
import SubagentRuntime from '@qilin/subagent'
import * as SubagentSpawn from '@qilin/subagent-spawn-in-process'
import * as SubagentTool from '@qilin/tool-subagent'
import Selection from '@qilin/tool-subagent/model-selection-settings'
import * as Schedule from '@qilin/schedule'
...
describe('serial creation listener integrations', () => {
  ...
  it('starts due Schedule work only after every creation listener finishes', async () => {
    const root = await mkdtemp(join(tmpdir(), 'qilin-serial-schedule-'))
    roots.push(root)

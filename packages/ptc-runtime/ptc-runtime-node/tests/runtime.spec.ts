...
import type { PtcBindingFunction, PtcBindingNamespace, PtcRunRequest } from '@qilin/ptc-runtime'
...
async function setup(config: Config = {}, mode: 'read-only' | 'workspace-write' | 'danger-full-access' = 'danger-full-access') {
  const root = await mkdtemp(join(homedir(), '.qilin-node-runtime-test-'))
...
describe('Node program process', () => {
  ...
  it.skipIf(process.platform !== 'win32' || !sandboxUsable)('uses the common Windows grant lock and private native temp without exposing ambient values', async () => {
    ...
    vi.stubEnv('TMP', tmp)
    vi.stubEnv('QILIN_TEST_RUNTIME_SECRET', 'must-not-inherit')
    ...
    const result = await run({
      ...
      bindings: bindings({ inspect: async (args) => {
        ...
        expect(path.startsWith(`${temp}\\qilin-`)).toBe(true)
    ...
    expect(value.env).toEqual([])
    expect(value.native.env).not.toContain('QILIN_TEST_RUNTIME_SECRET')
    expect(value.native.temp.startsWith(`${temp}\\qilin-`)).toBe(true)
    expect(value.observed).toBe('native-temp')
    expect((await readdir(join(tmp, 'qilin-acl-locks'))).some(name => name.endsWith('.lock'))).toBe(true)
  })

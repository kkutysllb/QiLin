...
The backend is a thin adapter over the OTel JS SDK: it owns feedback authorization, resource identity, and an outer shutdown deadline. Canonical ledger records use the `@qilin/session-telemetry-otel` instrumentation scope; this backend captures no operational records. Resource identity carries `service.name`/`service.version` from `qilin-llm`'s `APP_IDENTITY` plus the anonymous `user.id` (from `$DSH_HOME/.anonymous-user-id`), once per export batch rather than per record.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#deepseek-aiqilin-session-telemetry-otel) — every accepted config field and its source declaration.

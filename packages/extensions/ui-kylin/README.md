---
description: "Kylin dynamic-plugin browser surfaces for users and maintainers choosing, composing, or debugging the panel, tool cards, and @pluginId input."
...
The `kylin_define` card is a record: the name and purpose the model wrote, the source it wrote, and whether the definition is running — no switch, no approval, and a pointer to the panel. The `kylin_run` card shows the mode, the plugin, package, and run ids, the outcome, and offers the package's own business view through the `tool.view.kylin` slot when the package registered one. `kylin_stop` and `kylin_undefine` render compact action rows. All cards render from the recorded call and result, so replay shows the same card.
...
| [`src/client/KylinPanel.tsx`](src/client/KylinPanel.tsx) | The frame-wide panel and its run controls |
| [`src/client/KylinDefineRow.tsx`](src/client/KylinDefineRow.tsx) | The read-only `kylin_define` card |
| [`src/client/KylinRunRow.tsx`](src/client/KylinRunRow.tsx) | The `kylin_run` card and its business-view seat |
| [`src/client/KylinActionRow.tsx`](src/client/KylinActionRow.tsx) | The `kylin_stop` / `kylin_undefine` rows |
...
| [`src/client/slots.ts`](src/client/slots.ts) | Injected faces and the package-owned `tool.view.kylin` slot declaration |
| [`src/client/run-card-index.ts`](src/client/run-card-index.ts) | Per-session index of the latest eligible `kylin_run` card |

### How the panel stays current

Announcements (`kylin/dynamic-package`, `kylin/dynamic-retract`, `kylin/request-run`, `kylin/request-run-resolved`) trigger an inventory re-read instead of a patch-in-place update, because they carry no labels and a definition can appear or disappear between them. Reads are single-flight so several announcements settling at once cannot multiply the call; a connection reset both discards the in-flight read and frees the slot for a fresh one, so a reconnect never publishes the old host's rows.
...
- [Client runner](../kylin-client-runner/README.md) — the browser face the panel reads and calls.
- [Host runner](../kylin-host-runner/README.md) — the inventory and lifecycle verbs behind the panel.
- [Tool package](../tool-kylin/README.md) — the model-facing tools whose calls these cards render.
- [Extensions subsystem](../../../docs/subsystems/extensions.md) — the generated `ctx.dynamicKylinRunner` API and forwarded `kylin/*` events.
...
Indirectly, through the run and stop verbs these surfaces drive — the browser-side runner's orchestration for a run, and the host's stop and remove verbs, the same host verbs the model's `kylin_run` and `kylin_stop` tools reach — so whatever a running definition then contributes is the runner's effect, while nothing model-visible originates in this package, which renders logged call and result slices and a host inventory read, adds no prompt content, writes no session event, and deliberately leaves no session-log trace of a person approving, declining, running, or stopping anything.
...
- **An open panel does not see registry changes that announce nothing** — `kylin_define`, and an undefine of a definition that was not running, change the registry without a dispatch announcement, so a panel left open across one of them keeps its rows until it is closed and opened again. A run request is the exception: it blocks the model, so it both renders its own row and triggers a read.
...
**Runtime invariant:** No companion is published. A single keyed toolview registration whose disposal is proven by the HMR-safety spec. The one mutable relation this package owns — the per-definition run-state observable — lives in the browser process, out of reach of the host invariant service, and the node half emits no kylin events and holds no cross-plugin state.

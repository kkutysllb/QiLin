/**
 * The browser twin of the tool-kylin context facade: a whitelist of
...
/** Reject any service return that is a kylin Context (host guard twin). */
function denyContext(value: unknown, service: string, env: DynamicCordisGuardEnv): unknown {
  if (value instanceof Context) {
    return rejectGuard(env,
      `service "${service}" returned a kylin Context, which the dynamic facade does not expose. `
  ...
  return new Proxy(slots, {
    get(target, prop) {
      ...
      return (rawOptions: unknown, component: unknown): unknown => {
        ...
        if (slot === 'tool.view.kylin') {
          if (options.key !== 'self') {
            return rejectGuard(env, 'tool.view.kylin only accepts key "self"; the runtime binds it to this Package')
          }
  ...
  return new Proxy(theme, {
    get(target, prop) {
      ...
      return (source: unknown, tokens: unknown): unknown => {
        ...
        ctx.effect(() => dispose, 'kylin-client-runner: dynamic theme override layer')
        return dispose
      }
    },
  })

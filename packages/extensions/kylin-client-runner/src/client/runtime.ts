...
export interface DynamicCordisRunnerEnv {
  ...
  ctx: Context
  /** Client kylin Loader: dynamic packages become entries under it. */
  loader: Loader
...
export class DynamicCordisPackageRunner {
  ...
  /**
   * Unload one package (`kylin/dynamic-retract`: a stop, or an undefine
   * that stops first).
   * @param pluginId - stable Plugin identity.
  ...
  private async mount(half: DynamicCordisClientHalf): Promise<DynamicCordisLoadResult> {
    ...
    try {
      plugin = await evaluateClientHalf(half.pluginId, half.code, {
        ...
        noteError: (message) => {
          ...
          console.error(`[kylin-client-runner] ${half.pluginId} logged an error:`, message)
        },
      }, styles)
    } catch (error) {
      styles.dispose()
    ...
    if (sink === undefined) {
      throw new Error('kylin-client-runner: window.__ModuleLoader__ is missing (booted outside the web shell?)')
    }

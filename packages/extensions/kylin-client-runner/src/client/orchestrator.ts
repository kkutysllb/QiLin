...
export class CordisRunOrchestrator {
  ...
  open(request: CordisRunRequest): void {
    ...
    if (!request.requiresApproval) {
      void this.orchestrate({
        ...
        console.error(`[kylin-client-runner] automatic activation ${request.requestId} failed:`, error)
      })
      return
    }
  ...
  private async finishClientFailure(
    ...
    console.error(
      `[kylin-client-runner] Client activation ${plan.pluginId}/${plan.packageId} (${pluginRunId}) failed:`,
  ...
  private async answer(requestId: ApprovalRequestId, resolution: DynamicCordisRunResolution): Promise<void> {
    try {
      ...
      console.error(`[kylin-client-runner] answering run request ${requestId} failed:`, error)
    }
  }

...
`dsh-kylin-client-runner` 为程序调用方和现有浏览器控件运行进程内动态包的浏览器部分。它在请求获批或用户显式操作后加载定义，并在 Host 撤销运行时移除定义。页面刷新不会恢复定义。Creator UI 插件通过 Plugin Manager 使用已安装的 Client 模块。
...
本包不提供工具或提示。它以激活成功、缺失服务、拒绝或 Host/Client 失败响应 `kylin/request-run`。发送给会话的任何消息由 Host runner 负责。

...
Inspect Host and Client runtime APIs before writing plugin code. Creator mode provides these read-only tools alongside Plugin Manager, which owns persistent profile changes. The inspection registry is supplied by the Kylin host runner; browser queries need a connected page.
...
Creator mode includes this toolset. Other compositions mount `@qilin/tool-kylin` alongside the host runner that provides `kylinInspect`. Call `kylin_inspect_list` to discover providers, then `kylin_inspect_query` for a provider's exact methods and types. Use [Plugin Manager](../../boot/plugin-manager/README.md) to install bundles containing plugin code or MCP configuration.
...
- [Kylin host runner](../kylin-host-runner/README.md) — inspection registry and existing runtime consumers.

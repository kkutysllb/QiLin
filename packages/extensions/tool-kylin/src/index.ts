...
export const inject = ['tools', 'systemPrompt', 'kylinInspect']

function requireAgent(exec: ToolExecution): Agent {
  if (exec.agent === undefined) throw new Error('Kylin inspection requires an Agent-backed session')
  return exec.agent
}
...
export function apply(ctx: Context): void {
  ctx.systemPrompt.section({ name: 'tool:kylin', order: ctx.systemPrompt.getSectionOrder('TOOL_CORDIS'), text: CORDIS_SYSTEM_PROMPT })
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'from kylin_inspect_list, and input must satisfy that method\'s schema. Use this Tool before writing plugin code '

/**
 * Model-facing Kylin runtime/package inspection, define, run, stop, and remove tools.
...
import {
  KylinDynamicPackageId, KylinDynamicPluginId,
} from '@qilin/kylin-host-runner'
import type { DynamicKylinReference } from '@qilin/kylin-host-runner'
...
export const inject = ['tools', 'systemPrompt', 'dynamicKylinRunner', 'kylinInspect']

function requireAgent(exec: ToolExecution): Agent {
  if (exec.agent === undefined) throw new Error('Kylin dynamic tools require an Agent-backed session')
  return exec.agent
}

/** Register the Kylin tools and explicit `@pluginId` context injection. */
export function apply(ctx: Context): void {
  ...
  for (const provider of hostInspectProviders(ctx)) {
    ctx.effect(() => ctx.kylinInspect.register(provider), `tool-kylin: inspect ${provider.manifest.id}`)
  }

  ctx.tools.register(defineTool({
    name: 'kylin_inspect_list',
    description:
      'List every Kylin Inspect Provider currently known to the Host, including local Host Providers and the latest '
      ...
      + 'method for kylin_inspect_query from its result. Do not guess names or treat an Inspect method as a business '
    ...
    execute(_args, _exec): Promise<JsonValue> {
      return Promise.resolve({ providers: ctx.kylinInspect.list() } as unknown as JsonValue)
    },
    presentCall: presentInspectListCall,
  }))

  ctx.tools.register(defineTool({
    name: 'kylin_inspect_query',
    description:
      'Run a read-only query explicitly declared by an Inspect Provider. platform, provider, and method must come '
      + 'from kylin_inspect_list, and input must satisfy that method\'s schema. Use this Tool before kylin_define '
    ...
    parameters: {
      ...
      provider: { type: 'string', required: true, description: 'Exact Provider ID returned by kylin_inspect_list.' },
    ...
    async execute(args, exec) {
      const data = await ctx.kylinInspect.query(
        args.platform,
        args.provider,
        args.method,
        args.input,
  ...
  ctx.tools.register(defineTool({
    name: 'kylin_inspect_self',
    description:
      'Inspect dynamic Kylin objects owned by the current Session at increasing levels of detail. With no IDs, '
    ...
    parameters: {
      pluginId: { type: 'string', description: 'Stable Plugin ID returned by kylin_define or injected by @pluginId; omit it to list every current Plugin.' },
    ...
    execute(args, exec): Promise<JsonValue> {
      ...
      if (args.packageId !== undefined && args.pluginId === undefined) {
        throw new Error('kylin_inspect_self packageId requires pluginId')
      }
      if (args.pluginId === undefined) {
        return Promise.resolve({
          mode: 'plugins',
          plugins: ctx.dynamicKylinRunner.listPlugins(agent).map(reference => selfSummary(reference)),
        } as unknown as JsonValue)
      }
      const pluginId = KylinDynamicPluginId(args.pluginId)
      if (args.packageId === undefined) {
        const plugin = ctx.dynamicKylinRunner.inspectPlugin(agent, pluginId)
        return Promise.resolve({
          mode: 'plugin',
          ...selfSummary(plugin),
      ...
      return Promise.resolve(inspectSelfPackage(
        ...
        agent,
        pluginId,
        KylinDynamicPackageId(args.packageId),
      ) as unknown as JsonValue)
    },
  ...
  ctx.tools.register(defineTool({
    name: 'kylin_define',
    description:
      'Define an immutable Kylin Package. For a new Plugin, use kind:"new" and provide only a semantic prefix of '
      ...
      + 'a Kylin Plugin; no TypeScript, JSX, or import transformation occurs. Query Inspect before depending on a '
      + 'Service, Event, Builtin, Slot, or token. Define only validates parameters and syntax and records source: it '
      + 'does not request approval, execute apply, or change currentPackageId. On success, call kylin_run with the '
      + 'returned IDs.',
    parameters: {
      plugin: {
        required: true,
      ...
      code: {
        ...
        required: true,
        properties: {
          host: { type: 'string', description: 'Plain JavaScript function body that returns the Host-half Kylin Plugin.' },
          client: { type: 'string', description: 'Plain JavaScript function body that returns the browser Client-half Kylin Plugin.' },
        },
      },
    },
    output: {
      ...
      render: (_args, value) => [{
        ...
        text: `Defined ${value.pluginId}/${value.packageId} (${value.name}); it is not running yet. `
          + 'Use kylin_run to activate this Package.',
      }],
    ...
    execute(args, exec) {
      const plugin = args.plugin.kind === 'new'
        ...
        : { kind: 'existing' as const, pluginId: KylinDynamicPluginId(args.plugin.pluginId) }
      const receipt = ctx.dynamicKylinRunner.define({
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'kylin_inspect_self, correct the same Plugin, and retry autonomously. Do not request approval again after '
      + 'the user rejects it.',
    parameters: {
      pluginId: { type: 'string', required: true, description: 'Stable Plugin ID returned by kylin_define.' },
    ...
    async execute(args, exec) {
      const agent = requireAgent(exec)
      const pluginId = KylinDynamicPluginId(args.pluginId)
      const packageId = KylinDynamicPackageId(args.packageId)
      const receipt = await ctx.dynamicKylinRunner.run(agent, pluginId, packageId, args.mode, exec.signal)
      ...
      const row = ctx.dynamicKylinRunner.snapshot(agent).find(candidate => candidate.pluginId === pluginId)
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'temporarily; use kylin_undefine for permanent removal.',
    parameters: {
    ...
    async execute(args, exec) {
      const receipt = await ctx.dynamicKylinRunner.stop(requireAgent(exec), KylinDynamicPluginId(args.pluginId))
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'or rollback; use kylin_stop instead.',
    parameters: {
    ...
    async execute(args, exec) {
      const receipt = await ctx.dynamicKylinRunner.undefine(requireAgent(exec), KylinDynamicPluginId(args.pluginId))
      if (!receipt.ok) throw new Error(receipt.message)
  ...
  ctx.on('agent/pre-step', async ({ agent, messages, signal }, next): Promise<PreStepDecision> => {
    ...
    const contexts = ids.map((id) => {
      const reference = ctx.dynamicKylinRunner.reference(agent, KylinDynamicPluginId(id))
...
function selfSummary(reference: DynamicKylinReference & { packages?: readonly unknown[] }): Record<string, JsonValue> {
...
function selfState(reference: DynamicKylinReference): SelfState {
...
function inspectSelfPackage(
  ctx: Context,
  agent: Agent,
  pluginId: ReturnType<typeof KylinDynamicPluginId>,
  packageId: ReturnType<typeof KylinDynamicPackageId>,
): Record<string, JsonValue> {
  const inspected = ctx.dynamicKylinRunner.inspectPackage(agent, pluginId, packageId)
  const row = ctx.dynamicKylinRunner.snapshot(agent).find(candidate => candidate.pluginId === pluginId)
...
function renderReference(reference: ReturnType<Context['dynamicKylinRunner']['reference']> & {}): string {
  const mode = reference.currentPackageId === undefined ? 'run' : 'update'
  return [
    '<kylin_dynamic_plugin_context>',
    ...
    `Before modifying it, call kylin_inspect_self with pluginId="${reference.pluginId}" and packageId="${reference.packageId}" to read the exact metadata and source.`,
    `Use kylin_define with plugin.kind="existing" and the original pluginId="${reference.pluginId}" to append an immutable Package.`,
    `Do not create a new Plugin for this request. After kylin_define succeeds, call kylin_run mode="${mode}" with the returned packageId.`,
    '</kylin_dynamic_plugin_context>',
  ].join('\n')
}
...
function renderUnavailableReference(id: string): string {
  return [
    ...
    'It may have been removed, belong to another Session, or have been lost when the QILIN process restarted.',

/** Pure replay-safe render intents for Kylin tools. */
...
export function presentRuntimeInspectCall(args: { what?: string; name?: string }): GenericCallView {
  ...
  return { card: 'generic', kind: 'read', title: target === undefined ? 'Inspect Kylin runtime' : `Inspect Kylin runtime: ${target}` }
}
...
export function presentPackageInspectCall(args: { pluginId: string; packageId: string }): GenericCallView {
  return { card: 'generic', kind: 'read', title: `Inspect Kylin Package ${args.pluginId}/${args.packageId}` }
}
...
export function presentDefineCall(args: {
  ...
  return {
    card: 'generic',
    kind: 'execute',
    title: `Register Kylin Plugin "${args.name}" for ${target}: ${args.purpose}`,
    rawInput: args.code,
  }
}
...
export function presentUndefineCall(args: { pluginId: string }): GenericCallView {
  return { card: 'generic', kind: 'delete', title: `Remove Kylin Plugin ${args.pluginId}` }
}
...
export function presentRunCall(args: { pluginId: string; packageId: string; mode: 'run' | 'update' }): GenericCallView {
  return {
    card: 'generic',
    kind: 'execute',
    title: `${args.mode === 'update' ? 'Update' : 'Run'} Kylin Plugin ${args.pluginId} · ${args.packageId}`,
  }
}
...
export function presentStopCall(args: { pluginId: string }): GenericCallView {
  return { card: 'generic', kind: 'execute', title: `Stop Kylin Plugin ${args.pluginId}` }
}

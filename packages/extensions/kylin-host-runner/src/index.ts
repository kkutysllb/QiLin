...
import { KylinInspectRegistryService } from './inspect-registry.ts'
import { missingServices, startHostHalf } from './lifecycle.ts'
import { DynamicKylinRegistry } from './registry.ts'
import type {
  DynamicKylinDefineReceipt, DynamicKylinDefineRequest, DynamicKylinDefinition,
  DynamicKylinPackageInspection, DynamicKylinPendingRequest, DynamicKylinPlugin,
  DynamicKylinPluginInspection,
  DynamicKylinReference, DynamicKylinRun,
} from './registry.ts'
import { createSandbox, evaluateHostCode, precheckCode } from './sandbox.ts'
import type {
  ApprovalRequestId, KylinDynamicPackageId, KylinDynamicPluginId, KylinDynamicPluginRunId, KylinErrorDetails,
  KylinDynamicRunMode, KylinInspectProviderManifest, KylinInspectQueryResolution,
  KylinInspectRequestId, KylinInspectResolveAck, DynamicKylinClientSource, DynamicKylinHostHalfResult,
  DynamicKylinInventoryRow, DynamicKylinInvokeResult, DynamicKylinRenderFailure, DynamicKylinResolveAck,
  DynamicKylinRunAttempt, DynamicKylinRunResolution, DynamicKylinRunResponse, DynamicKylinStopResponse,
  DynamicKylinUndefineReceipt, RequestRunOutcome,
} from './types.ts'

export type * from './types.ts'
export type {
  DynamicKylinDefineReceipt, DynamicKylinDefineRequest, DynamicKylinDefinition, DynamicKylinHandler,
  DynamicKylinPackageInspection, DynamicKylinPlugin, DynamicKylinPluginInspection,
  DynamicKylinReference, DynamicKylinRun,
} from './registry.ts'
export { KylinInspectRegistryService } from './inspect-registry.ts'
export type { HostKylinInspectProviderRegistration } from './inspect-registry.ts'
 ...
 */
export function KylinDynamicPluginId(id: string): KylinDynamicPluginId {
  return id as KylinDynamicPluginId
}
 ...
 */
export function KylinDynamicPackageId(id: string): KylinDynamicPackageId {
  return id as KylinDynamicPackageId
}
 ...
 */
export function KylinDynamicPluginRunId(id: string): KylinDynamicPluginRunId {
  return id as KylinDynamicPluginRunId
}
...
declare module '@qilin/kylin' {
  interface Context {
    ...
    dynamicKylinRunner: DynamicKylinRunnerService
  }
}
...
export interface DynamicKylinSnapshotRow {
  pluginId: KylinDynamicPluginId
  currentPackageId?: KylinDynamicPackageId
  nextPackageId?: KylinDynamicPackageId
  packages: Array<{
    packageId: KylinDynamicPackageId
    name: string
    purpose: string
    hasHostHalf: boolean
  ...
  activeRun?: {
    pluginRunId: KylinDynamicPluginRunId
    packageId: KylinDynamicPackageId
    fiber?: Fiber
    handlers: string[]
    renderFailure?: DynamicKylinRenderFailure
  }
  latestRun?: DynamicKylinRunAttempt
}

interface ActivationPlan {
  plugin: DynamicKylinPlugin
  definition: DynamicKylinDefinition
  mode: KylinDynamicRunMode
}

/** Dynamic Plugin registry and Host-half lifecycle. */
export class DynamicKylinRunnerService extends TypertRemoteService {
  static inject = ['tools']
  ...
  private readonly registry = new DynamicKylinRegistry()
  private readonly inspectRegistry: KylinInspectRegistryService
  private readonly starting = new Map<KylinDynamicPluginId, Promise<DynamicKylinHostHalfResult>>()
  private readonly resolved: ResolvedConfig
  private group: Fiber | undefined

  /** Create the service under the Host composition. */
  constructor(ctx: Context, config: Config) {
    super(ctx, 'dynamicKylinRunner')
    this.rootCtx = ctx
    this.resolved = config as ResolvedConfig
    this.inspectRegistry = new KylinInspectRegistryService(ctx)
  }
  ...
  define(request: DynamicKylinDefineRequest): DynamicKylinDefineReceipt {
    ...
    if (name.length === 0) throw new Error('kylin_define needs a non-empty `name`')
    if (purpose.length === 0) throw new Error('kylin_define needs a non-empty `purpose`')
    if (request.code.host === undefined && request.code.client === undefined) {
      throw new Error('kylin_define needs `code.host`, `code.client`, or both')
    }
    ...
    let plugin: DynamicKylinPlugin
    if (request.plugin.kind === 'new') {
      const prefix = request.plugin.idPrefix.trim()
      if (!/^[a-z]{3,6}$/.test(prefix)) {
        throw new Error('kylin_define `plugin.idPrefix` must contain 3–6 lowercase English letters')
      }
      const pluginId = KylinDynamicPluginId(this.registry.mintPluginId(prefix))
      plugin = {
        pluginId,
    ...
    const packageId = KylinDynamicPackageId(this.registry.mintPackageId())
    const definition: DynamicKylinDefinition = {
      packageId,
      name,
      purpose,
  ...
  async undefineFromPanel(agent: Agent, pluginId: KylinDynamicPluginId): Promise<DynamicKylinUndefineReceipt> {
  ...
  async run(
    ...
    mode: KylinDynamicRunMode,
    signal?: AbortSignal,
  ): Promise<DynamicKylinRunResponse> {
    ...
    this.ctx.emit('kylin/request-run', {
      requestId,
      agentId: agent.id,
      pluginId,
      packageId,
  ...
  async runHostHalf(
  ...
  ): Promise<DynamicKylinHostHalfResult> {
    ...
    let attempt: DynamicKylinRunAttempt
    if (requestId !== null) {
  ...
  async resolveRequestRun(
    ...
    resolution: DynamicKylinRunResolution,
  ): Promise<DynamicKylinResolveAck> {
    const pending = this.registry.peekRequest(requestId)
  ...
  async stopFromPanel(agent: Agent, pluginId: KylinDynamicPluginId): Promise<DynamicKylinStopResponse> {
  ...
  syncInspectManifest(providers: readonly KylinInspectProviderManifest[]): null {
  ...
  /* jscpd:ignore-start */
  @Remote('inventory')
  inventory(): DynamicKylinInventoryRow[] {
  ...
  async reportRenderFailure(
    ...
    pluginId: KylinDynamicPluginId,
    pluginRunId: KylinDynamicPluginRunId,
    failure: DynamicKylinRenderFailure,
  ): Promise<null> {
    const plugin = this.owned(agent, pluginId)
  ...
  async invoke(
    ...
    method: string,
    args: JsonValue,
  ): Promise<DynamicKylinInvokeResult> {
  ...
  ): { ok: true } & ActivationPlan | { ok: false; response: Extract<DynamicKylinRunResponse, { ok: false }> } {
    const plugin = this.owned(agent, pluginId)
  ...
  private activate(
    ...
    attempt: DynamicKylinRunAttempt,
  ): Promise<DynamicKylinHostHalfResult> {
  ...
  private async startFresh(
    ...
    plugin.run = run
    this.ctx.emit('kylin/dynamic-package', {
      pluginId: plugin.pluginId,
      packageId: definition.packageId,
  ...
  private async startHost(
    ...
    hostCode: string,
    run: DynamicKylinRun,
  ): Promise<KylinErrorDetails | undefined> {
  ...
  private async settleActivation(
    plugin: DynamicKylinPlugin | undefined,
    resolution: DynamicKylinRunResolution,
    requestId?: ApprovalRequestId,
  ): Promise<DynamicKylinRunResponse> {
  ...
  private commitActivation(plugin: DynamicKylinPlugin, run: DynamicKylinRun): void {
  ...
  private runResponse(
    plugin: DynamicKylinPlugin,
    started: Extract<DynamicKylinHostHalfResult, { ok: true }>,
  ): Extract<DynamicKylinRunResponse, { ok: true }> {
    return {
      ok: true,
      status: 'running',
      pluginId: plugin.pluginId,
  ...
  private announceResolved(
    ...
    this.ctx.emit('kylin/request-run-resolved', { requestId, outcome })
  }

  private steerRunOutcome(
    pending: DynamicKylinPendingRequest,
    settled: DynamicKylinRunResponse,
  ): void {
    ...
    let text: string
    if (settled.ok) {
      text = `Kylin ${pending.mode} ${identity} completed successfully. `
      ...
      text = `The user rejected Kylin ${pending.mode} ${identity}. `
      ...
      text = `Kylin ${pending.mode} ${identity} failed after kylin_run returned ${returnedStatus}: `
  ...
  private steerRenderFailure(
    ...
    agent.steer(createUserMessage({
      content: [{
        type: 'text',
        text: `Kylin Client UI ${plugin.pluginId}/${definition.packageId} (${pluginRunId}) failed while rendering `
          ...
          + 'activate that Package autonomously with kylin_run mode:"update".',
      }],
      source: { kind: 'plugin', plugin: 'kylin-host-runner' },
    }))
  }
  ...
  private steerHostHandlerFailure(
    ...
    agent.steer(createUserMessage({
      content: [{
        type: 'text',
        text: `Kylin Host handler ${plugin.pluginId}/${run.packageId} (${run.pluginRunId}) failed when the Client called `
          ...
          + 'the new Package autonomously with kylin_run mode:"update". If the handler needs a Service, either declare '
  ...
  private steerGuardFailure(
    ...
    agent.steer(createUserMessage({
      content: [{
        type: 'text',
        text: `Kylin ${platform} guard rejected runtime code in ${plugin.pluginId}/${run.packageId} `
          ...
          + 'activate it autonomously with kylin_run mode:"update".',
      }],
      source: { kind: 'plugin', plugin: 'kylin-host-runner' },
    }))
  }
  ...
  private claimRuntimeFailure(plugin: DynamicKylinPlugin, run: DynamicKylinRun, key: string): boolean {
    const attempt = plugin.latestRun
  ...
  private cancelPending(pluginId: KylinDynamicPluginId, message: string): void {
  ...
  private createAttempt(plan: ActivationPlan): DynamicKylinRunAttempt {
    return {
      pluginRunId: KylinDynamicPluginRunId(this.registry.mintPluginRunId()),
  ...
  private failAttempt(
    plugin: DynamicKylinPlugin,
    attempt: DynamicKylinRunAttempt,
    phase: NonNullable<DynamicKylinRunAttempt['error']>['phase'],
    failure: KylinErrorDetails,
  ): void {
    attempt.status = 'failed'
  ...
  private diagnostic(
    ...
    attempt: DynamicKylinRunAttempt,
    phase: NonNullable<DynamicKylinRunAttempt['error']>['phase'],
    failure: KylinErrorDetails | string,
  ): NonNullable<DynamicKylinRunAttempt['error']> {
  ...
  private async retract(plugin: DynamicKylinPlugin): Promise<void> {
    const run = plugin.run
    if (run === undefined) return
    delete plugin.run
    for (const dispose of run.handlerDisposers.splice(0)) dispose()
    if (run.fiber !== undefined) await run.fiber.dispose()
    this.ctx.emit('kylin/dynamic-retract', {
      pluginId: plugin.pluginId,
      packageId: run.packageId,
      pluginRunId: run.pluginRunId,
    })
  ...
  private owned(agent: Agent, pluginId: KylinDynamicPluginId): DynamicKylinPlugin | undefined {
    const plugin = this.registry.get(pluginId)
    return plugin?.sessionId === agent.id ? plugin : undefined
  }

  private requireGroup(): Fiber {
    this.group ??= this.rootCtx.plugin({ name: 'kylin-dynamic', apply: () => {} })
    return this.group
  }
}

function missingFor(ctx: Context, run: DynamicKylinRun): string[] {
  return run.fiber === undefined ? [] : missingServices(ctx, run.fiber)
}

function missingPluginMessage(id: KylinDynamicPluginId): string {
  return `no dynamic plugin "${id}" in this process — it may have been removed or lost on QILIN restart`
}

function errorDetails(error: unknown): KylinErrorDetails {
...
function formatErrorDetails(failure: KylinErrorDetails): string {
  return `message: ${failure.message}`
    + (failure.stack === undefined ? '' : `\nstack:\n${failure.stack}`)
}

function cloneAttempt(attempt: DynamicKylinRunAttempt): DynamicKylinRunAttempt {
  return {
    ...attempt,
...
export default DynamicKylinRunnerService

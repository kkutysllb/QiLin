...
/**
 * Reject any injected-service return that is a kylin `Context`. Harness
...
function denyContext(value: unknown, service: string, reportFailure: (error: Error) => void): unknown {
  if (value instanceof Context) {
    return rejectGuard(reportFailure,
      `service "${service}" returned a kylin Context, which the sandbox does not expose. `
 ...
 * the `{ required, optional }` object form — kylin resolves it into a single
 ...
 * it declared — that is what lets kylin park it when a declared provider
 * goes away.
 */
  ...
  const denyRead = (prop: string): never => {
    if (ctx.get(prop) !== undefined) {
      return rejectGuard(reportFailure,
        ...
        + 'so kylin parks this dynamic package if the provider later goes away.',
      )
    }
    return rejectGuard(reportFailure,
...
/**
 * Narrow an arbitrary sandbox return value to a runnable kylin plugin: a

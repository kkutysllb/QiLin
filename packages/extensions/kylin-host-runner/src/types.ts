/**
 * Client-safe wire vocabulary of the dynamic Kylin plugin runner.
...
export type KylinDynamicPluginId = Branded<'KylinDynamicPluginId'>

/** Identity of one immutable package version belonging to a dynamic plugin. */
export type KylinDynamicPackageId = Branded<'KylinDynamicPackageId'>

/** Identity of one successful activation attempt. */
export type KylinDynamicPluginRunId = Branded<'KylinDynamicPluginRunId'>
...
export type KylinInspectRequestId = Branded<'KylinInspectRequestId'>

/** Runtime plane that owns an inspect provider. */
export type KylinInspectPlatform = 'host' | 'client'

/** One model-callable read-only query exposed by an inspect provider. */
export interface KylinInspectMethodManifest {
...
export interface KylinInspectProviderManifest {
  /** Provider identity, unique within one platform. */
  id: string
  /** Capability described by this provider. */
  description: string
  /** Explicit read-only queries. */
  methods: readonly KylinInspectMethodManifest[]
}

/** Provider directory row returned by `cordis_inspect_list`. */
export interface KylinInspectProviderView extends KylinInspectProviderManifest {
...
export interface KylinInspectQueryRequest {
...
export type KylinInspectQueryResolution =
...
export interface KylinErrorDetails {
...
export interface KylinHalfState {
...
export interface KylinRunDiagnostic {
  /** Stage that failed. */
...
export interface DynamicKylinRunAttempt {
  /** Exact attempt identity. */
  pluginRunId: KylinDynamicPluginRunId
  /** Target Package. */
  packageId: KylinDynamicPackageId
  ...
  host: KylinHalfState
  /** Client-half state. */
  client: KylinHalfState
  /** Most recent failure. */
  error?: KylinRunDiagnostic
}

/** One running package announced to browser pages. */
export interface DynamicKylinPackage {
...
export interface DynamicKylinRunRequest {
...
export interface DynamicKylinRequestResolved {
...
export interface DynamicKylinRetracted {
  /** Stable plugin instance. */
  pluginId: KylinDynamicPluginId
...
export interface DynamicKylinInventoryPackage {
...
export interface DynamicKylinInventoryRow {
  /** Stable plugin instance. */
  pluginId: KylinDynamicPluginId
  /** Session that owns this plugin. */
  agentId: SessionId
  /** Immutable versions in define order. */
  packages: readonly DynamicKylinInventoryPackage[]
...
export type DynamicKylinUndefineReceipt =
...
export interface DynamicKylinRenderFailure {
...
export type DynamicKylinRunResponse =
  | {
    ok: true
...
export type DynamicKylinStopResponse =
  | { ok: true }
...
export type DynamicKylinHostHalfResult =
  | {
    ok: true
    pluginId: KylinDynamicPluginId
    packageId: KylinDynamicPackageId
  ...
  | ({ ok: false } & KylinErrorDetails)

/** Client-half source for one exact activation. */
export interface DynamicKylinClientSource {
...
export type DynamicKylinRunResolution =
  | { ok: true; pluginRunId: KylinDynamicPluginRunId; waitingFor?: readonly string[] }
  | {
    ok: false
...
export interface DynamicKylinResolveAck {
  /** False for late, unknown, or stale answers. */
  accepted: boolean
}

/** Result of routing one Client call to the active Host half. */
export type DynamicKylinInvokeResult =
  | { ok: true; value: JsonValue }
  | ({ ok: false; code: 'plugin-not-running' | 'stale-run' | 'method-not-found' | 'handler-error' } & KylinErrorDetails)
...
declare module '@qilin/kylin' {
  interface Events {
     ...
     * @mode emit
     */
    'cordis/request-run'(request: DynamicKylinRunRequest): void
    /**
     * A pending Client activation request left the answerable state.
     * @param resolved - request identity and outcome.
     * @mode emit
     */
    'cordis/request-run-resolved'(resolved: DynamicKylinRequestResolved): void
    /**
     ...
     * @mode emit
     */
    'cordis/dynamic-package'(pkg: DynamicKylinPackage): void
    /**
     * One exact activation was withdrawn.
     * @param retracted - plugin, package, and run identity.
     * @mode emit
     */
    'cordis/dynamic-retract'(retracted: DynamicKylinRetracted): void
    /**
     ...
     * @mode emit
     */
    'cordis/inspect-query'(request: KylinInspectQueryRequest): void
    /**
     ...
     * @mode emit
     */
    'cordis/inspect-query-resolved'(resolved: KylinInspectQueryResolved): void
  }
}

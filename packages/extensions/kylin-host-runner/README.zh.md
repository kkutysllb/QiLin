---
description: "动态 Kylin 包的 host 半说明，供选择、组合或排查注册表、沙箱与运行往返的 agent（智能体）与维护者阅读。"
...
`dsh-kylin-host-runner` 让动态包在本进程中可运行：模型用 `kylin_define` 记录的定义留在这里，host 半在 `node:vm` 沙箱中运行，带浏览器半的包会等待人在页面上批准或拒绝，模型也可以在这里检查实时运行时及其定义。面向模型的工具在 `@qilin/tool-kylin` 中，浏览器半经 `@qilin/kylin-client-runner` 装载。定义只存在于进程内存中，因此 QILIN 重启即清空，也不会向磁盘写任何东西。唯一的配置字段 `vmTimeoutMs` 限制同步沙箱求值的时长。
...
在任何一个应当支持动态包的组合中挂载本插件——它支撑模型的 `kylin_*` 工具，而带浏览器半的包还需要在客户端组合中额外挂载 client runner 与 UI 包。常用路径是显式的：加载本包，按需设置 `vmTimeoutMs`，其余交给工具与浏览器。
...
定义由 `kylin_define` 记录、由 `kylin_run` 激活。只有 host 半的包直接在本进程中激活：它的代码在沙箱中运行。带浏览器半的包变成一次请求：它一直等到有人在一个页面上允许或拒绝，或提问的轮次被取消；作答页面随后先装载 host 半、再装载浏览器半。`mode: "run"` 启动当前包或重启它，`mode: "update"` 切换到另一个包版本。`kylin_stop` 结束一次存活运行——移除该包的 handler 与任何已装载的浏览器 UI——同时保留可再次运行的定义；`kylin_undefine` 停止并忘掉它。

### 定义的去向

定义以会话为界、以进程为本：包只对定义它的会话可见，其他会话读取时视其为不存在，QILIN 重启后一切都消失。会话日志保留一次 define 调用的参数——包括它提交的代码——以及回执；解析出的定义只存于内存注册表。浏览器半只能经一次运行到达页面，因此刷新后的页面手上什么都没有，直到有人再次运行该包。

### 信任立场

沙箱隔离全局变量，但不是安全边界：Node 全局变量不存在，或重定向到 Kylin 服务（`ctx.fs`、`ctx.web`、`ctx.bash` 与定时器 helper），host 半收到的是不含框架内部机制的 façade，但它声明的服务仍会触达存活运行时。对待动态包要像对待 bash 访问一样，参见[自引用工具集 Agent Note](../../../.agents/notes/implemented/feature/2026-07-08-self-referential-kylin-toolset.zh.md)。
...
runner 基于两项职责划分。**注册表与沙箱是同一个服务。** `DynamicKylinRunnerService` 拥有定义注册表、vm 沙箱、host 半 fiber 生命周期与 invoke handler 表，因此一个定义的整个生命周期只有一个 owner。**版本是不可变的包。** 插件持有 `define` 之后永不变化的包；`currentPackageId` 与 `nextPackageId` 指向运行中与目标版本，`mode: "run"` 与 `"update"` 编码目标是否等于当前版本。浏览器往返之所以存在，是因为浏览器半只能由页面执行：服务 emit 请求并挂起，由页面的结论结算，调用方的 `AbortSignal` 是唯一的另一条出路。
...
| [`src/lifecycle.ts`](src/lifecycle.ts) | 在 `kylin-dynamic` fiber 组下启动 host 半 |
| [`src/inspect-registry.ts`](src/inspect-registry.ts) | `ctx.kylinInspect` 注册表：host 提供方加镜像的 client manifest（元数据清单） |
| [`src/types.ts`](src/types.ts) | `dynamicKylinRunner` remote namespace 与转发事件共享的 client 安全载荷形态 |
...
- [工具包](../tool-kylin/README.zh.md)——调用本服务的模型侧工具。
...
- [extensions 子系统](../../../docs/subsystems/extensions.zh.md)——生成的 `ctx.kylinInspect` 与 `ctx.dynamicKylinRunner` API 及 `kylin/*` 事件。
- [自引用 Kylin 工具集 Agent Note](../../../.agents/notes/implemented/feature/2026-07-08-self-referential-kylin-toolset.zh.md)——沙箱语义、生命周期与组合的理由。
...
- **run 成功不等于 UI 渲染成功**——只要作答页面已装载浏览器半，`run` 就会返回；React 是随后才渲染的，因此抛异常的组件不可能出现在 run 回执里。该失败经 steering 与 `kylin_inspect_self` 诊断浮现。
...
- **运行播报不携带服务声明**——浏览器半声明的 `inject` 是从它在页面里返回的插件上读出的，因此 `kylin/request-run` 只携带元数据，绝无代码或服务清单。

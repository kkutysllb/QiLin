...
describe('ui-sidebar-plans apply', () => {
  ...
  it('registers the type, its dictionaries, and the body seat under the type\'s id', async () => {
    ...
    const definition = tabs.get(PLANS_KIND)
    expect(definition?.id).toBe(PLANS_ID)
    expect(definition?.priority).toBe('builtin')
    expect(definition?.title('sidebar://plans')).toBe('type.label')
    expect(definition?.guide?.map(entry => [entry.order, entry.title(), entry.description?.()]))
      .toEqual([[50, 'guide.title', 'guide.description']])
    expect(dictionaries.get('sidebarPlans')).toEqual({ zh, en })
    ...
    expect(registered.map(entry => [entry.name, entry.key, entry.locale, entry.component])).toEqual([
      ['sidebar.right.pane.tab', PLANS_ID, 'sidebarPlans', PlansBody],
    ])
  ...
  it('takes every registration back when the plugin is disposed', async () => {
    ...
    await fiber.dispose()
    expect(tabs.get(PLANS_KIND)).toBeUndefined()

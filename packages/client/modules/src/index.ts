...
function comboSectionMap(resource: ComboResource, original: Record<string, unknown>): Record<string, unknown> {
  ...
  const base = new URL(`/plugins/${resource.id}/client.js.map`, 'http://qilin.invalid')
...
export class ClientModuleRegistry extends Service {
  ...
  constructor(ctx: Context) {
    ...
    // The carrier arrives through the declared injection so the callback context owns
    // the property proxy; ctx.get() would report the service without providing it.

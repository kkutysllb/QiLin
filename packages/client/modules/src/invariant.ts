/**
 * Package-owned invariant companion for `@qilin/client-modules`.
 * @module @qilin/client-modules/invariant
 */
  ...
  ctx.on('internal/plugin', () => {
    ...
    for (const row of host.graph().entries) {
      if (host.clientPath(row.id) === undefined) {
        fail(`web plugin graph row "${row.id}" advertises ${row.url} but resolves no client bundle path — the served __QILIN_BOOT__ would 404 on fetch`)
      }
    }
  }, { global: true })
}

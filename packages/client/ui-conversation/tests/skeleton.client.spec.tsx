...
import type {
  ...
  ConversationViewsProps, ConversationWidthControlsProps,
...
/** Occurrence props plus the registration-kit share the runtime hands a local Component. */
type WidthControlLocalProps =
  Pick<ConversationWidthControlsProps, 'container' | 'phase' | 'useContentWidth' | 'setContentWidth'>

/** Caller fallback for an occurrence that selects no width controls. */
function NoWidthControls(): null { return null }

const FactoryWidthControlsTestContext = createContext<(props: never) => ReactNode>(NoWidthControls)

/** Stands in for the runtime's local-position binding: the caller-selected
 * component receives the occurrence props alongside the kit's width share. */
function StableConversationWidthControls(props: WidthControlLocalProps) {
  const kit = useContext(FactoryViewsTestContext)
  const Selected = useContext(FactoryWidthControlsTestContext) as ComponentType<WidthControlLocalProps>
  if (kit === undefined) throw new Error('Factory views test context is missing')
  return (
    <Selected
      container={props.container}
      phase={props.phase}
      useContentWidth={kit.useContentWidth}
      setContentWidth={kit.setContentWidth}
    />
  )
}
...
function mount(
  ...
  const renderFactorySlot = ((_name: string, input: ConversationContentInputProps, factoryOptions?: {
    ...
    const useFactorySlot = ((name: string, fallback: (props: never) => ReactNode) => {
      if (name === 'views') return StableConversationViews
      if (name === 'widthControls') return StableConversationWidthControls
      return factoryOptions?.slots?.[name] ?? fallback
    }) as ConversationContentProps['useFactorySlot']
    return (
      <FactoryWidthControlsTestContext.Provider value={factoryOptions?.slots?.widthControls ?? NoWidthControls}>
      ...
      </FactoryWidthControlsTestContext.Provider>
    )

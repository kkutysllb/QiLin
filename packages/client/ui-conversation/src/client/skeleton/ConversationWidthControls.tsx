...
export function ConversationWidthControls({ container, phase }: ConversationWidthControlsProps) {
  const publishWidths = useCallback((container: HTMLDivElement): void => {
    ...
    if (preference === null) target.style.removeProperty('--qilin-chat-user-width')
    else target.style.setProperty('--qilin-chat-user-width', `${resolveContentWidth(column, preference)}px`)
  }, [])

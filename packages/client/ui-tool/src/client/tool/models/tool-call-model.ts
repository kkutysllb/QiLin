 ...
 *
 * `kylin_define` is deliberately absent: ui-kylin registers a keyed
...
const TOOL_VARIANTS: Record<string, ToolRowVariant> = {
  ...
  edit: 'edit',
  run_code: 'code',
  kylin_package_inspect: 'read',
  kylin_runtime_inspect: 'read',
  ...
  // no program to show (that is `kylin_define`'s card) and no file to open. The
  // id lands in the summary slot, and the titles below name the act.
  kylin_run: 'others',
  kylin_stop: 'others',
  kylin_undefine: 'others',
}

/** Tool-owned titles that refine a generic row variant without replacing it. */
const TOOL_TITLE_KEYS: Record<string, ToolTitleKey> = {
  kylin_package_inspect: 'tool.title.inspect',
  kylin_runtime_inspect: 'tool.title.inspect',
  kylin_run: 'tool.title.runCordis',
  kylin_stop: 'tool.title.stopCordis',
  kylin_undefine: 'tool.title.removeCordis',

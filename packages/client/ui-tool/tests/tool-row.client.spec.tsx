...
describe('tool-call-model', () => {
  it('classifies known tools and falls back to others', () => {
    ...
    expect(classifyTool('kylin_runtime_inspect')).toBe('read')
    ...
    expect(classifyTool('kylin_run')).toBe('others')
    expect(classifyTool('kylin_stop')).toBe('others')
    expect(classifyTool('kylin_undefine')).toBe('others')
    expect(classifyTool('todo_write')).toBe('others')
  })

  it('names each kylin verb instead of leaving it a bare tool call', () => {
    // Every define/run pair the model makes puts a row in the flow, so the
    // generic "Tool call · kylin_run · dyn-1" fallback is user-visible slop.
    const titleOf = (name: string) => toolRowModel(name, running({ name, argsRaw: '{"id":"dyn-1"}' }))
    expect(t(titleOf('kylin_run').titleKey)).toBe('运行 Cordis 插件')
    expect(t(titleOf('kylin_stop').titleKey)).toBe('停止 Cordis 插件')
    expect(t(titleOf('kylin_undefine').titleKey)).toBe('移除 Cordis 插件')
    // An owned title takes the tool name out of the summary slot, leaving the
    // package id as the only mutable text.
    expect(titleOf('kylin_run').summary).toBe('dyn-1')
  })

  it('leaves kylin_define to its own keyed toolview', () => {
    // ui-kylin registers a keyed `tool.call.toolview` entry for kylin_define,
    ...
    const model = toolRowModel('kylin_define', running({ name: 'kylin_define', argsRaw: '{"name":"clock"}' }))
    expect(model.variant).toBe('others')
    expect(t(model.titleKey)).toBe('工具调用')
  })

  it('renders kylin mount verbs no shipped tool implements as generic calls', () => {
    // No shipped tool implements these kylin mount verbs, so a mapping would
    // be unreachable.
    expect(classifyTool('kylin_mount')).toBe('others')
    expect(t(toolRowModel('kylin_mount', running({ name: 'kylin_mount', argsRaw: '{}' })).titleKey)).toBe('工具调用')
    expect(t(toolRowModel('kylin_unmount', running({ name: 'kylin_unmount', argsRaw: '{}' })).titleKey)).toBe('工具调用')
  })
  ...
  it('gives Cordis lifecycle tools action titles over their generic variants', () => {
    expect(toolRowModel('kylin_runtime_inspect', running({
      name: 'kylin_runtime_inspect',
    ...
    expect(toolRowModel('kylin_run', running({
      name: 'kylin_run',
      argsRaw: '{"id":"dyn-2"}',
    }))).toMatchObject({
      variant: 'others',
    ...
    expect(toolRowModel('kylin_undefine', result({
      call: { name: 'kylin_undefine', argsRaw: '{"id":"dyn-2"}' },
    }))).toMatchObject({
      variant: 'others',

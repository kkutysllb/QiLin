...
import type { ClientRemote, DirectoryListing } from '@qilin/api-remotes/client'
...
function persistSelection(selection: {
  ...
  const backing = new Map([['qilin.sessions.current', JSON.stringify(selection)]])
...
describe('UiWorkspaceService', () => {
  ...
  it('persists a catalog-resolved address after string subagent navigation', () => {
    ...
    expect(JSON.parse(backing.get('qilin.sessions.current')!)).toEqual({

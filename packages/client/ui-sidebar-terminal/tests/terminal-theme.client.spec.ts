...
it('preserves indexed, extended and default application colors while updating QILIN selection colors', () => {
  const h = harness()
...
it('restores selected entries, all indexed colors and special colors to the current QILIN defaults', () => {
  const h = harness()

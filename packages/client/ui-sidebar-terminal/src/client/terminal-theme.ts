/** QILIN defaults and application OSC colors for one xterm screen. */
import type { IDisposable, ITheme, Terminal } from '@xterm/xterm'

/* oxlint-disable typescript/no-non-null-assertion -- OSC widths/indices are checked; QILIN defaults precede cursor reads. */
...
/** Keep program color overrides separate from the QILIN defaults used by OSC resets. */
export class TerminalTheme implements IDisposable {
  ...
  /**
   * Apply changed QILIN colors without replacing application-defined palette entries.
   * @param background - resolved opaque QILIN background.
   * @param foreground - resolved QILIN text color.
   */
  ...
  /** Preferred cursor color after the initial QILIN update, including OSC 12 overrides. */

...
In Windows Electron, `html[data-windows-titlebar]` keeps fullscreen panels below the caption and to the right of the expanded sidebar. The frame's `--qilin-windows-sidebar-width` supplies that inset and maximum width; `--qilin-windows-content-radius` supplies the top-left radius. Only fullscreen panels clip overflow for that corner.

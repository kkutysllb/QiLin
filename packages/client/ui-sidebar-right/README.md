---
description: "The right Sidebar of the qilin web client: one docking surface per session, two presentations, the navigation controller ctx.sidebarRight, the tab-type registry ctx.sidebarRightTabs, and the Tab domain."
kind: "package-reference"
---

# @qilin/client-ui-sidebar-right
...
The layout itself — the split tree, its operations, the drag gestures, the floating panels — belongs to `@qilin/client-ui-dockkit` and stays host-agnostic. This package supplies everything that kit refuses to know: the product's copy, what a tab's `kind` means, which tab a fresh pane is seeded with, where the surface is mounted, and how other plugins reach it.
...
Each session's surface persists under `qilin.sidebarRight.surface.v1`, keyed by session id, so a reload returns to the layout the user left in that session and a pruned session takes its layout with it. The recorded sequence rides along, bounded at 100 entries: the oldest drop first, which shortens how far back a step reaches and changes nothing else. Switching sessions keeps each surface where it was.
...
1. **The type** — `ctx.sidebarRightTabs.register({ id, kind, label, icon?, patterns?, priority?, canOpen?, title, single?, guide? })`, a static declaration with no runtime hook, returning a disposer. `id` is this implementation's identity in the tab system, unique across every registration (a package name is the natural value; the shipped guide is `@qilin/client-ui-sidebar-right/guide`): a kind is not unique once an extension may take a builtin's over, so the implementation names itself, and a second registration of an `id` throws. `label()` is the type's own name, which the switch row reads: a page type's `title(address)` names the content it opened and a resource type has no single address to name, so neither stands for the type. A resource type names `patterns`, globs over `qilin-resource://` addresses: one containing `:` matches the whole address (`qilin-resource://file/**`); one without matches the URI's path at any depth, ignoring case (`*.md`), and an address that is not a URI matches no such pattern. A page type — the guide, a file tree — names none and is opened by kind. `canOpen(address)` vetoes a match. `title(address)` is the tab chip's text, captured when the tab opens. `icon` is the glyph the chip draws before its title; omit it for a chip whose title carries the whole identity. `single: true` focuses an open tab of this kind wherever it sits instead of opening a second one, so the surface holds at most one; omit it for the default, where one tab opens per address and a page opens once per pane. `guide` lists entry boxes for the guide page; picking one opens the contributing type as a page. A `kind` carries at most one `builtin` and one `extension` registration (the extension is in force; the builtin resumes when it leaves); any other collision on a kind throws. The `id` is also the key the type's body, title, and badge register under, so an extension and the builtin it takes over hold distinct cells and the seat renders the one in force.
...
Three more seats extend what is already there: `sidebar.right.pane.tab.badge` (keyed by the definition's `id`, Session scope) draws a status pill between the chip's glyph and its title, with the same `useTabInfo()` share as the body and nothing drawn without a registrant; `sidebar.right.tab.guide` (chain) replaces the guide tab's body without replacing the tab; and `sidebar.right.tab.menu.item` (list) appends content-level actions to a tab's menu after the kit's own layout actions. No seat exists for pane-level actions or for collapsed-state controls yet, because nothing needs one.

<a id="tab-switches"></a>
## Tab switches

The package contributes the settings shell's `sidebar-right` section: one row per registered type, in registration order, carrying the type's `icon` when it declares one and named by its `label()`, with a switch. The list is the registry's, so a type shipped from another package appears without an edit here and a late registration appears on the next commit.

A switch decides what the surface offers. Turning a type off removes its guide entries — how a user reaches it — and makes `openResource` and `openTab` return without opening it, while every tab already open keeps rendering: nothing is closed behind the user's back. Only the switched-off ids are stored, in `localStorage` under `qilin.sidebarRight.disabledTabs`, which leaves a newly shipped type on by default and keeps the choice out of the Host and the Session log. A browser that refuses storage keeps the switches for this page and loses them on the next load.

<a id="ctxsidebarright"></a>
## `ctx.sidebarRight`

`openResource(address, options?)` and `openTab(kind, options?)` are the navigation controller, and every way into the column calls one of them: the conversation's file links and a tool row's line reference (`openResource(fileAddress, { params: { line } })`), the strip's add control and a guide entry box (`openTab`), a file tree's rows (`tab.actions.openResource`). A resource address is a `qilin-resource://<type>/…` URI; without `options.kind` the registry claims it (globs and `canOpen`, best band wins), with it that kind's type in force opens it. A page is named by kind; the tab is recorded under an address this package composes and nobody else spells (`contract/seed.ts`). Both run the same steps as one history entry: a resource tab already showing the same (kind, contentId) is focused wherever it sits unless `revealIfOpened: false`; page tabs always deduplicate within the target pane, regardless of that option; otherwise a new tab lands in `options.replaceTab`'s pane and slot (closing that tab), else `options.paneId`, else the active docked pane; the panel expands, because content the user cannot see is not opened. Then the Tab domain records the navigation — `params` reach the body as `navigation.params`, with `revision` stepped — outside the layout history. `params` is typed by what is opened: a viewer for a resource type merges its entry into `SidebarRightResourceParamsMap` (the text preview declares `{ line?: number }`); a page type that takes parameters merges into `SidebarRightTabParamsMap` under its kind; values are JSON-shaped by convention, unchecked at run time. An address outside `qilin-resource://`, one no type claims, or a kind nothing registered throws: that is a wiring mistake, not a user error. A type the user has turned off is not opened at all — the call returns without writing, because the offer that would have opened it is gone too — and only a kind nothing registered still throws.

`options.scope` lands an open in another session instead of the mounted one, without switching the conversation the user is looking at, and `openFile(scope, path, options?)` is the shortcut for that path: it builds the file address for `path` and opens it in `scope`, which is what a file tree's rows and Chat's file links call. `features` is this build's capability list and part of the public contract for a tab type shipped from another package: the type gates new API use on membership rather than on a version comparison, so it degrades against an older Sidebar instead of failing. The list covers the chip icon and badge seats, the switch page, single-instance opens, `openFile`, opens targeted at another session, and layout persistence, and no member is ever removed.
...
Every string in the column comes from the `sidebarRight` locale namespace, including the kit's accessible names. A tab's title is fixed when the tab is minted; a type's `label()` and its guide entries' copy are read on every use, so a language change needs no re-registration.
...
- **Titles are fixed at open time.** A type's `title(address)` is captured into the record; a live title comes only from the optional title seat. Rewriting the record's title is deliberately absent: the title seat already draws a live chip, so a second write path would add state the seat computes.
...
- **Switches are browser-local.** The turned-off set lives in this browser's `localStorage`, so another browser, or one that refuses storage, offers every registered type.

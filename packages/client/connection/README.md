...
A static desktop page can provide `__QILIN_TRANSPORT__.streamBaseUrl` for the HTTP origin of its owned Host. The Gateway uses that origin for its WebSocket while HTTP transport remains independently selected. The desktop carrier owns authentication; setting the origin alone grants no access.

The browser uses HTTP POST for Remote unary calls. API Gateway owns the `/api/remote.mux` WebSocket and its logical streams; shell-owned compositions provide equivalent Remote streams through `connection.rpc.open` without opening a WebSocket. The browser plugin reads the page transport, recovery settings, and location, then delegates to `installConnection(ctx, options)`. A composition that owns its carrier may call the same installer directly; the whole-client test tier does so. Each invocation creates one Context-owned service, so several Client trees can use different carriers in one realm. The Host half always provides the carrier-neutral RPC and exact `GET`/`HEAD`/`POST` route registries. When a Web carrier is present it also owns the sole `/api` route, Fetch bridge, browser authentication, and Host/Origin checks; a shell-owned carrier dispatches the shared Fetch handler directly. Each exact route declares buffered or streaming request-body handling before the bridge reads any bytes. Typert Gateway claims generated Remote endpoints, feature packages register non-JSON responses such as Session-log downloads and raw file uploads, and unclaimed requests return 404. Loopback hostname classification remains package-internal to the browser-facing Client state. Browser raw-body transfer is provided by [`qilin-client-file-upload`](../file-upload/README.md).
...
Every Host RPC method and WebSocket stream requires one browser session; there is no method-specific loopback tier. Whichever authentication owns the deployment supplies that session. Each process mints a random launch token, and `qilin-web-app` prints and opens the application entry URL carrying it — `GET /workspace?token=...`. `frontend-static` delegates every configured index request to `ctx.connection.authorizeIndex`, which accepts that token only on `GET /workspace`, writes an authority-bound signed cookie, and redirects to the clean entry path; a missing, expired, malformed, or wrong-authority cookie then returns 401 before RPC dispatch. Static assets and configured public documents remain public. The HTTP carrier accepts no query token outside the entry exchange and no Authorization-header token. `src/web-entry.ts` authors the entry path as `WEB_ENTRY_PATH` and `ctx.connection.entryPath` publishes it, so an index server mounts the same value the handoff targets.

`ctx.connection.session` is the seat where an authentication capability installs an account-session authority. `install(authority)` seats one `ConnectionSessionAuthority` and returns the disposer releasing it; a second installation fails the installing plugin's load, because two session authorities cannot compose. The authority owns three verdicts: `authorizeIndex` owns the browser response for a gated index request, `isPublicApiRequest` marks the requests that belong to the authentication surface itself and need no session, and `verify` checks the request's session. While an authority claims the seat, `requestRejection` requires `verify` for every `/api` request it does not declare public, and `authorizeIndex` runs the launch-token exchange first — cleaning the printed URL into the entry redirect — then serves a gated index only for a verified session. Without an authority the device cookie stays the only browser credential, which is what a deployment with the gate disabled keeps.

The cookie signing secret is the owner-scoped `client-connection/browser-session` grant record in `ctx.credentials`. The local provider persists it in `$QILIN_HOME/.credentials.yaml`; `BrowserAuth` loads or creates the record during Connection activation and retains the secret in memory, so request authentication is synchronous. Deleting or replacing the record takes effect on the next Connection activation. Cookies carry an absolute issue/expiry interval, defaulting to 30 days through `cookieMaxAgeDays`, and bind the normalized hostname plus port in both their deterministic name and signed payload. They are host-only, `Path=/`, `HttpOnly`, and `SameSite=Strict`; they deliberately omit `Secure` because the shipped server uses loopback HTTP.

Before authentication, every request still passes `src/api-request-trust.ts`. Its `Host` must be loopback or match a `trustedHosts` entry: exact on `host:port`, any port on port-less entries, both sides WHATWG-normalized. An attached `Origin` must equal that Host and `sec-fetch-site: cross-site` is refused. Malformed configured authorities fail plugin load. These checks defend DNS rebinding and cross-site browser requests; they never establish identity. A failed Host/Origin check returns 403, while a trusted but unauthenticated request returns 401. `qilin web --host 0.0.0.0` remains unsupported. Decision records: [browser request trust](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.md) and [browser token authentication](../../../.agents/notes/implemented/architecture/2026-08-24-browser-token-authentication.md).
...
- **The device cookie has no logout operation** — clearing the browser cookie ends one browser session; deleting the owner credential record and restarting `qilin` revokes every session.

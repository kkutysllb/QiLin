...
| `TabRenderer` | one tab's body (`renderTab`), drawn flush to the pane's edges and the unbordered strip's bottom edge with the insets it chooses; optionally what its chip or panel header shows as a title (`renderTabTitle`, falling back to the record's `title`), a chip's and a panel header's leading glyph (`renderTabIcon`), and a docked chip's status pill (`renderTabBadge`); the embedder dispatches on `tab.kind` |
| `DockIntents` | the settled results of every gesture |

`DockController` satisfies `DockIntents` as written, so the simplest embedding hands the controller straight to `DockSurface`. An embedder that routes through its own store implements the same method names instead. Three props carry control policy rather than gestures: `canSplit` (surface-wide, the pane budget; disables the split control with `splitPaneDisabled`), `canAddTab(paneId)` (per pane, omits the add control; leave it out to draw one in every pane), and `canCloseTab(tabId)` (per tab, withholds the chip's close control and the menu's close item together; leave it out to keep every tab closable). Hiding the add control moves nothing else in the strip, and a withheld close moves nothing in the chip — the close control paints over the title's end rather than beside it. A pane's lone chip whose close is withheld draws quiet — no capsule, no hover fill — since there is nothing to select against and nothing to do to it. `renderTabIcon` and `renderTabBadge` are the two optional chip renderers, and the kit interprets neither: it holds no icon set and no badge copy, and a renderer that returns nothing draws nothing. `FloatLayer` takes `renderTabIcon` for a panel header's leading glyph and no badge, so a floating panel shows no status pill. A badge renderer runs on every strip render, so it reads an already-computed fact rather than deriving one. The kit adds one policy of its own, the room rule below, which disables a pane's split control with `splitPaneNarrow`; `onRoom(fits)` reports its readings so an embedder splitting programmatically can honour the same rule.

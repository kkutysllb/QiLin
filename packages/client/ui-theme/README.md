...
When the host composition includes an HTTP server, the host half embeds the registered `ui-theme` settings, or schema defaults, into each index response. Head CSS selects the document canvas color scheme before any script runs, including a `prefers-color-scheme` query for the `system` preference. A body script then sets `body[data-ds-dark-theme]` `--qilin-content-font-size`, and `--qilin-content-leading` before the loading page and application scripts, so the first paint uses the selected palette, text size, and line height.
...
- [ui-conversation](../ui-conversation/README.md) — a consumer of `--qilin-scrollbar-width` for the composer seat.

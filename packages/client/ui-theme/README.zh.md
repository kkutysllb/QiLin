...
插件在「通用」分区注册字号步进器；明/暗/跟随系统的选择由账户菜单负责，两者共用同一服务。步进器接受 12 至 17 px 的整数，默认值为 14 px。它以相同增量调整会话标题与基础文本，包括用户气泡与 composer 草稿；流内行的标题、摘要与表格跟随比正文低一档的字号，小号文本和代码保持固定字号。每次通过的变更都经 Host settings API 写入。连续快速变更按操作顺序携带命名空间 revision 串行写入，最新写入被拒时重新加载持久值。非 loopback 页面把这些选择都保留在进程内。

行间距步进器在每一个随字号缩放的档位行高上叠加一个 −2 至 8 px 的整数增量，默认值 0 保持出厂行高不变。字号与行距任一变动时，标题、正文与表格之间的相对间距关系都不改变；固定的小号文本与代码两档都不跟随。
...
当主机组合包含 HTTP 服务器时，宿主侧会把已注册的 `ui-theme` 设置或 schema 默认值嵌入每份 index 响应。浏览器在加载页面渲染前设置 `color-scheme`、`body[data-ds-dark-theme]`、`--qilin-content-font-size` 与 `--qilin-content-leading`，因此首帧绘制就采用所选调色板、字号与行高。
当主机组合包含 HTTP 服务器时，宿主侧会把已注册的 `ui-theme` 设置或 schema 默认值嵌入每份 index 响应。head CSS 会在任何脚本运行前选择文档画布的配色方案，其中 `system` 偏好使用 `prefers-color-scheme` 查询；随后，body 脚本会在加载页面和应用脚本之前设置 `body[data-ds-dark-theme]` 与 `--qilin-content-font-size`，因此首帧绘制就采用所选调色板与字号。
...
服务拥有主题、字号与行距状态并发布快照。ui-layout 呈现器应用这些快照，token 样式表则拥有颜色与会话文本尺度。
...
`scrollbar.css` 在 `body` 上把 `--qilin-scrollbar-thumb` 与 `--qilin-scrollbar-thumb-hover` 绑定到 l1 基础表面 token；高层级表面（菜单、浮层、对话框）在自己的容器上把它们重新绑定为 l2 token；这组变量的另一个合法目标是 `transparent`（ui-sidebar 在指针不在栏内时就这样重新绑定自己的列）。WebKit 系浏览器还会读取 `--qilin-scrollbar-width`、`--qilin-scrollbar-thumb-border` 与 `--qilin-scrollbar-track-margin`；滚动表面可以重新绑定它们，以在较窄的可见滑块外保留一条较宽的可拖拽轨道，或让轨道从圆角两端内缩。两条渲染路径在构造上互斥：Firefox 走 `@supports not selector(::-webkit-scrollbar)` 内的标准细滚动条，WebKit 系引擎走伪元素，因此几何与 hover 定制只经由伪元素这条路径生效。

### 偏好持久化

在 loopback 浏览器上，服务先以 schema 默认值立即提供自身，随后加载 `ui-theme` 命名空间，并把每次通过的主题、字号或行距变更经 Host settings API 写入。收到推送的设置变更时或重连后都会重新拉取该命名空间。非 loopback 页面不会创建该 Host-backed scope。该持久化边界由 [Host 支撑的偏好笔记](../../../.agents/notes/implemented/bug-fix/2026-08-06-host-backed-web-preferences.zh.md) 拥有。
...
- [ui-conversation](../ui-conversation/README.zh.md)——为 composer 席位消费 `--qilin-scrollbar-width` 的消费方。

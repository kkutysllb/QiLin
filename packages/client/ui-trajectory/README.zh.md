---
description: "qilin Web 客户端右侧边栏的 Trajectory 页面：按轮次组织的事件记录表加交互式时间概览，以及把同一记录表画成实时节点与边流动图的轨迹图视图。"
...
右侧边栏中的 Trajectory 标签页以按轮次组织的事件记录表和时间概览检查活动；轨迹图标签页则把记录画成实时流动的节点与边。记录表对用户、助手、工具、嵌套子工具和压缩记录分组，标示轮次与步骤边界，并为所选记录打开检查器，显示 token 用量、耗时、输入、输出、计时、图片和附件。较长历史打开时定位于尾部，按需加载更早页面，并且只渲染可见行。流式输出期间，视图会跟随尾部，直到你向上滚动；进行中的记录只显示开始标记，不会虚构耗时。
...
轨迹图页面从同一个引导页入口打开——入口序号为 21，紧跟记录表之后——按 `trajectory-graph` kind 打开，每个表面至多一个图并与记录表并列，正文以自己的 id 占用同一个键控标签正文座位。两个页面读取同一个 `useTrajectory` 投影的 Trajectory target，轨迹图自身不新增任何会话请求：记录表把记录呈现为行，轨迹图把记录画成输入、模型、工具三条泳道中的芯片，装在编号的轮次条带里，并用记录表自身记录的事实连成边——喂给每个请求的提示词、请求产出的结果、助手派发的每个调用、工具派发的子调用，以及转回下一个请求的循环。流式前缀与进行中的调用渲染为活动记录，其边带流动动画；工具栏可暂停流动，并在适应视图与尾部跟随之间切换；选中芯片会打开检查器，查看该记录的参数、结果与计时。
...
轨迹图页面是同一 snapshot 的纯投影：每个记录一个节点，外加流式前缀与每个未落定的调用；边只来自记录表自身记录的事实，不做任何虚构。一个不依赖 React 的布局模块把记录顺序作为纵轴、把行为者泳道作为横轴；渲染窗口只保留最近 400 条记录及其间的边。
...
- **轨迹图渲染尾部窗口**：轨迹图页面只保留最近 400 条记录及其间的边，并显示被裁掉的计数，因此很长的记录表展示的是最近的流动而不是完整会话。

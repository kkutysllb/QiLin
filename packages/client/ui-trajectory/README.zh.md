---
description: "qilin Web 客户端右侧边栏的 Trajectory 标签页：按轮次组织的事件记录表加交互式时间概览。"
...
右侧边栏中的 Trajectory 标签页让你以按轮次组织的事件记录表和交互式时间概览检查 agent 活动。它对用户、助手、工具、嵌套子工具和压缩记录分组，标示轮次与步骤边界，并为所选记录打开检查器，显示 token 用量、耗时、输入、输出、计时、图片和附件摘要。较长历史打开时定位于当前尾部，按需加载更早页面，并且只渲染可见行。流式输出期间，视图会跟随尾部，直到你向上滚动；进行中的记录只显示开始标记，不会虚构耗时。
...
从右侧边栏的引导页打开 Trajectory 标签页（或点击 Chat 工具卡片的查看操作，直接聚焦该调用），把 agent 活动作为事件记录表与时间线查看。初始尾部完成定位前，记录表会用明确的加载行遮住真实记录；更早的前缀仍未加载时，首行控件会在点击时加载一页更早的历史，并在该页加载期间显示禁用的加载状态。
...
记录表填满所在侧边栏面板并自持纵向滚动容器，不为会话的浮层 composer 预留空间。可滚动的 Summary 区域在悬停或聚焦前保持滚动条滑块透明，同时不改变滚动几何空间。本包不提供 service，也不声明 Context 合并。
...
- [ui-sidebar-right](../ui-sidebar-right/README.zh.md)——本包注册标签页类型与键控正文座位的侧边栏。
...
**运行时不变式：** 不发布伴生入口。这是纯消费插件，不发出 Cordis 事件，也不持有跨插件可变状态；标签页类型与正文注册均为普通 effect，其释放由本包的行为测试观察。

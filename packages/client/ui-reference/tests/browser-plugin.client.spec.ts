...
async function bench(
  ...
  sessions: RemoteLookup<SessionReferenceMentionCandidate> = vi.fn(() => Promise.resolve({
    ...
    value: [{
      ...
      mention: '@[Research](qilin-session:InNvdXJjZSI)',
    }],
  })),
...
describe('candidates', () => {
  ...
  it('labels a session without a cwd and still dates it', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: false,
        createdAt: CREATED_AT,
        mention: '@[same](qilin-session:InNhbWUi)',
      }],
    }))
    const { source } = await bench(files, sessions)
  ...
  it('falls back to the candidate createdAt for a session the Host list does not carry', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: true,
        createdAt: CREATED_AT,
        mention: '@[Unlisted run](qilin-session:InVubGlzdGVkIg)',
      }],
    }))
  ...
  it('uses current Session titles and groups direct subagents above ordinary Sessions', async () => {
      ...
      value: [
        {
          ...
          sameWorkspace: true,
          createdAt: CREATED_AT,
          mention: '@[Ordinary title](qilin-session:Im9yZGluYXJ5Ig)',
        },
        {
          ...
          sameWorkspace: true,
          createdAt: CREATED_AT,
          mention: '@[researcher](qilin-session:IndvcmtlciI)',
        },
      ],
    }))
    const { source } = await bench(files, sessions, {
    ...
    expect(source.onPick({
      ...
      insert: {
        source: 'reference',
        ref: '@[researcher](qilin-session:IndvcmtlciI)',
        label: 'researcher',
        appearance: 'session',
        clipboardText: '@[researcher](qilin-session:IndvcmtlciI)',
      },
    })
  })
  ...
  it('reads a session opened moments ago as the present, not a zero distance', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: true,
        createdAt: NOW - 1_000,
        mention: '@[Just now](qilin-session:Imp1c3Qtbm93Ig)',
      }],
    }))
  ...
  it('dates a session in the current workspace without repeating that workspace', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: true,
        createdAt: CREATED_AT,
        mention: '@[Sibling run](qilin-session:InNpYmxpbmdyIg)',
      }],
    }))
    const { source } = await bench(files, sessions)
...
describe('reference preview', () => {
  it('opens plain and quoted file references without treating folders or sessions as files', async () => {
    ...
    expect(openResource).toHaveBeenNthCalledWith(1, 'qilin-resource://file/session/target/notes/readme.md')
    expect(openResource).toHaveBeenNthCalledWith(2, 'qilin-resource://file/session/target/docs/a%20b.md')
    expect(source.openReference?.(session, { ref: '@docs/', appearance: 'folder' })).toBe(false)
    expect(source.openReference?.(session, { ref: '@[Research](qilin-session:abc)', appearance: 'session' })).toBe(false)

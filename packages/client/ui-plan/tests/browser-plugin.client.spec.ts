...
describe('ui-plan browser apply', () => {
  ...
  it('binds both entry points to the same Session resource and removes the sidebar seats', async () => {
    ...
    try {
      ...
      removeFileEntry()
      const address = 'qilin-resource://plan/s-plan/call'
      ...
      injected.openPlan(plan.callId)
      expect(b.openResourceIn).toHaveBeenLastCalledWith(SID, 'qilin-resource://plan/subagent/parent/s-plan/continuable/call')
      reviewInjected.openReview(pending, 'question:1')
      expect(b.openResourceIn).toHaveBeenLastCalledWith(SID, 'qilin-resource://plan/subagent/parent/s-plan/continuable/call')
      ...
      expect(first).toEqual([SID, expect.stringMatching(/^qilin-resource:\/\/plan-review\/s-plan\//), {

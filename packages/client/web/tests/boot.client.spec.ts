...
const transportGlobal = globalThis as {
  __QILIN_TRANSPORT__?: { loadBundle(url: string): Promise<void> }
}
...
afterEach(() => {
  ...
  delete transportGlobal.__QILIN_TRANSPORT__
...
describe('bootstrap failure rendering', () => {
  it('leaves the loading page visible when a carrier owns fatal presentation', async () => {
    ...
    expect(container.querySelector('[data-qilin-boot-spinner]')).not.toBeNull()
    await entry.dispose()
  })
  ...
  it('renders a malformed boot manifest', async () => {
    await expectBootFailure(() => {
      installFacade()
      delete win.__QILIN_BOOT__
    }, 'window.__QILIN_BOOT__ is missing or not an object')
  })
...
describe('plugin activation', () => {
  it('prefetches a parser-loaded immediate row through the injected bundle transport', async () => {
    ...
    transportGlobal.__QILIN_TRANSPORT__ = {
...
it('draws the shared boot page before Host injections and resumes without replacing the document', async () => {
  ...
  vi.stubGlobal('__QILIN_BOOT_READY__', gate)
  const container = document.createElement('div')
  document.body.append(container)
  ...
  try {
    const page = container.querySelector('[data-qilin-boot]')
    const spinner = container.querySelector('[data-qilin-boot-spinner]')
    ...
    expect(container.querySelector('[data-qilin-boot]')).toBe(page)

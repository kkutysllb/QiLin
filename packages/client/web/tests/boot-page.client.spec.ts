...
describe('BootPage', () => {
  it('draws the loading skeleton before any plugin state arrives', () => {
    const { el } = mount()
    expect(el.firstElementChild?.getAttribute('data-qilin-boot')).toBe('')
    expect(el.textContent).toContain('QiLin')
  ...
  it('keeps loading while entries are active or loading', () => {
    const { el, page } = mount()
    page.setTotal(2)
    const spinner = el.querySelector<HTMLElement>('[data-qilin-boot-spinner]')
    expect(spinner?.style.getPropertyValue('--qilin-boot-arc')).toBe('72deg')
    page.setState('a', 'active')
    expect(spinner?.style.getPropertyValue('--qilin-boot-arc')).toBe('180deg')
    page.setState('b', 'loading')
    expect(el.querySelector('[data-qilin-boot-spinner]')).toBe(spinner)
    page.setState('b', 'active')
    expect(spinner?.style.getPropertyValue('--qilin-boot-arc')).toBe('288deg')
    expect(el.textContent).toContain('Loading plugins…')

...
**Runtime invariant:** No companion is published. The Vite entry shell provides boot glue and module-table seeding, emits no Kylin events, and holds no cross-plugin mutable state; the boot chain (loading page → settled → one-flip UI) is verified by the web smoke e2e against the real carrier.

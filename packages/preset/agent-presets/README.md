...
| `includeUserRoot` | `true` | Append `<qilinHome>/.agent-presets` as a `user` root, after every configured root |

The generated [configuration catalog](../../../docs/config-catalog.md#deepseek-aiqilin-agent-presets) is the exhaustive source for every accepted field and its JSDoc.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#deepseek-aiqilin-agent-presets) — every accepted config field and its source declaration.

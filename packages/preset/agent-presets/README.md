...
The presets you can choose from come from two places: the presets shipped inside this package under `presets/`, and your own presets under `<qilinHome>/.agent-presets`. The picker shows each preset's display name and description; a preset whose composition cannot load is listed with the reason rather than hidden, so you can see what to fix or delete.

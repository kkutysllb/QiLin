...
describe('local attachment service', () => {
  ...
  it('saves and reads through the service boundary', async () => {
    const qilinHome = await mkdtemp(join(tmpdir(), 'qilin-attachment-service-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { qilinHome })
      ...
      await rm(qilinHome, { recursive: true, force: true })
    }
  })

  it('commits a fully prepared image batch in input order', async () => {
    const qilinHome = await mkdtemp(join(tmpdir(), 'qilin-attachment-batch-success-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { qilinHome })
  ...
  it.each([3, 4] as const)('admits a 16-bit %s-channel PNG as an 8-bit normalized object', async (channels) => {
    const qilinHome = await mkdtemp(join(tmpdir(), 'qilin-attachment-16-bit-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { qilinHome })
  ...
  it('prepares every batch member before any write', async () => {
    const qilinHome = await mkdtemp(join(tmpdir(), 'qilin-attachment-batch-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { qilinHome })
  ...
  it('validates without persisting: a rejected image leaves no storage root behind', async () => {
    const qilinHome = await mkdtemp(join(tmpdir(), 'qilin-attachment-validate-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { qilinHome })
      ...
      const limited = new LocalAttachmentStore(new Context(), { qilinHome, maxImageBytes: 1 })

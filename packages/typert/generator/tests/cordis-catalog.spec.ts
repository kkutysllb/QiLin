...
const SOURCE_LINK_POLICY: CordisCatalogPolicy = {
  ...
  typeLinkExemptions: {},
  inheritedEvents: [{ name: 'ready', summary: 'Ready.', source: 'vendor/kylin/src/events.ts:9' }],
  inheritedServices: [{ name: 'ctx.root', summary: 'Root.', source: 'vendor/kylin/src/context.ts:12' }],
}
...
describe('Typert-backed Cordis catalog', () => {
  it('omits subsystem source lines while preserving inherited Cordis source lines', () => {
    ...
    expect(inherited).toContain('([`vendor/kylin/src/events.ts:9`](../../vendor/kylin/src/events.ts))')
    expect(inherited).toContain('([`vendor/kylin/src/context.ts:12`](../../vendor/kylin/src/context.ts))')
  })
  ...
  it('resolves each key to the declaration a caller meets, and drops keys no plugin provides', { timeout: 480_000 }, () => {
    ...
    expect(byKey.has('qilinHomePath')).toBe(false)

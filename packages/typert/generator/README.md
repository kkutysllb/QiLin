...
The root export includes the model-driven extraction, completeness checks, and deterministic text renderers used by this repository's Kylin catalogs. They accept a `KylinCatalogPolicy`; repository-owned type links, foundation and exemption classifications, and inherited Kylin entries stay in `scripts/gen-cordis-catalog.ts` and are passed in explicitly, so this package contains projection mechanics, not a hidden copy of the repository's documentation taxonomy.

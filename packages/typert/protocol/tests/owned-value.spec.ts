...
describe('TypertOwnedValue', () => {
  ...
  it('recognizes a separately produced marker without accepting false markers or borrowed values', () => {
    expect(TYPERT_OWNED_VALUE).toBe(Symbol.for('qilin.typert.owned-value'))
    const siblingBundle = { [Symbol.for('qilin.typert.owned-value')]: true, value: {}, [Symbol.dispose]() {} }
    expect(isTypertOwnedValue(siblingBundle)).toBe(true)
    for (const value of [undefined, null, 1, {}, { [Symbol.for('qilin.typert.owned-value')]: false }]) {

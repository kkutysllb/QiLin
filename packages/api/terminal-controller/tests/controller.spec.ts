...
import { SubprocessExecutableNotFoundError, type SubprocessRuntime, type SubprocessTerminalEnvironment, type SubprocessTerminalHandle } from '@qilin/subprocess'
...
describe('TerminalController', () => {
  ...
  it('uses the Session sandbox policy to confine its selected shell', async () => {
    ...
    expect(subprocess.spawnTerminal).toHaveBeenCalledWith(expect.objectContaining({ argv: ['sandbox-runner', '/bin/bash', '--noprofile', '--norc', '-i'], env: { QILIN_SESSION_ID: agent.id }, graceMs: 100 }))
  })
...
it.skipIf(process.platform === 'win32')('runs a real interactive shell with completion, TERM and live window dimensions', async () => {
  const cwd = await mkdtemp(join(tmpdir(), 'qilin-web-terminal-'))
  const ctx = new Context()

...
describe('bundled Office skills', () => {
  it('loads each packaged body and removes all candidates on disposal', async () => {
    ...
    try {
      ...
      for (const skill of catalog) {
        ...
        expect(skill).toMatchObject({ source: 'bundled', provider: 'qilin-office', invocation: { modelInvocable: true, userInvocable: true } })
  ...
  it('loads relocated resources through a real cordis.yml composition', async () => {
    const root = await mkdtemp(join(tmpdir(), 'qilin-office-skills-'))
    const ctx = new Context()
    try {
  ...
  it.each([
    ...
    const root = await mkdtemp(join(tmpdir(), 'qilin-office-metadata-'))
    const ctx = new Context()
    try {
  ...
  it('rejects relative resource paths and incomplete asset trees before registration', async () => {
    const root = await mkdtemp(join(tmpdir(), 'qilin-office-invalid-'))
    const ctx = new Context()
    try {
      await ctx.plugin(SkillRegistry)

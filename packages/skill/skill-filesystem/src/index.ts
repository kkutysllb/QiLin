...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  private readonly qilinHome: string
  ...
  constructor(
    ...
    this.qilinHome = resolveQilinHome(config.qilinHome)
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.includeDefaultRoots && cwd !== undefined) {
      ...
      roots.push(
        { path: join(projectRoot, '.qilin/skills'), source: 'project-qilin', rank: PROJECT_QILIN_RANK, projectRoot },
    ...
    if (this.includeDefaultRoots) {
      roots.push(
        { path: join(this.qilinHome, 'skills'), source: 'user-qilin', rank: USER_QILIN_RANK, skipSystem: true },

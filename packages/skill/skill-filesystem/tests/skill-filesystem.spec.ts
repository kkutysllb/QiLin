...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(noGit, '.qilin/skills'), 'fallback-root', 'Fallback root')
  ...
  it.each([false, true])('publishes regular-file paths for linked skills while retaining their resource roots (filesystem service: %s)', async (withFileSystem) => {
    ...
    const fiber = ctx.plugin(SkillFileSystem, { qilinHome: join(home, '.qilin'), agentsHome: join(home, '.agents'), watch: false })
    await fiber
    ...
    try {
      ...
      for (const name of ['linked-dir', 'linked-flat']) {
        ...
        expect(loaded?.resourceBase).toEqual({ kind: 'directory', path: name === 'linked-dir' ? join(home, '.qilin/skills', name) : join(home, '.qilin/skills') })
        expect((await lstat(path)).isFile()).toBe(true)
      }
      await writeFile(join(external, 'replacement.md'), '---\nname: linked-flat\ndescription: Replacement\n---\n\nReplacement body.\n')
      await rm(join(home, '.qilin/skills/linked-flat.md'))
      await symlink(join(external, 'replacement.md'), join(home, '.qilin/skills/linked-flat.md'))

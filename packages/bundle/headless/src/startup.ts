...
Examples:
  ...
  echo "run the tests" | qilin --profile headless   read the task from stdin
  qilin --profile headless --json "run the tests"   emit machine-readable run events
  qilin --profile headless --session-id session-… "continue"   resume an existing Session
`)
}
...
export function apply(ctx: Context): void {
  ...
  if (jsonRequested(ctx.get('cmdlineArgs')?.get() ?? [])) {
    program.error = (message: string, errorOptions?: Parameters<Command['error']>[1]): never => {
      ...
      // The JSON contract keeps stderr to `qilin:` diagnostics, so commander's
  ...
  program.action(() => {
    ...
    if (program.args.length > 0 && joined.trim() === '') {
      program.error('error: a task is required, for example: qilin --profile headless "run the tests"')
    }

...
function currentPreset(header: AdoptableHeader, events: Iterable<SessionEvent>, sessionId: SessionId): string | undefined {
  ...
  for (const event of events) {
    // Owned by qilin-agent-presets, which this bundle does not compose, so the
...
async function resolveAgent(
  ...
  if (query === undefined) {
    throw new Error('headless --session-id requires the sessionQuery service; qilin-base provides it')
  }
...
async function run(ctx: Context, config: Config, io: HeadlessIo): Promise<void> {
  ...
  const task = config.task === undefined || config.task === '-'
    ...
    : config.task
  if (task.trim() === '') {
    throw new Error('a task is required, for example: qilin --profile headless "run the tests"')
  }

...
export const Config: z<Config> = z.object({
  label: z.string().default('qilin web'),
...
function webSurfacePrompt(webUrl: string): string {
  ...
  return `You are interacting with the user through the QiLin Web GUI at ${webUrl}. `
    ...
    + 'The apps/web Vite entry builds the shell but is not a standalone application because only qilin web injects window.__QILIN_BOOT__. '
      ...
      const announceReady = (): void => {
        ...
        if (handoffBrowser) {
          ...
          void internals.openBrowser(authenticatedUrl).catch((error: unknown) => {
            ...
            console.error(`web-app: could not open the default browser because ${reason}; use the ${config.label} URL printed at startup`)
          })
        }
      }

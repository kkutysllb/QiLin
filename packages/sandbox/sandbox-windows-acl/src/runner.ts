 ...
 * @module @qilin/sandbox-windows-acl/runner
 */

import { SUBPROCESS_CONTROL_ENV, SUBPROCESS_CONTROL_FD } from '@qilin/subprocess/control'
  ...
  try {
    ...
    if (parsed.mode === 'workspace-write') {
      ...
      if (seamManaged) {
        ...
        ownedTempDir = mkdtempSync(join(parsed.temp, 'qilin-'))

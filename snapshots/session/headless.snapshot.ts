...
async function verifyProviderCwdResume(
  ...
  const providerCwd = scenario.manifest.environment?.QILIN_SNAPSHOT_PROVIDER_CWD
  ...
  const otherHostCwd = await mkdtemp(join(tmpdir(), 'qilin-provider-resume-'))
  const env = {
    QILIN_HOME: join(cwd, '.qilin'),
    QILIN_SNAPSHOT: 'replay',
    QILIN_SNAPSHOT_FILE: fixture,
    QILIN_SNAPSHOT_PROVIDER: model.provider,
    QILIN_SNAPSHOT_MODEL: model.model,
    QILIN_PERMISSION_MODE: 'read-only',
  ...
  try {
    const refused = await runLoaderSmoke({
      ...
      env: { ...env, QILIN_SNAPSHOT_PROVIDER_CWD: `${providerCwd}/other` },
    })
    ...
    const resumed = await runLoaderSmoke({
      ...
      env: { ...env, QILIN_SNAPSHOT_PROVIDER_CWD: providerCwd },
    })
        ...
        result = await runLoaderSmoke({
          ...
          env: {
            ...
            ...(scenario.name === 'mcp-resources' || scenario.name === 'mcp-resources-ptc' ? {
              QILIN_MCP_RESOURCES_FIXTURE: join(repoRoot, 'packages/mcp/mcp-client/tests/fixtures/resources-server.ts'),
            } : {}),
            NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
            QILIN_TELEMETRY_DISABLED: '1',
            ...(mcpDemo === undefined ? {} : { QILIN_MCP_DEMO_URL: mcpDemo.url }),
          },
          prepare: async (cwd) => {
            ...
            if (mcpDemo !== undefined) {
              const profileDir = join(cwd, '.qilin/profiles/headless')
          ...
          inspect: async (cwd) => {
            ...
            if (mcpDemo !== undefined) {
              ...
              const saved = await readFile(join(cwd, '.qilin/profiles/headless/cordis.patch.yml'), 'utf8')
